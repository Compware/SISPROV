...
type
  TfrmRelUnidadesImp = class(TfrmRelPad)
    TabSheet1: TTabSheet;
    GroupBox1: TGroupBox;
    Label3: TLabel;
    Label4: TLabel;
...
var
  frmRelUnidadesImp: TfrmRelUnidadesImp;
  strSQL:String;
implementation
...
procedure TfrmRelUnidadesImp.cbTodosClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmRelUnidadesImp.btImprimirClick(Sender: TObject);
...
begin
  ...
  qyUnidades.Open;
  frxReport1.LoadFromFile(sPathLayOut + '\RelUnidadesImp.fr3');
  frxReport1.ShowReport;
  qyUnidades.Close;
end;

procedure TfrmRelUnidadesImp.btLocProvinciaButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
...
procedure TfrmRelUnidadesImp.btLocProvinciaChange(Sender: TObject);
begin
  inherited;
  if btLocProvincia.Text = '' then
    edProvincia.Clear
...
procedure TfrmRelUnidadesImp.btLocProvinciaExit(Sender: TObject);
begin
  inherited;
  if btLocProvincia.Text <> '' then
  begin
...
procedure TfrmRelUnidadesImp.btLocMunicipioButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
  inherited;
  frmLocalizar := TfrmLocalizar.Create(application);
...
procedure TfrmRelUnidadesImp.btLocMunicipioChange(Sender: TObject);
begin
  inherited;
  if btLocMunicipio.Text = '' then
    edMunicipio.Clear
...
procedure TfrmRelUnidadesImp.btLocMunicipioExit(Sender: TObject);
begin
  inherited;
  if btLocMunicipio.Text <> '' then
    begin
...
procedure TfrmRelUnidadesImp.FormCreate(Sender: TObject);
begin
  inherited;
  strSQL := qyUnidades.SQL.Text;
end;

procedure TfrmRelUnidadesImp.cbAtivasClick(Sender: TObject);
begin
  inherited;
  if cbAtivas.Checked then
    cbInativas.Checked := false;
...
procedure TfrmRelUnidadesImp.cbInativasClick(Sender: TObject);
begin
  inherited;
  if cbInativas.Checked then
    cbAtivas.Checked := false;
