unit UfrmRelTituloReceberAVencer;
...
type
  TfrmRelTituloReceberAVencer = class(TfrmRelPad)
    RvDtCnTituloReceberAVencer: TRvDataSetConnection;
    RvDtCnTituloReceberAVencerConfRel: TRvDataSetConnection;
    RvSysTituloReceberAVencer: TRvSystem;
    RvPrjTituloReceberAVencer: TRvProject;
    ...
    procedure RvPrjTituloReceberAVencerCreate(Sender: TObject);
...
var
  frmRelTituloReceberAVencer: TfrmRelTituloReceberAVencer;
...
procedure TfrmRelTituloReceberAVencer.qyGeralBeforeOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmRelTituloReceberAVencer.RvPrjTituloReceberAVencerCreate(Sender: TObject);
begin
  inherited;
  RvPrjTituloReceberAVencer.ProjectFile := sPathLayout +'\RelTituloReceberAVencer.rav';
end;

procedure TfrmRelTituloReceberAVencer.dtateExit(Sender: TObject);
begin
  inherited;
...
procedure TfrmRelTituloReceberAVencer.btImprimirClick(Sender: TObject);
begin
  inherited;
  if (dtDe.Text = '  /  /    ') then
  begin
  ...
  try
    ...
    qyGeral.open;
    RvPrjTituloReceberAVencer.Execute;
  finally
    qyGeral.Close;
