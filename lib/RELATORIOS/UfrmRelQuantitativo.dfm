inherited frmRelQuantitativo: TfrmRelQuantitativo
  Left = 620
  Top = 190
  Caption = 'Relat'#243'rio Quantitativo'
  ClientHeight = 420
  ClientWidth = 510
  Font.Name = 'Arial'
  OldCreateOrder = True
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 14
  object Label2: TLabel [1]
    Left = 156
    Top = 23
    Width = 253
    Height = 29
    Caption = 'Relat'#243'rio Quantitativo'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -24
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  inherited pgFiltros: TPageControl
    Width = 359
    Height = 279
    ActivePage = pgFiltro
    Anchors = [akLeft, akTop, akRight, akBottom]
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    object pgFiltro: TTabSheet
      Caption = 'Filtro'
      object GroupBox1: TGroupBox
        Left = 8
        Top = 8
        Width = 332
        Height = 81
        Caption = 'Per'#237'odo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        object Label3: TLabel
          Left = 16
          Top = 24
          Width = 42
          Height = 16
          Caption = 'In'#237'cio:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label4: TLabel
          Left = 192
          Top = 24
          Width = 39
          Height = 16
          Caption = 'Final:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object edDataInicio: TDateEdit
          Left = 16
          Top = 40
          Width = 121
          Height = 21
          NumGlyphs = 2
          TabOrder = 0
        end
        object edDataFinal: TDateEdit
          Left = 192
          Top = 40
          Width = 121
          Height = 21
          NumGlyphs = 2
          TabOrder = 1
        end
      end
    end
  end
  inherited Panel2: TPanel
    Left = -474
    Top = 364
    Width = 977
    Anchors = [akRight, akBottom]
    inherited btImprimir: TBitBtn
      Left = 714
      Anchors = [akRight, akBottom]
      Font.Name = 'MS Sans Serif'
      OnClick = btImprimirClick
    end
    inherited btCancelar: TBitBtn
      Left = 847
      Anchors = [akRight, akBottom]
      Font.Name = 'MS Sans Serif'
    end
  end
  inherited qyGeral: TIBQuery
    SQL.Strings = (
      'SELECT FAIXA,'
      
        #9'CASE FL_SEXO WHEN '#39'M'#39' THEN '#39'MASCULINO'#39' WHEN '#39'F'#39' THEN '#39'FEMININO'#39 +
        ' END FL_SEXO,'
      '    SUM(TESTAGEM_ACONSELHADOS) TESTAGEM_ACONSELHADOS,'
      '    SUM(TESTAGEM_TESTADOS) TESTAGEM_TESTADOS,'
      #9'SUM(TESTAGEM_NEGATIVO) TESTAGEM_NEGATIVO,'
      #9'SUM(TESTAGEM_POSITIVO) TESTAGEM_POSITIVO,'
      #9'SUM(TESTAGEM_INDETERMINADO) TESTAGEM_INDETERMINADO,'
      #9'SUM(TESTAGEM_TOTAL) TESTAGEM_TOTAL,'
      #9'SUM(PACIENTES_COM_IDENTIFICACAO) PACIENTES_COM_IDENTIFICACAO,'
      #9'SUM(PACIENTES_COM_1_CONSULTA) PACIENTES_COM_1_CONSULTA,'
      
        #9'SUM(PACIENTES_COM_1_CONSULTA_E_TARV) PACIENTES_COM_1_CONSULTA_E' +
        '_TARV,'
      
        #9'SUM(PACIENTES_COM_1_CONSULTA_SEM_TARV) PACIENTES_COM_1_CONSULTA' +
        '_SEM_TARV,'
      #9'SUM(CN_CD4_MENOR_350) CN_CD4_MENOR_350,'
      #9'SUM(CN_CD4_351_500) CN_CD4_351_500,'
      #9'SUM(CN_CD4_MAIOR_500) CN_CD4_MAIOR_500,'
      #9'SUM(CN_CD4_NULLO) CN_CD4_NULLO,'
      #9'SUM(CN_OMS_I) CN_OMS_I,'
      #9'SUM(CN_OMS_II) CN_OMS_II,'
      #9'SUM(CN_OMS_III) CN_OMS_III,'
      #9'SUM(CN_OMS_IV) CN_OMS_IV,'
      #9'SUM(CN_OMS_NULLO) CN_OMS_NULLO,'
      #9'SUM(CN_SEM_CD4_E_OMS) CN_SEM_CD4_E_OMS'
      'FROM'
      '    (select'
      '        TESTAGEM_ACONSELHADOS,'
      #9#9'TESTAGEM_TESTADOS,'
      #9#9'TESTAGEM_NEGATIVO,'
      #9#9'TESTAGEM_POSITIVO,'
      #9#9'TESTAGEM_INDETERMINADO,'
      #9#9'TESTAGEM_TOTAL,'
      #9#9'PACIENTES_COM_IDENTIFICACAO,'
      #9#9'PACIENTES_COM_1_CONSULTA,'
      #9#9'PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9'PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9'CN_CD4_MENOR_350,'
      #9#9'CN_CD4_351_500,'
      #9#9'CN_CD4_MAIOR_500,'
      #9#9'CN_CD4_NULLO,'
      #9#9'CN_OMS_I,'
      #9#9'CN_OMS_II,'
      #9#9'CN_OMS_III,'
      #9#9'CN_OMS_IV,'
      #9#9'CN_OMS_NULLO,'
      #9#9'CN_SEM_CD4_E_OMS,'
      '        FL_SEXO,'
      '        CASE'
      '        WHEN NR_IDADE < 1 THEN '#39'<1 Ano'#39
      '        WHEN NR_IDADE >=1 AND NR_IDADE <=3 THEN '#39'1-3 Anos'#39
      '        WHEN NR_IDADE >=4 AND NR_IDADE <=5 THEN '#39'4-5 Anos'#39
      '        WHEN NR_IDADE >=6 AND NR_IDADE <=9 THEN '#39'6-9 Anos'#39
      '        WHEN NR_IDADE >=10 AND NR_IDADE <=14 THEN '#39'10-14 Anos'#39
      '        WHEN NR_IDADE >=15 AND NR_IDADE <=19 THEN '#39'15-19 Anos'#39
      '        WHEN NR_IDADE >=20 AND NR_IDADE <=24 THEN '#39'20-24 Anos'#39
      '        WHEN NR_IDADE >=25 AND NR_IDADE <=49 THEN '#39'25-49 Anos'#39
      '        WHEN NR_IDADE >=50 THEN '#39'>50 Anos'#39
      '        else '#39'N'#227'o Consta'#39
      '        END AS FAIXA'
      '    from'
      '        (SELECT'
      '            NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'COUNT(NR_IDADE) TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      '        FROM MAPA_TESTAGEM_PACIENTE MTP'
      '        INNER JOIN MAPA_TESTAGEM MT'
      '            ON MT.CD_MAPA_TESTAGEM = MTP.CD_MAPA_TESTAGEM'
      '        WHERE'
      '         MT.DT_MAPA >= :DATA_INICIO AND MT.DT_MAPA <= :DATA_FIM'
      '                AND FL_ACONSELHADO='#39'S'#39
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        union ALL'
      '        SELECT'
      '            NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'COUNT(NR_IDADE) TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      '        FROM MAPA_TESTAGEM_PACIENTE MTP'
      '        INNER JOIN MAPA_TESTAGEM MT'
      '            ON MT.CD_MAPA_TESTAGEM = MTP.CD_MAPA_TESTAGEM'
      '        WHERE'
      
        '            MT.DT_MAPA >= :DATA_INICIO AND MT.DT_MAPA <= :DATA_F' +
        'IM'
      
        '            AND (FL_RESULTADO_POSITIVO='#39'S'#39' OR FL_RESULTADO_NEGAT' +
        'IVO='#39'S'#39' OR FL_RESULTADO_INDETERMINADO='#39'S'#39')'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'COUNT(NR_IDADE) TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM MAPA_TESTAGEM_PACIENTE MTP'
      #9#9'INNER JOIN MAPA_TESTAGEM MT'
      #9#9#9'ON MT.CD_MAPA_TESTAGEM = MTP.CD_MAPA_TESTAGEM'
      #9#9'WHERE'
      #9#9#9'MT.DT_MAPA >= :DATA_INICIO AND MT.DT_MAPA <= :DATA_FIM'
      #9#9#9'AND FL_RESULTADO_NEGATIVO='#39'S'#39
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'COUNT(NR_IDADE) TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM MAPA_TESTAGEM_PACIENTE MTP'
      #9#9'INNER JOIN MAPA_TESTAGEM MT'
      #9#9#9'ON MT.CD_MAPA_TESTAGEM = MTP.CD_MAPA_TESTAGEM'
      #9#9'WHERE'
      #9#9#9'MT.DT_MAPA >= :DATA_INICIO AND MT.DT_MAPA <= :DATA_FIM'
      #9#9#9'AND FL_RESULTADO_POSITIVO='#39'S'#39
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'COUNT(NR_IDADE) TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM MAPA_TESTAGEM_PACIENTE MTP'
      #9#9'INNER JOIN MAPA_TESTAGEM MT'
      #9#9#9'ON MT.CD_MAPA_TESTAGEM = MTP.CD_MAPA_TESTAGEM'
      #9#9'WHERE'
      #9#9#9'MT.DT_MAPA >= :DATA_INICIO AND MT.DT_MAPA <= :DATA_FIM'
      #9#9#9'AND FL_RESULTADO_INDETERMINADO='#39'S'#39
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'COUNT(NR_IDADE) TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM MAPA_TESTAGEM_PACIENTE MTP'
      #9#9'INNER JOIN MAPA_TESTAGEM MT'
      #9#9#9'ON MT.CD_MAPA_TESTAGEM = MTP.CD_MAPA_TESTAGEM'
      #9#9'WHERE'
      #9#9#9'MT.DT_MAPA >= :DATA_INICIO AND MT.DT_MAPA <= :DATA_FIM'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'IDADE NR_IDADE,'
      #9#9#9'TP_SEXO FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'COUNT(IDADE) PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM NOTIFICACAO N'
      #9#9'INNER JOIN PACIENTE P'
      #9#9#9'ON P.CD_PACIENTE = N.CD_PACIENTE'
      #9#9'WHERE'
      
        #9#9#9'DT_DIAGNOSTICO >= :DATA_INICIO AND DT_DIAGNOSTICO <= :DATA_FI' +
        'M'
      #9#9#9'AND TP_ENTRADA = 1'
      #9#9'GROUP BY IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND TP_ENTRADA = 1)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      
        #9#9#9#9'AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIMEIRO_TARV <=' +
        ' :DATA_FIM'
      #9#9#9#9'AND TP_ENTRADA = 1)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND DT_PRIMEIRO_TARV IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 1)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'COUNT(NR_IDADE) CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC <= 350'
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'COUNT(NR_IDADE) CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC > 350 AND VL_CD4_PC <= 500'
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'COUNT(NR_IDADE) CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC > 500'
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'COUNT(NR_IDADE) CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'COUNT(NR_IDADE) CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'I'#39
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'COUNT(NR_IDADE) CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'II'#39
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'COUNT(NR_IDADE) CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'III'#39
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'COUNT(NR_IDADE) CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'IV'#39
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'COUNT(NR_IDADE) CN_OMS_NULLO,'
      #9#9#9'0 CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 TESTAGEM_ACONSELHADOS,'
      #9#9#9'0 TESTAGEM_TESTADOS,'
      #9#9#9'0 TESTAGEM_NEGATIVO,'
      #9#9#9'0 TESTAGEM_POSITIVO,'
      #9#9#9'0 TESTAGEM_INDETERMINADO,'
      #9#9#9'0 TESTAGEM_TOTAL,'
      #9#9#9'0 PACIENTES_COM_IDENTIFICACAO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 CN_CD4_MENOR_350,'
      #9#9#9'0 CN_CD4_351_500,'
      #9#9#9'0 CN_CD4_MAIOR_500,'
      #9#9#9'0 CN_CD4_NULLO,'
      #9#9#9'0 CN_OMS_I,'
      #9#9#9'0 CN_OMS_II,'
      #9#9#9'0 CN_OMS_III,'
      #9#9#9'0 CN_OMS_IV,'
      #9#9#9'0 CN_OMS_NULLO,'
      #9#9#9'COUNT(NR_IDADE) CN_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC IS NULL AND VL_CD4_PC IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 1'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'))'
      'GROUP BY FL_SEXO, FAIXA'
      'ORDER BY FL_SEXO ASC, CASE WHEN FAIXA = '#39'<1 Ano'#39' THEN '#39'1'#39
      '              WHEN FAIXA = '#39'1-3 Anos'#39' THEN '#39'2'#39
      '              WHEN FAIXA = '#39'4-5 Anos'#39' THEN '#39'3'#39
      '              WHEN FAIXA = '#39'6-9 Anos'#39' THEN '#39'4'#39
      '              WHEN FAIXA = '#39'10-14 Anos'#39' THEN '#39'5'#39
      '              WHEN FAIXA = '#39'15-19 Anos'#39' THEN '#39'6'#39
      '              WHEN FAIXA = '#39'20-24 Anos'#39' THEN '#39'7'#39
      '              WHEN FAIXA = '#39'25-49 Anos'#39' THEN '#39'8'#39
      '              WHEN FAIXA = '#39'>50 Anos'#39' THEN '#39'9'#39
      '              ELSE FAIXA END ASC')
    Left = 8
    Top = 360
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end>
    object qyGeralFAIXA: TIBStringField
      FieldName = 'FAIXA'
      Required = True
      FixedChar = True
      Size = 10
    end
    object qyGeralFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      FixedChar = True
      Size = 9
    end
    object qyGeralTESTAGEM_ACONSELHADOS: TLargeintField
      FieldName = 'TESTAGEM_ACONSELHADOS'
    end
    object qyGeralTESTAGEM_TESTADOS: TLargeintField
      FieldName = 'TESTAGEM_TESTADOS'
    end
    object qyGeralTESTAGEM_NEGATIVO: TLargeintField
      FieldName = 'TESTAGEM_NEGATIVO'
    end
    object qyGeralTESTAGEM_POSITIVO: TLargeintField
      FieldName = 'TESTAGEM_POSITIVO'
    end
    object qyGeralTESTAGEM_INDETERMINADO: TLargeintField
      FieldName = 'TESTAGEM_INDETERMINADO'
    end
    object qyGeralTESTAGEM_TOTAL: TLargeintField
      FieldName = 'TESTAGEM_TOTAL'
    end
    object qyGeralPACIENTES_COM_IDENTIFICACAO: TLargeintField
      FieldName = 'PACIENTES_COM_IDENTIFICACAO'
    end
    object qyGeralPACIENTES_COM_1_CONSULTA: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA'
    end
    object qyGeralPACIENTES_COM_1_CONSULTA_E_TARV: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA_E_TARV'
    end
    object qyGeralPACIENTES_COM_1_CONSULTA_SEM_TA: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA_SEM_TA'
    end
    object qyGeralCN_CD4_MENOR_350: TLargeintField
      FieldName = 'CN_CD4_MENOR_350'
    end
    object qyGeralCN_CD4_351_500: TLargeintField
      FieldName = 'CN_CD4_351_500'
    end
    object qyGeralCN_CD4_MAIOR_500: TLargeintField
      FieldName = 'CN_CD4_MAIOR_500'
    end
    object qyGeralCN_CD4_NULLO: TLargeintField
      FieldName = 'CN_CD4_NULLO'
    end
    object qyGeralCN_OMS_I: TLargeintField
      FieldName = 'CN_OMS_I'
    end
    object qyGeralCN_OMS_II: TLargeintField
      FieldName = 'CN_OMS_II'
    end
    object qyGeralCN_OMS_III: TLargeintField
      FieldName = 'CN_OMS_III'
    end
    object qyGeralCN_OMS_IV: TLargeintField
      FieldName = 'CN_OMS_IV'
    end
    object qyGeralCN_OMS_NULLO: TLargeintField
      FieldName = 'CN_OMS_NULLO'
    end
    object qyGeralCN_SEM_CD4_E_OMS: TLargeintField
      FieldName = 'CN_SEM_CD4_E_OMS'
    end
  end
  inherited frxReport1: TfrxReport
    ReportOptions.CreateDate = 41894.334857743100000000
    ReportOptions.LastChange = 42676.140018587960000000
    Left = 72
    Top = 360
    Datasets = <
      item
        DataSet = frxDBAcompanhados
        DataSetName = 'frxDBAcompanhados'
      end
      item
        DataSet = frxDBAcumulados
        DataSetName = 'frxDBAcumulados'
      end
      item
        DataSet = frxDBCasosNovos
        DataSetName = 'frxDBCasosNovos'
      end
      item
        DataSet = frxDBConsolidados
        DataSetName = 'frxDBConsolidados'
      end
      item
        DataSet = frxDBDataset1
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = frxDBTransferidos
        DataSetName = 'frxDBTransferidos'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      OnAfterPrint = 'Page1OnAfterPrint'
      object ReportTitle1: TfrxReportTitle
        Height = 207.874150000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture1: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo1: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'RELAT'#195#8220'RIO QUANTITATIVO')
          ParentFont = False
        end
        object Memo46: TfrxMemoView
          Left = 7.559059999999990000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object MemoPeriodo: TfrxMemoView
          Left = 9.559060000000000000
          Top = 181.417440000000000000
          Width = 260.787570000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Per'#195#173'odo')
          ParentFont = False
        end
        object Memo51: TfrxMemoView
          Left = 60.472480000000000000
          Top = 158.740260000000000000
          Width = 245.669450000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBDataset1."DS_UNIDADE"]')
          ParentFont = False
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 480.000310000000000000
        Width = 1046.929810000000000000
        object Shape3: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo9: TfrxMemoView
          Left = 84.929190000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 11.338590000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 998.795920000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Left = 943.882500000000000000
          Top = 1.779530000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
      object GroupHeader1: TfrxGroupHeader
        Height = 45.354360000000000000
        Top = 332.598640000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBCasosNovos."FL_SEXO"'
        PrintChildIfInvisible = True
        object Shape1: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 45.354360000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo81: TfrxMemoView
          Left = 3.779530000000000000
          Top = 7.559060000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sexo')
          ParentFont = False
        end
        object Memo82: TfrxMemoView
          Left = 0.779530000000000000
          Top = 22.897650000000000000
          Width = 66.141732280000000000
          Height = 11.338590000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          DataField = 'FL_SEXO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."FL_SEXO"]')
          ParentFont = False
        end
        object Shape4: TfrxShapeView
          Left = 68.031540000000000000
          Width = 68.031540000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Shape5: TfrxShapeView
          Left = 136.063080000000000000
          Width = 73.700794720000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo2: TfrxMemoView
          Left = 69.370130000000000000
          Top = 1.889763780000000000
          Width = 64.252010000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Faixa Et'#195#161'ria')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo4: TfrxMemoView
          Left = 134.842610000000000000
          Top = 1.779530000000000000
          Width = 75.590600000000000000
          Height = 41.574830000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'Aconselhados')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape7: TfrxShapeView
          Left = 209.763781970000000000
          Width = 51.023624490000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo5: TfrxMemoView
          Left = 210.653680000000000000
          Top = 1.889763780000000000
          Width = 49.133890000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Testados')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape8: TfrxShapeView
          Left = 260.787570000000000000
          Width = 49.511813460000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo7: TfrxMemoView
          Left = 260.456998030000000000
          Top = 1.889763780000000000
          Width = 49.133890000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Testados'
            'Negativo')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape9: TfrxShapeView
          Left = 310.252031970000000000
          Width = 49.133890000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo14: TfrxMemoView
          Left = 309.921460000000000000
          Top = 3.669293780000000000
          Width = 49.133890000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Testados'
            'Positivo')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape10: TfrxShapeView
          Left = 359.385921970000000000
          Width = 52.913420000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo16: TfrxMemoView
          Left = 359.055350000000000000
          Top = 3.669293780000000000
          Width = 52.913420000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Testados'
            'Indeter-'
            'minado')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape11: TfrxShapeView
          Left = 412.299341970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo17: TfrxMemoView
          Left = 411.968770000000000000
          Top = 3.669293780000000000
          Width = 30.236240000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Total')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape12: TfrxShapeView
          Left = 442.535581970000000000
          Width = 56.692950000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo18: TfrxMemoView
          Left = 442.205010000000000000
          Top = 0.889763780000000000
          Width = 56.692950000000000000
          Height = 43.464566930000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'com'
            'Identifi-'
            'ca'#195#167#195#163'o')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape13: TfrxShapeView
          Left = 499.228531970000000000
          Width = 56.692950000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo19: TfrxMemoView
          Left = 498.897960000000000000
          Top = 3.669293780000000000
          Width = 56.692950000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'com 1'#194#170
            'consulta')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape14: TfrxShapeView
          Left = 555.921481970000000000
          Width = 56.692950000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo20: TfrxMemoView
          Left = 555.590910000000000000
          Top = 2.669293780000000000
          Width = 56.692950000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'que'
            'iniciaram'
            'TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape15: TfrxShapeView
          Left = 612.614431970000000000
          Width = 56.692950000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo21: TfrxMemoView
          Left = 612.283860000000000000
          Top = 3.669293780000000000
          Width = 56.692950000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'em pr'#195#169
            'TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape16: TfrxShapeView
          Left = 669.307381970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo22: TfrxMemoView
          Left = 665.197280000000000000
          Top = 3.669293780000000000
          Width = 45.354360000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '<=350')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape17: TfrxShapeView
          Left = 707.102681970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo23: TfrxMemoView
          Left = 710.551640000000000000
          Top = 3.669293780000000000
          Width = 30.236240000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '351-500')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape18: TfrxShapeView
          Left = 744.897981970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo24: TfrxMemoView
          Left = 744.567410000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '>500')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape19: TfrxShapeView
          Left = 782.693281970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo25: TfrxMemoView
          Left = 782.362710000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape20: TfrxShapeView
          Left = 820.488581970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo26: TfrxMemoView
          Left = 820.158010000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS I')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape21: TfrxShapeView
          Left = 858.283881970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo27: TfrxMemoView
          Left = 857.953310000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS II')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape22: TfrxShapeView
          Left = 896.079181970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo28: TfrxMemoView
          Left = 895.748610000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS III')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape23: TfrxShapeView
          Left = 933.874481970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo29: TfrxMemoView
          Left = 933.543910000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS IV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape24: TfrxShapeView
          Left = 971.669781970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo30: TfrxMemoView
          Left = 971.339210000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape25: TfrxShapeView
          Left = 1009.465081970000000000
          Width = 37.417322834645700000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo31: TfrxMemoView
          Left = 1009.134510000000000000
          Top = 3.669293780000000000
          Width = 37.795300000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4 e'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData1: TfrxMasterData
        Height = 17.007874020000000000
        Top = 400.630180000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBCasosNovos
        DataSetName = 'frxDBCasosNovos'
        RowCount = 0
        object Memo58: TfrxMemoView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 17.007874020000000000
          ShowHint = False
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          GapX = 1.000000000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Color = 13750737
          Highlight.Condition = '<Line> mod 2 = 0'
        end
        object Memo84: TfrxMemoView
          Left = 70.031540000000000000
          Top = 1.889763780000000000
          Width = 64.252010000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'FAIXA'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."FAIXA"]')
          ParentFont = False
        end
        object Memo85: TfrxMemoView
          Left = 143.401670000000000000
          Top = 1.889763780000000000
          Width = 56.692913390000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TESTAGEM_ACONSELHADOS'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."TESTAGEM_ACONSELHADOS"]')
          ParentFont = False
        end
        object Memo86: TfrxMemoView
          Left = 215.433210000000000000
          Top = 1.889763780000000000
          Width = 41.574830000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TESTAGEM_TESTADOS'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."TESTAGEM_TESTADOS"]')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 264.346630000000000000
          Top = 1.889763780000000000
          Width = 41.574830000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TESTAGEM_NEGATIVO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."TESTAGEM_NEGATIVO"]')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 313.480520000000000000
          Top = 1.889763780000000000
          Width = 41.574830000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TESTAGEM_POSITIVO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."TESTAGEM_POSITIVO"]')
          ParentFont = False
        end
        object Memo32: TfrxMemoView
          Left = 364.834880000000000000
          Top = 1.889763780000000000
          Width = 41.574830000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TESTAGEM_INDETERMINADO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."TESTAGEM_INDETERMINADO"]')
          ParentFont = False
        end
        object Memo33: TfrxMemoView
          Left = 411.748300000000000000
          Top = 1.889763780000000000
          Width = 30.236240000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TESTAGEM_TOTAL'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."TESTAGEM_TOTAL"]')
          ParentFont = False
        end
        object Memo34: TfrxMemoView
          Left = 447.323130000000000000
          Top = 1.889763780000000000
          Width = 49.133890000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_IDENTIFICACAO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."PACIENTES_COM_IDENTIFICACAO"]')
          ParentFont = False
        end
        object Memo35: TfrxMemoView
          Left = 502.677490000000000000
          Top = 1.889763780000000000
          Width = 49.133890000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."PACIENTES_COM_1_CONSULTA"]')
          ParentFont = False
        end
        object Memo36: TfrxMemoView
          Left = 559.370440000000000000
          Top = 1.889763780000000000
          Width = 49.133890000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA_E_TARV'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."PACIENTES_COM_1_CONSULTA_E_TARV"]')
          ParentFont = False
        end
        object Memo37: TfrxMemoView
          Left = 616.063390000000000000
          Top = 1.889763780000000000
          Width = 49.133890000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA_SEM_TA'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."PACIENTES_COM_1_CONSULTA_SEM_TA"]')
          ParentFont = False
        end
        object Memo38: TfrxMemoView
          Left = 668.976810000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_CD4_MENOR_350'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_CD4_MENOR_350"]')
          ParentFont = False
        end
        object Memo39: TfrxMemoView
          Left = 706.772110000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_CD4_351_500'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_CD4_351_500"]')
          ParentFont = False
        end
        object Memo40: TfrxMemoView
          Left = 744.567410000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_CD4_MAIOR_500'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_CD4_MAIOR_500"]')
          ParentFont = False
        end
        object Memo41: TfrxMemoView
          Left = 782.362710000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_CD4_NULLO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_CD4_NULLO"]')
          ParentFont = False
        end
        object Memo42: TfrxMemoView
          Left = 820.158010000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_OMS_I'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_OMS_I"]')
          ParentFont = False
        end
        object Memo43: TfrxMemoView
          Left = 857.953310000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_OMS_II'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_OMS_II"]')
          ParentFont = False
        end
        object Memo54: TfrxMemoView
          Left = 895.748610000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_OMS_III'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_OMS_III"]')
          ParentFont = False
        end
        object Memo55: TfrxMemoView
          Left = 933.543910000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_OMS_IV'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_OMS_IV"]')
          ParentFont = False
        end
        object Memo56: TfrxMemoView
          Left = 971.339210000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_OMS_NULLO'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_OMS_NULLO"]')
          ParentFont = False
        end
        object Memo57: TfrxMemoView
          Left = 1009.134510000000000000
          Top = 1.889763780000000000
          Width = 37.795300000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CN_SEM_CD4_E_OMS'
          DataSet = frxDBCasosNovos
          DataSetName = 'frxDBCasosNovos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBCasosNovos."CN_SEM_CD4_E_OMS"]')
          ParentFont = False
        end
      end
      object Header1: TfrxHeader
        Height = 22.677180000000000000
        Top = 287.244280000000000000
        Width = 1046.929810000000000000
        object Shape2: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo6: TfrxMemoView
          Left = 3.779530000000000000
          Top = 2.779530000000000000
          Width = 1035.591220000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Casos Novos')
          ParentFont = False
        end
      end
    end
    object Page2: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle2: TfrxReportTitle
        Height = 207.874150000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture2: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo44: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo45: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'RELAT'#195#8220'RIO QUANTITATIVO')
          ParentFont = False
        end
        object Memo47: TfrxMemoView
          Left = 7.559060000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object MemoPeriodo2: TfrxMemoView
          Left = 9.559060000000000000
          Top = 181.417440000000000000
          Width = 260.787570000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Per'#195#173'odo')
          ParentFont = False
        end
        object Memo49: TfrxMemoView
          Left = 60.472480000000000000
          Top = 158.740260000000000000
          Width = 245.669450000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBDataset1."DS_UNIDADE"]')
          ParentFont = False
        end
      end
      object PageFooter2: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 480.000310000000000000
        Width = 1046.929810000000000000
        object Shape26: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo50: TfrxMemoView
          Left = 84.929190000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo52: TfrxMemoView
          Left = 11.338590000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo53: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo59: TfrxMemoView
          Left = 998.795920000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo60: TfrxMemoView
          Left = 943.882500000000000000
          Top = 1.779530000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
      object GroupHeader2: TfrxGroupHeader
        Height = 45.354360000000000000
        Top = 332.598640000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBTransferidos."FL_SEXO"'
        PrintChildIfInvisible = True
        object Shape27: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 45.354360000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo61: TfrxMemoView
          Left = 3.779530000000000000
          Top = 7.559060000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sexo')
          ParentFont = False
        end
        object Memo62: TfrxMemoView
          Left = 0.779530000000000000
          Top = 22.897650000000000000
          Width = 66.141732280000000000
          Height = 11.338590000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          DataField = 'FL_SEXO'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."FL_SEXO"]')
          ParentFont = False
        end
        object Shape28: TfrxShapeView
          Left = 68.031540000000000000
          Width = 68.031540000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo63: TfrxMemoView
          Left = 69.370130000000000000
          Top = 1.889763780000000000
          Width = 64.252010000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Faixa Et'#195#161'ria')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape36: TfrxShapeView
          Left = 136.393651970000000000
          Width = 90.708661417322800000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo71: TfrxMemoView
          Left = 136.063080000000000000
          Top = 3.669293780000000000
          Width = 90.708661420000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes com 1'#194#170' consulta')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape37: TfrxShapeView
          Left = 227.102371970000000000
          Width = 90.708671180000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo72: TfrxMemoView
          Left = 226.771800000000000000
          Top = 2.669293780000000000
          Width = 90.708671180000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes que iniciaram TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape38: TfrxShapeView
          Left = 408.519811970000000000
          Width = 90.708661417322800000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo73: TfrxMemoView
          Left = 408.189240000000000000
          Top = 3.669293780000000000
          Width = 90.708661420000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'em pr'#195#169
            'TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape39: TfrxShapeView
          Left = 499.228531970000000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo74: TfrxMemoView
          Left = 498.897960000000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '<=350')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape40: TfrxShapeView
          Left = 554.078740157480000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Shape41: TfrxShapeView
          Left = 608.881889763780000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo76: TfrxMemoView
          Left = 608.881889760000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '>500')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape42: TfrxShapeView
          Left = 663.685039370079000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo77: TfrxMemoView
          Left = 663.685039370000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape43: TfrxShapeView
          Left = 718.488188976378000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo78: TfrxMemoView
          Left = 718.488188980000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS I')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape44: TfrxShapeView
          Left = 773.291338582677000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo79: TfrxMemoView
          Left = 773.291338580000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS II')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape45: TfrxShapeView
          Left = 828.094488188976000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo80: TfrxMemoView
          Left = 828.094488190000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS III')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape46: TfrxShapeView
          Left = 882.897637795276000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo83: TfrxMemoView
          Left = 882.897637800000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS IV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape47: TfrxShapeView
          Left = 937.700787401575000000
          Width = 54.803149610000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo87: TfrxMemoView
          Left = 937.700787400000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape48: TfrxShapeView
          Left = 992.503937010000000000
          Width = 54.425196850393700000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo88: TfrxMemoView
          Left = 992.503937010000000000
          Top = 3.669293780000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4 e'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape29: TfrxShapeView
          Left = 317.811091970000000000
          Width = 90.708661420000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo64: TfrxMemoView
          Left = 317.480520000000000000
          Width = 90.708661420000000000
          Height = 44.976377950000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes que j'#195#161' estavam em TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo75: TfrxMemoView
          Left = 554.078740160000000000
          Top = 3.779530000000000000
          Width = 54.803149610000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '351-500')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData2: TfrxMasterData
        Height = 17.007874020000000000
        Top = 400.630180000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBTransferidos
        DataSetName = 'frxDBTransferidos'
        RowCount = 0
        object Memo89: TfrxMemoView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 17.007874020000000000
          ShowHint = False
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          GapX = 1.000000000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Color = 13750737
          Highlight.Condition = '<Line> mod 2 = 0'
        end
        object Memo90: TfrxMemoView
          Left = 70.031540000000000000
          Top = 1.889763780000000000
          Width = 64.252010000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'FAIXA'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."FAIXA"]')
          ParentFont = False
        end
        object Memo98: TfrxMemoView
          Left = 136.062992125984000000
          Top = 1.889763780000000000
          Width = 90.708661417322800000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."PACIENTES_COM_1_CONSULTA"]')
          ParentFont = False
        end
        object Memo99: TfrxMemoView
          Left = 226.771653543307000000
          Top = 1.889763780000000000
          Width = 90.708661417322800000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA_E_TARV'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."PACIENTES_COM_1_CONSULTA_E_TARV"]')
          ParentFont = False
        end
        object Memo100: TfrxMemoView
          Left = 408.188976377953000000
          Top = 1.889763780000000000
          Width = 90.708661417322800000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA_SEM_TA'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."PACIENTES_COM_1_CONSULTA_SEM_TA"]')
          ParentFont = False
        end
        object Memo101: TfrxMemoView
          Left = 498.897637795276000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_CD4_MENOR_350'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_CD4_MENOR_350"]')
          ParentFont = False
        end
        object Memo102: TfrxMemoView
          Left = 554.078740157480000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_CD4_351_500'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_CD4_351_500"]')
          ParentFont = False
        end
        object Memo103: TfrxMemoView
          Left = 608.881889763780000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_CD4_MAIOR_500'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_CD4_MAIOR_500"]')
          ParentFont = False
        end
        object Memo104: TfrxMemoView
          Left = 663.685039370079000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_CD4_NULLO'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_CD4_NULLO"]')
          ParentFont = False
        end
        object Memo105: TfrxMemoView
          Left = 718.488188976378000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_OMS_I'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_OMS_I"]')
          ParentFont = False
        end
        object Memo106: TfrxMemoView
          Left = 773.291338582677000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_OMS_II'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_OMS_II"]')
          ParentFont = False
        end
        object Memo107: TfrxMemoView
          Left = 828.094488188976000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_OMS_III'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_OMS_III"]')
          ParentFont = False
        end
        object Memo108: TfrxMemoView
          Left = 882.897637795276000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_OMS_IV'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_OMS_IV"]')
          ParentFont = False
        end
        object Memo109: TfrxMemoView
          Left = 937.700787401575000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_OMS_NULLO'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_OMS_NULLO"]')
          ParentFont = False
        end
        object Memo110: TfrxMemoView
          Left = 992.503937007874000000
          Top = 1.889763780000000000
          Width = 54.803149610000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSF_SEM_CD4_E_OMS'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."TRANSF_SEM_CD4_E_OMS"]')
          ParentFont = False
        end
        object Memo65: TfrxMemoView
          Left = 317.480314960000000000
          Top = 1.889763780000000000
          Width = 90.708661420000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_COM_1_CONSULTA_E_TAR1'
          DataSet = frxDBTransferidos
          DataSetName = 'frxDBTransferidos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBTransferidos."PACIENTES_COM_1_CONSULTA_E_TAR1"]')
          ParentFont = False
        end
      end
      object Header2: TfrxHeader
        Height = 22.677180000000000000
        Top = 287.244280000000000000
        Width = 1046.929810000000000000
        object Shape50: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo111: TfrxMemoView
          Left = 3.779530000000000000
          Top = 2.779530000000000000
          Width = 1035.591220000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Casos Transferidos')
          ParentFont = False
        end
      end
    end
    object Page3: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle3: TfrxReportTitle
        Height = 207.874150000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture3: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo48: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo66: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'RELAT'#195#8220'RIO QUANTITATIVO')
          ParentFont = False
        end
        object Memo67: TfrxMemoView
          Left = 7.559060000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object MemoPeriodo3: TfrxMemoView
          Left = 9.559060000000000000
          Top = 181.417440000000000000
          Width = 260.787570000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Per'#195#173'odo')
          ParentFont = False
        end
        object Memo69: TfrxMemoView
          Left = 60.472480000000000000
          Top = 158.740260000000000000
          Width = 245.669450000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBDataset1."DS_UNIDADE"]')
          ParentFont = False
        end
      end
      object PageFooter3: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 480.000310000000000000
        Width = 1046.929810000000000000
        object Shape30: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo70: TfrxMemoView
          Left = 84.929190000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo91: TfrxMemoView
          Left = 11.338590000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo92: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo93: TfrxMemoView
          Left = 998.795920000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo94: TfrxMemoView
          Left = 943.882500000000000000
          Top = 1.779530000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
      object GroupHeader3: TfrxGroupHeader
        Height = 45.354360000000000000
        Top = 332.598640000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBAcompanhados."FL_SEXO"'
        PrintChildIfInvisible = True
        object Shape31: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 45.354360000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo95: TfrxMemoView
          Left = 7.559060000000000000
          Top = 7.559060000000000000
          Width = 83.149660000000000000
          Height = 15.118120000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sexo')
          ParentFont = False
        end
        object Memo96: TfrxMemoView
          Left = 4.559060000000000000
          Top = 22.897650000000000000
          Width = 92.598442280000000000
          Height = 11.338590000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          DataField = 'FL_SEXO'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."FL_SEXO"]')
          ParentFont = False
        end
        object Shape32: TfrxShapeView
          Left = 102.047310000000000000
          Width = 90.708720000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo97: TfrxMemoView
          Left = 103.385900000000000000
          Top = 1.889763780000000000
          Width = 86.929190000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Faixa Et'#195#161'ria')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape34: TfrxShapeView
          Left = 192.881901970000000000
          Width = 105.826840000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo113: TfrxMemoView
          Left = 192.551330000000000000
          Top = 2.669293780000000000
          Width = 105.826840000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes que'
            'iniciaram TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape35: TfrxShapeView
          Left = 412.094641970000000000
          Width = 94.488250000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo114: TfrxMemoView
          Left = 411.764070000000000000
          Top = 3.669293780000000000
          Width = 94.488250000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes em'
            'pr'#195#169' TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape51: TfrxShapeView
          Left = 506.787591970000000000
          Width = 64.252010000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo115: TfrxMemoView
          Left = 502.677490000000000000
          Top = 3.669293780000000000
          Width = 71.811070000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '<=350')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape52: TfrxShapeView
          Left = 571.039601970000000000
          Width = 68.031540000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Shape53: TfrxShapeView
          Left = 639.071141970000000000
          Width = 49.133890000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo117: TfrxMemoView
          Left = 638.740570000000000000
          Top = 3.669293780000000000
          Width = 49.133890000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4 > 500')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape54: TfrxShapeView
          Left = 688.205031970000000000
          Width = 45.354360000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo118: TfrxMemoView
          Left = 687.874460000000000000
          Top = 3.669293780000000000
          Width = 45.354360000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape55: TfrxShapeView
          Left = 733.559391970000000000
          Width = 45.354360000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo119: TfrxMemoView
          Left = 733.228820000000000000
          Top = 3.669293780000000000
          Width = 45.354360000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS I')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape56: TfrxShapeView
          Left = 778.913751970000000000
          Width = 49.133890000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo120: TfrxMemoView
          Left = 778.583180000000000000
          Top = 3.669293780000000000
          Width = 49.133890000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS II')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape57: TfrxShapeView
          Left = 828.047641970000000000
          Width = 52.913420000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo121: TfrxMemoView
          Left = 827.717070000000000000
          Top = 3.669293780000000000
          Width = 52.913420000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS III')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape58: TfrxShapeView
          Left = 880.961061970000000000
          Width = 52.913420000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo122: TfrxMemoView
          Left = 880.630490000000000000
          Top = 3.669293780000000000
          Width = 52.913420000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS IV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo123: TfrxMemoView
          Left = 933.543910000000000000
          Top = 3.669293780000000000
          Width = 56.692950000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape60: TfrxShapeView
          Left = 990.567431970000000000
          Width = 56.314972830000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo124: TfrxMemoView
          Left = 986.457330000000000000
          Top = 3.669293780000000000
          Width = 68.031540000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4 e'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape61: TfrxShapeView
          Left = 298.708741970000000000
          Width = 113.385900000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo125: TfrxMemoView
          Left = 298.378170000000000000
          Width = 113.385900000000000000
          Height = 44.976377950000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes que j'#195#161
            'estavam em TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo116: TfrxMemoView
          Left = 574.268090000000000000
          Top = 1.669293780000000000
          Width = 60.472480000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '351-500')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData3: TfrxMasterData
        Height = 17.007874020000000000
        Top = 400.630180000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBAcompanhados
        DataSetName = 'frxDBAcompanhados'
        RowCount = 0
        object Memo126: TfrxMemoView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 17.007874020000000000
          ShowHint = False
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          GapX = 1.000000000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Color = 13750737
          Highlight.Condition = '<Line> mod 2 = 0'
        end
        object Memo127: TfrxMemoView
          Left = 103.559055120000000000
          Top = 1.889763780000000000
          Width = 86.929133860000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'FAIXA'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."FAIXA"]')
          ParentFont = False
        end
        object Memo128: TfrxMemoView
          Left = 192.377952760000000000
          Top = 1.889763780000000000
          Width = 105.826840000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_INICIARAM_TARV'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."PACIENTES_INICIARAM_TARV"]')
          ParentFont = False
        end
        object Memo129: TfrxMemoView
          Left = 298.204724410000000000
          Top = 1.889763780000000000
          Width = 113.385826770000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_TARV_ANTERIOR'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."PACIENTES_TARV_ANTERIOR"]')
          ParentFont = False
        end
        object Memo130: TfrxMemoView
          Left = 502.677165350000000000
          Top = 1.889763780000000000
          Width = 71.811023620000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_CD4_MENOR_350'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_CD4_MENOR_350"]')
          ParentFont = False
        end
        object Memo131: TfrxMemoView
          Left = 574.110236220000000000
          Top = 1.889763780000000000
          Width = 60.472440940000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_CD4_351_500'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_CD4_351_500"]')
          ParentFont = False
        end
        object Memo132: TfrxMemoView
          Left = 638.740157480000000000
          Top = 1.889763780000000000
          Width = 49.133858270000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_CD4_MAIOR_500'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_CD4_MAIOR_500"]')
          ParentFont = False
        end
        object Memo133: TfrxMemoView
          Left = 687.874015750000000000
          Top = 1.889763780000000000
          Width = 45.354330710000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_CD4_NULLO'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_CD4_NULLO"]')
          ParentFont = False
        end
        object Memo134: TfrxMemoView
          Left = 733.228346460000000000
          Top = 1.889763780000000000
          Width = 45.354330710000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_OMS_I'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_OMS_I"]')
          ParentFont = False
        end
        object Memo135: TfrxMemoView
          Left = 778.582677170000000000
          Top = 1.889763780000000000
          Width = 49.133858270000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_OMS_II'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_OMS_II"]')
          ParentFont = False
        end
        object Memo136: TfrxMemoView
          Left = 827.716535430000000000
          Top = 1.889763780000000000
          Width = 52.913385830000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_OMS_III'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_OMS_III"]')
          ParentFont = False
        end
        object Memo137: TfrxMemoView
          Left = 880.629921260000000000
          Top = 1.889763780000000000
          Width = 52.913385830000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_OMS_IV'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_OMS_IV"]')
          ParentFont = False
        end
        object Memo138: TfrxMemoView
          Left = 933.543307090000000000
          Top = 1.889763780000000000
          Width = 56.692913390000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_OMS_NULLO'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_OMS_NULLO"]')
          ParentFont = False
        end
        object Memo139: TfrxMemoView
          Left = 986.456692910000000000
          Top = 1.889763780000000000
          Width = 68.031496060000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ACOMPANHADOS_SEM_CD4_E_OMS'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."ACOMPANHADOS_SEM_CD4_E_OMS"]')
          ParentFont = False
        end
        object Memo141: TfrxMemoView
          Left = 411.590551180000000000
          Top = 1.889763780000000000
          Width = 94.488188980000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_PRE_TARV'
          DataSet = frxDBAcompanhados
          DataSetName = 'frxDBAcompanhados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcompanhados."PACIENTES_PRE_TARV"]')
          ParentFont = False
        end
      end
      object Header3: TfrxHeader
        Height = 22.677180000000000000
        Top = 287.244280000000000000
        Width = 1046.929810000000000000
        object Shape63: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo142: TfrxMemoView
          Left = 3.779530000000000000
          Top = 2.779530000000000000
          Width = 1035.591220000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes J'#195#161' Acompanhados')
          ParentFont = False
        end
      end
    end
    object Page4: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 256
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle4: TfrxReportTitle
        Height = 207.874150000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture4: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo68: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo112: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'RELAT'#195#8220'RIO QUANTITATIVO')
          ParentFont = False
        end
        object Memo140: TfrxMemoView
          Left = 7.559060000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object MemoPeriodo4: TfrxMemoView
          Left = 9.559060000000000000
          Top = 181.417440000000000000
          Width = 260.787570000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Per'#195#173'odo')
          ParentFont = False
        end
        object Memo144: TfrxMemoView
          Left = 60.472480000000000000
          Top = 158.740260000000000000
          Width = 245.669450000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBDataset1."DS_UNIDADE"]')
          ParentFont = False
        end
      end
      object GroupHeader4: TfrxGroupHeader
        Height = 45.354360000000000000
        Top = 332.598640000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBConsolidados."FL_SEXO"'
        PrintChildIfInvisible = True
        object Shape6: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 45.354360000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo145: TfrxMemoView
          Left = 3.779530000000000000
          Top = 7.559060000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sexo')
          ParentFont = False
        end
        object Memo146: TfrxMemoView
          Left = 0.779530000000000000
          Top = 22.897650000000000000
          Width = 60.472443390000000000
          Height = 15.118120000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          DataField = 'FL_SEXO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -8
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."FL_SEXO"]')
          ParentFont = False
        end
        object Shape33: TfrxShapeView
          Left = 64.252010000000000000
          Width = 71.811070000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo147: TfrxMemoView
          Left = 65.590600000000000000
          Top = 1.889763780000000000
          Width = 71.811070000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Faixa Et'#195#161'ria')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape49: TfrxShapeView
          Left = 242.015791970000000000
          Width = 56.692950000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo148: TfrxMemoView
          Left = 245.464750000000000000
          Top = 1.669293780000000000
          Width = 52.913420000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -8
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'que'
            'iniciaram '
            'TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape62: TfrxShapeView
          Left = 351.622161970000000000
          Width = 45.354360000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo149: TfrxMemoView
          Left = 351.291590000000000000
          Top = 3.669293780000000000
          Width = 45.354360000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -8
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'em pr'#195#169' '
            'TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape64: TfrxShapeView
          Left = 397.181221970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo150: TfrxMemoView
          Left = 393.071120000000000000
          Top = 3.669293780000000000
          Width = 45.354360000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '<=350')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape65: TfrxShapeView
          Left = 434.976521970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Shape66: TfrxShapeView
          Left = 472.771821970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo151: TfrxMemoView
          Left = 472.441250000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4 > 500')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape67: TfrxShapeView
          Left = 506.787591970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo152: TfrxMemoView
          Left = 506.457020000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'CD4')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape68: TfrxShapeView
          Left = 540.803361970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo153: TfrxMemoView
          Left = 540.472790000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS I')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape69: TfrxShapeView
          Left = 574.819131970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo154: TfrxMemoView
          Left = 574.488560000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS II')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape70: TfrxShapeView
          Left = 608.834901970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo155: TfrxMemoView
          Left = 608.504330000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS III')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape71: TfrxShapeView
          Left = 642.850671970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo156: TfrxMemoView
          Left = 642.520100000000000000
          Top = 3.669293780000000000
          Width = 34.015770000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'OMS IV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape74: TfrxShapeView
          Left = 298.708741970000000000
          Width = 52.913420000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo159: TfrxMemoView
          Left = 298.378170000000000000
          Width = 52.913420000000000000
          Height = 44.976377950000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -8
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes '
            'que j'#195#161
            'estavam'
            'em TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo160: TfrxMemoView
          Left = 438.205010000000000000
          Top = 1.669293780000000000
          Width = 30.236240000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CD4'
            '351-500')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape76: TfrxShapeView
          Left = 136.393651970000000000
          Width = 56.692950000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo177: TfrxMemoView
          Left = 136.063080000000000000
          Top = 2.669293780000000000
          Width = 56.692950000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'Atendidos')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape77: TfrxShapeView
          Left = 193.086601970000000000
          Width = 49.133890000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo178: TfrxMemoView
          Left = 192.756030000000000000
          Top = 2.669293780000000000
          Width = 49.133890000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '1'#194#170' '
            'Consulta')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape78: TfrxShapeView
          Left = 676.866441970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo179: TfrxMemoView
          Left = 676.535870000000000000
          Top = 3.669293780000000000
          Width = 30.236240000000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'OMS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape79: TfrxShapeView
          Left = 707.102681970000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo180: TfrxMemoView
          Left = 706.772110000000000000
          Width = 34.015770000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -8
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sem'
            'OMS e'
            'Sem'
            'CD4')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape80: TfrxShapeView
          Left = 741.118451970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo181: TfrxMemoView
          Left = 740.787880000000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CV'
            '< 1000')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape81: TfrxShapeView
          Left = 771.354691970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo182: TfrxMemoView
          Left = 771.024120000000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CV'
            '>='
            '1000')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape82: TfrxShapeView
          Left = 801.590931970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo183: TfrxMemoView
          Left = 801.260360000000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'CV'
            '"<'
            'LDL"')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape83: TfrxShapeView
          Left = 831.827171970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo184: TfrxMemoView
          Left = 831.496600000000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'DPI'
            'DET')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape84: TfrxShapeView
          Left = 862.063411970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo185: TfrxMemoView
          Left = 861.732840000000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'DPI'
            'IND')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape85: TfrxShapeView
          Left = 892.299651970000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo186: TfrxMemoView
          Left = 891.969080000000000000
          Width = 30.236240000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'SEM'
            'CV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape86: TfrxShapeView
          Left = 922.535891970000000000
          Width = 45.354360000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo187: TfrxMemoView
          Left = 922.205320000000000000
          Width = 45.354360000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Abandonos')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape87: TfrxShapeView
          Left = 967.890251970000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo188: TfrxMemoView
          Left = 967.559680000000000000
          Width = 37.795300000000000000
          Height = 45.354360000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            #195#8220'bitos')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo189: TfrxMemoView
          Left = 1005.354980000000000000
          Top = 9.338590000000000000
          Width = 41.574830000000000000
          Height = 26.456710000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Transfe'
            'ridos')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData4: TfrxMasterData
        Height = 17.007874020000000000
        Top = 400.630180000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBConsolidados
        DataSetName = 'frxDBConsolidados'
        RowCount = 0
        object Memo161: TfrxMemoView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 17.007874020000000000
          ShowHint = False
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          GapX = 1.000000000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Color = 13750737
          Highlight.Condition = '<Line> mod 2 = 0'
        end
        object Memo162: TfrxMemoView
          Left = 65.763779527559100000
          Top = 1.889763780000000000
          Width = 71.811023620000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'FAIXA'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."FAIXA"]')
          ParentFont = False
        end
        object Memo163: TfrxMemoView
          Left = 245.291338582677000000
          Top = 1.889763780000000000
          Width = 52.913385830000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_INICIARAM_TARV'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."PACIENTES_INICIARAM_TARV"]')
          ParentFont = False
        end
        object Memo164: TfrxMemoView
          Left = 298.204724409449000000
          Top = 1.889763780000000000
          Width = 52.913385830000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_TARV_ANTERIOR'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."PACIENTES_TARV_ANTERIOR"]')
          ParentFont = False
        end
        object Memo165: TfrxMemoView
          Left = 393.070866141732000000
          Top = 1.889763780000000000
          Width = 45.354330710000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_CD4_MENOR_350'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_CD4_MENOR_350"]')
          ParentFont = False
        end
        object Memo166: TfrxMemoView
          Left = 438.047244090000000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_CD4_351_500'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_CD4_351_500"]')
          ParentFont = False
        end
        object Memo167: TfrxMemoView
          Left = 472.440944880000000000
          Top = 1.889763780000000000
          Width = 34.015748030000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_CD4_MAIOR_500'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_CD4_MAIOR_500"]')
          ParentFont = False
        end
        object Memo168: TfrxMemoView
          Left = 506.456692913386000000
          Top = 1.889763780000000000
          Width = 34.015748031496100000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_CD4_NULLO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_CD4_NULLO"]')
          ParentFont = False
        end
        object Memo169: TfrxMemoView
          Left = 540.472440944882000000
          Top = 1.889763780000000000
          Width = 34.015748031496100000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_OMS_I'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_OMS_I"]')
          ParentFont = False
        end
        object Memo170: TfrxMemoView
          Left = 574.488188980000000000
          Top = 1.889763780000000000
          Width = 34.015748031496100000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_OMS_II'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_OMS_II"]')
          ParentFont = False
        end
        object Memo171: TfrxMemoView
          Left = 608.881889760000000000
          Top = 1.889763780000000000
          Width = 34.015748030000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_OMS_III'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_OMS_III"]')
          ParentFont = False
        end
        object Memo172: TfrxMemoView
          Left = 642.519685040000000000
          Top = 1.889763780000000000
          Width = 34.015748030000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_OMS_IV'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_OMS_IV"]')
          ParentFont = False
        end
        object Memo173: TfrxMemoView
          Left = 676.535433070866000000
          Top = 1.889763780000000000
          Width = 30.236220472440900000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_OMS_NULLO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_OMS_NULLO"]')
          ParentFont = False
        end
        object Memo174: TfrxMemoView
          Left = 706.771653543307000000
          Top = 1.889763780000000000
          Width = 34.015748031496100000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CONSOLIDADOS_SEM_CD4_E_OMS'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CONSOLIDADOS_SEM_CD4_E_OMS"]')
          ParentFont = False
        end
        object Memo175: TfrxMemoView
          Left = 351.118110236220000000
          Top = 1.889763780000000000
          Width = 45.354330708661400000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PACIENTES_PRE_TARV'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."PACIENTES_PRE_TARV"]')
          ParentFont = False
        end
        object Memo195: TfrxMemoView
          Left = 136.062992125984000000
          Top = 1.889763780000000000
          Width = 56.692913390000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TOTAL_ATENDIMENTOS'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."TOTAL_ATENDIMENTOS"]')
          ParentFont = False
        end
        object Memo196: TfrxMemoView
          Left = 192.755905511811000000
          Top = 1.889763780000000000
          Width = 49.133858270000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TOTAL_1_CONSULTA'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."TOTAL_1_CONSULTA"]')
          ParentFont = False
        end
        object Memo197: TfrxMemoView
          Left = 740.787401574803000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CV_MENOR_1000'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CV_MENOR_1000"]')
          ParentFont = False
        end
        object Memo198: TfrxMemoView
          Left = 771.023622047244000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CV_MAIOR_IGUAL_1000'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CV_MAIOR_IGUAL_1000"]')
          ParentFont = False
        end
        object Memo199: TfrxMemoView
          Left = 801.259842519685000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CV_IGUAL_LDL'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CV_IGUAL_LDL"]')
          ParentFont = False
        end
        object Memo200: TfrxMemoView
          Left = 831.496062992126000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CV_DETERMINADO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CV_DETERMINADO"]')
          ParentFont = False
        end
        object Memo201: TfrxMemoView
          Left = 861.732283464567000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CV_INDETERMINADO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CV_INDETERMINADO"]')
          ParentFont = False
        end
        object Memo202: TfrxMemoView
          Left = 891.968503940000000000
          Top = 1.889763780000000000
          Width = 30.236220470000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'CV_NULLO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."CV_NULLO"]')
          ParentFont = False
        end
        object Memo203: TfrxMemoView
          Left = 922.204724409449000000
          Top = 1.889763780000000000
          Width = 45.354330710000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'ABANDONO'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."ABANDONO"]')
          ParentFont = False
        end
        object Memo204: TfrxMemoView
          Left = 967.559055118110000000
          Top = 1.889763780000000000
          Width = 37.795275590551200000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'OBITOS'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."OBITOS"]')
          ParentFont = False
        end
        object Memo205: TfrxMemoView
          Left = 1005.354330708660000000
          Top = 1.889763780000000000
          Width = 41.574803149606300000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TRANSFERIDOS'
          DataSet = frxDBConsolidados
          DataSetName = 'frxDBConsolidados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBConsolidados."TRANSFERIDOS"]')
          ParentFont = False
        end
      end
      object Header4: TfrxHeader
        Height = 22.677180000000000000
        Top = 287.244280000000000000
        Width = 1046.929810000000000000
        object Shape75: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo176: TfrxMemoView
          Left = 3.779530000000000000
          Top = 2.779530000000000000
          Width = 1035.591220000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Consolidados No Per'#195#173'odo')
          ParentFont = False
        end
      end
      object PageFooter4: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 480.000310000000000000
        Width = 1046.929810000000000000
        object Shape89: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo190: TfrxMemoView
          Left = 84.929190000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo191: TfrxMemoView
          Left = 11.338590000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo192: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo193: TfrxMemoView
          Left = 998.795920000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo194: TfrxMemoView
          Left = 943.882500000000000000
          Top = 1.779530000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
    end
    object Page5: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle5: TfrxReportTitle
        Height = 207.874150000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture5: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo143: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo157: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'RELAT'#195#8220'RIO QUANTITATIVO')
          ParentFont = False
        end
        object Memo158: TfrxMemoView
          Left = 7.559060000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object MemoPeriodo5: TfrxMemoView
          Left = 9.559060000000000000
          Top = 181.417440000000000000
          Width = 260.787570000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Per'#195#173'odo')
          ParentFont = False
        end
        object Memo207: TfrxMemoView
          Left = 60.472480000000000000
          Top = 158.740260000000000000
          Width = 245.669450000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[frxDBDataset1."DS_UNIDADE"]')
          ParentFont = False
        end
      end
      object Header5: TfrxHeader
        Height = 22.677180000000000000
        Top = 287.244280000000000000
        Width = 1046.929810000000000000
        object Shape112: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo260: TfrxMemoView
          Left = 3.779530000000000000
          Top = 2.779530000000000000
          Width = 1035.591220000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Acumulados at'#195#169' o Per'#195#173'odo')
          ParentFont = False
        end
      end
      object PageFooter5: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 480.000310000000000000
        Width = 1046.929810000000000000
        object Shape113: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo261: TfrxMemoView
          Left = 84.929190000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo262: TfrxMemoView
          Left = 11.338590000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo263: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo264: TfrxMemoView
          Left = 998.795920000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo265: TfrxMemoView
          Left = 943.882500000000000000
          Top = 1.779530000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
      object GroupHeader5: TfrxGroupHeader
        Height = 45.354360000000000000
        Top = 332.598640000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBAcumulados."FL_SEXO"'
        PrintChildIfInvisible = True
        object Shape72: TfrxShapeView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 45.354360000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo206: TfrxMemoView
          Left = 7.559060000000000000
          Top = 7.559060000000000000
          Width = 83.149660000000000000
          Height = 15.118120000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Sexo')
          ParentFont = False
        end
        object Memo208: TfrxMemoView
          Left = 4.559060000000000000
          Top = 22.897650000000000000
          Width = 92.598442280000000000
          Height = 11.338590000000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          DataField = 'FL_SEXO'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."FL_SEXO"]')
          ParentFont = False
        end
        object Shape73: TfrxShapeView
          Left = 102.047310000000000000
          Width = 90.708720000000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo209: TfrxMemoView
          Left = 103.385900000000000000
          Top = 1.889763780000000000
          Width = 86.929190000000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Faixa Et'#195#161'ria')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape88: TfrxShapeView
          Left = 192.881901970000000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo210: TfrxMemoView
          Left = 192.551330000000000000
          Top = 2.669293780000000000
          Width = 106.582677170000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Total Pacientes Identificados')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape90: TfrxShapeView
          Left = 406.315111970000000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo211: TfrxMemoView
          Left = 405.984540000000000000
          Top = 3.669293780000000000
          Width = 106.582677170000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes ativos')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape91: TfrxShapeView
          Left = 512.503937007874000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo212: TfrxMemoView
          Left = 512.503937010000000000
          Top = 3.669293780000000000
          Width = 106.582677170000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Abandonos')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape92: TfrxShapeView
          Left = 619.086614173228000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Shape93: TfrxShapeView
          Left = 725.669291338583000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo213: TfrxMemoView
          Left = 725.669291340000000000
          Top = 3.669293780000000000
          Width = 106.582677170000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Transferidos para outra unidade')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape94: TfrxShapeView
          Left = 832.251968503937000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo214: TfrxMemoView
          Left = 832.251968500000000000
          Top = 3.669293780000000000
          Width = 106.582677170000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'em TARV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape101: TfrxShapeView
          Left = 299.338582677165000000
          Width = 106.582677170000000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo221: TfrxMemoView
          Left = 299.338582680000000000
          Width = 106.582677170000000000
          Height = 44.976377950000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes com processos aberto')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo222: TfrxMemoView
          Left = 619.086614170000000000
          Top = 1.669293780000000000
          Width = 106.582677170000000000
          Height = 41.574803150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            #195#8220'bitos')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape95: TfrxShapeView
          Left = 938.834645670000000000
          Width = 108.094488188976000000
          Height = 45.354360000000000000
          ShowHint = False
        end
        object Memo215: TfrxMemoView
          Left = 938.834645670000000000
          Top = 3.669293780000000000
          Width = 106.582677170000000000
          Height = 37.795273150000000000
          ShowHint = False
          CharSpacing = 0.500000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            'Pacientes'
            'pr'#195#169' TARV')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData5: TfrxMasterData
        Height = 17.007874020000000000
        Top = 400.630180000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBAcumulados
        DataSetName = 'frxDBAcumulados'
        RowCount = 0
        object Memo223: TfrxMemoView
          Align = baClient
          Width = 1046.929810000000000000
          Height = 17.007874020000000000
          ShowHint = False
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          GapX = 1.000000000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Color = 13750737
          Highlight.Condition = '<Line> mod 2 = 0'
        end
        object Memo224: TfrxMemoView
          Left = 103.559055120000000000
          Top = 1.889763780000000000
          Width = 86.929133860000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'FAIXA'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."FAIXA"]')
          ParentFont = False
        end
        object Memo225: TfrxMemoView
          Left = 192.377952760000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'TOTAL_IDENTIFICADOS'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."TOTAL_IDENTIFICADOS"]')
          ParentFont = False
        end
        object Memo226: TfrxMemoView
          Left = 299.338582680000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_COM_PROCESSO'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_COM_PROCESSO"]')
          ParentFont = False
        end
        object Memo227: TfrxMemoView
          Left = 512.503937010000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_ABANDONO'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_ABANDONO"]')
          ParentFont = False
        end
        object Memo228: TfrxMemoView
          Left = 619.086614170000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_OBITO'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_OBITO"]')
          ParentFont = False
        end
        object Memo230: TfrxMemoView
          Left = 725.669291340000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_TRANSFERIDOS'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_TRANSFERIDOS"]')
          ParentFont = False
        end
        object Memo232: TfrxMemoView
          Left = 832.251968500000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_COM_TARV'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_COM_TARV"]')
          ParentFont = False
        end
        object Memo234: TfrxMemoView
          Left = 938.834645670000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_PRE_TARV'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_PRE_TARV"]')
          ParentFont = False
        end
        object Memo237: TfrxMemoView
          Left = 405.921259840000000000
          Top = 1.889763780000000000
          Width = 106.582677170000000000
          Height = 11.338590000000000000
          ShowHint = False
          DataField = 'PCTS_ATIVOS'
          DataSet = frxDBAcumulados
          DataSetName = 'frxDBAcumulados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Verdana'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[frxDBAcumulados."PCTS_ATIVOS"]')
          ParentFont = False
        end
      end
    end
  end
  inherited frxDBDataset1: TfrxDBDataset
    FieldAliases.Strings = (
      'DS_UNIDADE=DS_UNIDADE')
    DataSource = dsUnidade
    Left = 104
    Top = 360
  end
  inherited frxXLSExport1: TfrxXLSExport
    Left = 192
    Top = 360
  end
  inherited frxPDFExport1: TfrxPDFExport
    Left = 136
    Top = 360
  end
  inherited frxHTMLExport1: TfrxHTMLExport
    Left = 168
    Top = 360
  end
  inherited frxRTFExport1: TfrxRTFExport
    Left = 224
    Top = 360
  end
  inherited frxJPEGExport1: TfrxJPEGExport
    Left = 248
    Top = 360
  end
  object dsRelatorio: TDataSource
    DataSet = qyGeral
    Left = 40
    Top = 360
  end
  object frxDBCasosNovos: TfrxDBDataset
    UserName = 'frxDBCasosNovos'
    CloseDataSource = False
    FieldAliases.Strings = (
      'FAIXA=FAIXA'
      'FL_SEXO=FL_SEXO'
      'TESTAGEM_ACONSELHADOS=TESTAGEM_ACONSELHADOS'
      'TESTAGEM_TESTADOS=TESTAGEM_TESTADOS'
      'TESTAGEM_NEGATIVO=TESTAGEM_NEGATIVO'
      'TESTAGEM_POSITIVO=TESTAGEM_POSITIVO'
      'TESTAGEM_INDETERMINADO=TESTAGEM_INDETERMINADO'
      'TESTAGEM_TOTAL=TESTAGEM_TOTAL'
      'PACIENTES_COM_IDENTIFICACAO=PACIENTES_COM_IDENTIFICACAO'
      'PACIENTES_COM_1_CONSULTA=PACIENTES_COM_1_CONSULTA'
      'PACIENTES_COM_1_CONSULTA_E_TARV=PACIENTES_COM_1_CONSULTA_E_TARV'
      'PACIENTES_COM_1_CONSULTA_SEM_TA=PACIENTES_COM_1_CONSULTA_SEM_TA'
      'CN_CD4_MENOR_350=CN_CD4_MENOR_350'
      'CN_CD4_351_500=CN_CD4_351_500'
      'CN_CD4_MAIOR_500=CN_CD4_MAIOR_500'
      'CN_CD4_NULLO=CN_CD4_NULLO'
      'CN_OMS_I=CN_OMS_I'
      'CN_OMS_II=CN_OMS_II'
      'CN_OMS_III=CN_OMS_III'
      'CN_OMS_IV=CN_OMS_IV'
      'CN_OMS_NULLO=CN_OMS_NULLO'
      'CN_SEM_CD4_E_OMS=CN_SEM_CD4_E_OMS')
    DataSource = dsRelatorio
    BCDToCurrency = False
    Left = 105
    Top = 326
  end
  object qyUnidade: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select DS_UNIDADE FROM unidade WHERE CD_UNIDADE = :prmUnidade')
    Left = 10
    Top = 325
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'prmUnidade'
        ParamType = ptUnknown
      end>
    object qyUnidadeDS_UNIDADE: TIBStringField
      FieldName = 'DS_UNIDADE'
      Origin = 'UNIDADE.DS_UNIDADE'
      Required = True
      Size = 50
    end
  end
  object dsUnidade: TDataSource
    DataSet = qyUnidade
    Left = 42
    Top = 325
  end
  object qyTransferencia: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT FAIXA,'
      
        #9'CASE FL_SEXO WHEN '#39'M'#39' THEN '#39'MASCULINO'#39' WHEN '#39'F'#39' THEN '#39'FEMININO'#39 +
        ' END FL_SEXO,'
      #9'SUM(PACIENTES_COM_1_CONSULTA) PACIENTES_COM_1_CONSULTA,'
      
        #9'SUM(PACIENTES_COM_1_CONSULTA_E_TARV) PACIENTES_COM_1_CONSULTA_E' +
        '_TARV,'
      
        #9'SUM(PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR) PACIENTES_COM_1_C' +
        'ONSULTA_E_TARV_ANTERIOR,'
      
        #9'SUM(PACIENTES_COM_1_CONSULTA_SEM_TARV) PACIENTES_COM_1_CONSULTA' +
        '_SEM_TARV,'
      #9'SUM(TRANSF_CD4_MENOR_350) TRANSF_CD4_MENOR_350,'
      #9'SUM(TRANSF_CD4_351_500) TRANSF_CD4_351_500,'
      #9'SUM(TRANSF_CD4_MAIOR_500) TRANSF_CD4_MAIOR_500,'
      #9'SUM(TRANSF_CD4_NULLO) TRANSF_CD4_NULLO,'
      #9'SUM(TRANSF_OMS_I) TRANSF_OMS_I,'
      #9'SUM(TRANSF_OMS_II) TRANSF_OMS_II,'
      #9'SUM(TRANSF_OMS_III) TRANSF_OMS_III,'
      #9'SUM(TRANSF_OMS_IV) TRANSF_OMS_IV,'
      #9'SUM(TRANSF_OMS_NULLO) TRANSF_OMS_NULLO,'
      #9'SUM(TRANSF_SEM_CD4_E_OMS) TRANSF_SEM_CD4_E_OMS'
      'FROM'
      '    (select'
      #9#9'PACIENTES_COM_1_CONSULTA,'
      #9#9'PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9'PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9'PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9'TRANSF_CD4_MENOR_350,'
      #9#9'TRANSF_CD4_351_500,'
      #9#9'TRANSF_CD4_MAIOR_500,'
      #9#9'TRANSF_CD4_NULLO,'
      #9#9'TRANSF_OMS_I,'
      #9#9'TRANSF_OMS_II,'
      #9#9'TRANSF_OMS_III,'
      #9#9'TRANSF_OMS_IV,'
      #9#9'TRANSF_OMS_NULLO,'
      #9#9'TRANSF_SEM_CD4_E_OMS,'
      '        FL_SEXO,'
      '        CASE'
      '        WHEN NR_IDADE < 1 THEN '#39'<1 Ano'#39
      '        WHEN NR_IDADE >=1 AND NR_IDADE <=3 THEN '#39'1-3 Anos'#39
      '        WHEN NR_IDADE >=4 AND NR_IDADE <=5 THEN '#39'4-5 Anos'#39
      '        WHEN NR_IDADE >=6 AND NR_IDADE <=9 THEN '#39'6-9 Anos'#39
      '        WHEN NR_IDADE >=10 AND NR_IDADE <=14 THEN '#39'10-14 Anos'#39
      '        WHEN NR_IDADE >=15 AND NR_IDADE <=19 THEN '#39'15-19 Anos'#39
      '        WHEN NR_IDADE >=20 AND NR_IDADE <=24 THEN '#39'20-24 Anos'#39
      '        WHEN NR_IDADE >=25 AND NR_IDADE <=49 THEN '#39'25-49 Anos'#39
      '        WHEN NR_IDADE >=50 THEN '#39'>50 Anos'#39
      '        else '#39'N'#227'o Consta'#39
      '        END AS FAIXA'
      '    from'
      '        (SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND TP_ENTRADA = 2)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      
        #9#9#9#9'AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIMEIRO_TARV <=' +
        ' :DATA_FIM'
      #9#9#9#9'AND DT_PRIMEIRO_TARV >= DT_PRIMEIRA_CONSULTA'
      #9#9#9#9'AND TP_ENTRADA = 2)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      
        #9#9#9#9'AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIMEIRO_TARV <=' +
        ' :DATA_FIM'
      #9#9#9#9'AND DT_PRIMEIRO_TARV < DT_PRIMEIRA_CONSULTA'
      #9#9#9#9'AND TP_ENTRADA = 2)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND DT_PRIMEIRO_TARV IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 2)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC <= 350'
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC > 350 AND VL_CD4_PC <= 500'
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC > 500'
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_CD4_PC IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'I'#39
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'II'#39
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'III'#39
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'IV'#39
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_OMS_NULLO,'
      #9#9#9'0 TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_E_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_COM_1_CONSULTA_SEM_TARV,'
      #9#9#9'0 TRANSF_CD4_MENOR_350,'
      #9#9#9'0 TRANSF_CD4_351_500,'
      #9#9#9'0 TRANSF_CD4_MAIOR_500,'
      #9#9#9'0 TRANSF_CD4_NULLO,'
      #9#9#9'0 TRANSF_OMS_I,'
      #9#9#9'0 TRANSF_OMS_II,'
      #9#9#9'0 TRANSF_OMS_III,'
      #9#9#9'0 TRANSF_OMS_IV,'
      #9#9#9'0 TRANSF_OMS_NULLO,'
      #9#9#9'COUNT(NR_IDADE) TRANSF_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      
        #9#9#9#9'DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIMEIRA_CONSULT' +
        'A <= :DATA_FIM'
      #9#9#9#9'AND VL_ESTADIO_PC IS NULL AND VL_CD4_PC IS NULL'
      #9#9#9#9'AND TP_ENTRADA = 2'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'))'
      'GROUP BY FL_SEXO, FAIXA'
      'ORDER BY FL_SEXO ASC, CASE WHEN FAIXA = '#39'<1 Ano'#39' THEN '#39'1'#39
      '              WHEN FAIXA = '#39'1-3 Anos'#39' THEN '#39'2'#39
      '              WHEN FAIXA = '#39'4-5 Anos'#39' THEN '#39'3'#39
      '              WHEN FAIXA = '#39'6-9 Anos'#39' THEN '#39'4'#39
      '              WHEN FAIXA = '#39'10-14 Anos'#39' THEN '#39'5'#39
      '              WHEN FAIXA = '#39'15-19 Anos'#39' THEN '#39'6'#39
      '              WHEN FAIXA = '#39'20-24 Anos'#39' THEN '#39'7'#39
      '              WHEN FAIXA = '#39'25-49 Anos'#39' THEN '#39'8'#39
      '              WHEN FAIXA = '#39'>50 Anos'#39' THEN '#39'9'#39
      '              ELSE FAIXA END ASC')
    Left = 10
    Top = 292
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end>
    object qyTransferenciaFAIXA: TIBStringField
      FieldName = 'FAIXA'
      Required = True
      FixedChar = True
      Size = 10
    end
    object qyTransferenciaFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      FixedChar = True
      Size = 9
    end
    object qyTransferenciaPACIENTES_COM_1_CONSULTA: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA'
    end
    object qyTransferenciaPACIENTES_COM_1_CONSULTA_E_TARV: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA_E_TARV'
    end
    object qyTransferenciaPACIENTES_COM_1_CONSULTA_E_TAR1: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA_E_TAR1'
    end
    object qyTransferenciaPACIENTES_COM_1_CONSULTA_SEM_TA: TLargeintField
      FieldName = 'PACIENTES_COM_1_CONSULTA_SEM_TA'
    end
    object qyTransferenciaTRANSF_CD4_MENOR_350: TLargeintField
      FieldName = 'TRANSF_CD4_MENOR_350'
    end
    object qyTransferenciaTRANSF_CD4_351_500: TLargeintField
      FieldName = 'TRANSF_CD4_351_500'
    end
    object qyTransferenciaTRANSF_CD4_MAIOR_500: TLargeintField
      FieldName = 'TRANSF_CD4_MAIOR_500'
    end
    object qyTransferenciaTRANSF_CD4_NULLO: TLargeintField
      FieldName = 'TRANSF_CD4_NULLO'
    end
    object qyTransferenciaTRANSF_OMS_I: TLargeintField
      FieldName = 'TRANSF_OMS_I'
    end
    object qyTransferenciaTRANSF_OMS_II: TLargeintField
      FieldName = 'TRANSF_OMS_II'
    end
    object qyTransferenciaTRANSF_OMS_III: TLargeintField
      FieldName = 'TRANSF_OMS_III'
    end
    object qyTransferenciaTRANSF_OMS_IV: TLargeintField
      FieldName = 'TRANSF_OMS_IV'
    end
    object qyTransferenciaTRANSF_OMS_NULLO: TLargeintField
      FieldName = 'TRANSF_OMS_NULLO'
    end
    object qyTransferenciaTRANSF_SEM_CD4_E_OMS: TLargeintField
      FieldName = 'TRANSF_SEM_CD4_E_OMS'
    end
  end
  object dsTransferencia: TDataSource
    DataSet = qyTransferencia
    Left = 42
    Top = 293
  end
  object frxDBTransferidos: TfrxDBDataset
    UserName = 'frxDBTransferidos'
    CloseDataSource = False
    DataSource = dsTransferencia
    BCDToCurrency = False
    Left = 105
    Top = 294
  end
  object frxDBAcompanhados: TfrxDBDataset
    UserName = 'frxDBAcompanhados'
    CloseDataSource = False
    FieldAliases.Strings = (
      'FAIXA=FAIXA'
      'FL_SEXO=FL_SEXO'
      'PACIENTES_INICIARAM_TARV=PACIENTES_INICIARAM_TARV'
      'PACIENTES_TARV_ANTERIOR=PACIENTES_TARV_ANTERIOR'
      'PACIENTES_PRE_TARV=PACIENTES_PRE_TARV'
      'ACOMPANHADOS_CD4_MENOR_350=ACOMPANHADOS_CD4_MENOR_350'
      'ACOMPANHADOS_CD4_351_500=ACOMPANHADOS_CD4_351_500'
      'ACOMPANHADOS_CD4_MAIOR_500=ACOMPANHADOS_CD4_MAIOR_500'
      'ACOMPANHADOS_CD4_NULLO=ACOMPANHADOS_CD4_NULLO'
      'ACOMPANHADOS_OMS_I=ACOMPANHADOS_OMS_I'
      'ACOMPANHADOS_OMS_II=ACOMPANHADOS_OMS_II'
      'ACOMPANHADOS_OMS_III=ACOMPANHADOS_OMS_III'
      'ACOMPANHADOS_OMS_IV=ACOMPANHADOS_OMS_IV'
      'ACOMPANHADOS_OMS_NULLO=ACOMPANHADOS_OMS_NULLO'
      'ACOMPANHADOS_SEM_CD4_E_OMS=ACOMPANHADOS_SEM_CD4_E_OMS')
    DataSource = dsAcompanhados
    BCDToCurrency = False
    Left = 104
    Top = 264
  end
  object qyAcompanhados: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT FAIXA,'
      
        #9'CASE FL_SEXO WHEN '#39'M'#39' THEN '#39'MASCULINO'#39' WHEN '#39'F'#39' THEN '#39'FEMININO'#39 +
        ' END FL_SEXO,'
      #9'SUM(PACIENTES_INICIARAM_TARV) PACIENTES_INICIARAM_TARV,'
      #9'SUM(PACIENTES_TARV_ANTERIOR) PACIENTES_TARV_ANTERIOR,'
      #9'SUM(PACIENTES_PRE_TARV) PACIENTES_PRE_TARV,'
      #9'SUM(ACOMPANHADOS_CD4_MENOR_350) ACOMPANHADOS_CD4_MENOR_350,'
      #9'SUM(ACOMPANHADOS_CD4_351_500) ACOMPANHADOS_CD4_351_500,'
      #9'SUM(ACOMPANHADOS_CD4_MAIOR_500) ACOMPANHADOS_CD4_MAIOR_500,'
      #9'SUM(ACOMPANHADOS_CD4_NULLO) ACOMPANHADOS_CD4_NULLO,'
      #9'SUM(ACOMPANHADOS_OMS_I) ACOMPANHADOS_OMS_I,'
      #9'SUM(ACOMPANHADOS_OMS_II) ACOMPANHADOS_OMS_II,'
      #9'SUM(ACOMPANHADOS_OMS_III) ACOMPANHADOS_OMS_III,'
      #9'SUM(ACOMPANHADOS_OMS_IV) ACOMPANHADOS_OMS_IV,'
      #9'SUM(ACOMPANHADOS_OMS_NULLO) ACOMPANHADOS_OMS_NULLO,'
      #9'SUM(ACOMPANHADOS_SEM_CD4_E_OMS) ACOMPANHADOS_SEM_CD4_E_OMS'
      'FROM'
      '    (select'
      #9#9'PACIENTES_INICIARAM_TARV,'
      #9#9'PACIENTES_TARV_ANTERIOR,'
      #9#9'PACIENTES_PRE_TARV,'
      #9#9'ACOMPANHADOS_CD4_MENOR_350,'
      #9#9'ACOMPANHADOS_CD4_351_500,'
      #9#9'ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9'ACOMPANHADOS_CD4_NULLO,'
      #9#9'ACOMPANHADOS_OMS_I,'
      #9#9'ACOMPANHADOS_OMS_II,'
      #9#9'ACOMPANHADOS_OMS_III,'
      #9#9'ACOMPANHADOS_OMS_IV,'
      #9#9'ACOMPANHADOS_OMS_NULLO,'
      #9#9'ACOMPANHADOS_SEM_CD4_E_OMS,'
      '        FL_SEXO,'
      '        CASE'
      '        WHEN NR_IDADE < 1 THEN '#39'<1 Ano'#39
      '        WHEN NR_IDADE >=1 AND NR_IDADE <=3 THEN '#39'1-3 Anos'#39
      '        WHEN NR_IDADE >=4 AND NR_IDADE <=5 THEN '#39'4-5 Anos'#39
      '        WHEN NR_IDADE >=6 AND NR_IDADE <=9 THEN '#39'6-9 Anos'#39
      '        WHEN NR_IDADE >=10 AND NR_IDADE <=14 THEN '#39'10-14 Anos'#39
      '        WHEN NR_IDADE >=15 AND NR_IDADE <=19 THEN '#39'15-19 Anos'#39
      '        WHEN NR_IDADE >=20 AND NR_IDADE <=24 THEN '#39'20-24 Anos'#39
      '        WHEN NR_IDADE >=25 AND NR_IDADE <=49 THEN '#39'25-49 Anos'#39
      '        WHEN NR_IDADE >=50 THEN '#39'>50 Anos'#39
      '        else '#39'N'#227'o Consta'#39
      '        END AS FAIXA'
      '    from'
      '        (SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      
        #9#9#9#9'AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIMEIRO_TARV <=' +
        ' :DATA_FIM'
      #9#9#9')'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND DT_PRIMEIRO_TARV < :DATA_INICIO)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'COUNT(NR_IDADE) PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND DT_PRIMEIRO_TARV IS NULL)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_CD4_PC <= 350'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_CD4_PC > 350 AND VL_CD4_PC <= 500'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO '
      #9#9#9#9'AND VL_CD4_PC > 500'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_CD4_PC IS NULL'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'I'#39
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'II'#39
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'III'#39
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_ESTADIO_PC = '#39'IV'#39
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'0 ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_ESTADIO_PC IS NULL'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      #9#9#9'NR_IDADE,'
      #9#9#9'FL_SEXO,'
      #9#9#9'0 PACIENTES_INICIARAM_TARV,'
      #9#9#9'0 PACIENTES_TARV_ANTERIOR,'
      #9#9#9'0 PACIENTES_PRE_TARV,'
      #9#9#9'0 ACOMPANHADOS_CD4_MENOR_350,'
      #9#9#9'0 ACOMPANHADOS_CD4_351_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_MAIOR_500,'
      #9#9#9'0 ACOMPANHADOS_CD4_NULLO,'
      #9#9#9'0 ACOMPANHADOS_OMS_I,'
      #9#9#9'0 ACOMPANHADOS_OMS_II,'
      #9#9#9'0 ACOMPANHADOS_OMS_III,'
      #9#9#9'0 ACOMPANHADOS_OMS_IV,'
      #9#9#9'0 ACOMPANHADOS_OMS_NULLO,'
      #9#9#9'COUNT(NR_IDADE) ACOMPANHADOS_SEM_CD4_E_OMS'
      #9#9'FROM'
      #9#9#9'(SELECT DISTINCT'
      #9#9#9#9'N.CD_PACIENTE,'
      #9#9#9#9'MML.NR_IDADE_ANOS NR_IDADE,'
      #9#9#9#9'FL_SEXO'
      #9#9#9'FROM notificacao N'
      #9#9#9'INNER JOIN MAPA_MEDICO_LINHA MML'
      #9#9#9#9'ON N.CD_PACIENTE = MML.CD_PACIENTE'
      #9#9#9'INNER JOIN MAPA_MEDICO MM'
      #9#9#9#9'ON MM.CD_MAPA = MML.CD_MAPA'
      #9#9#9'WHERE'
      #9#9#9#9'DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      #9#9#9#9'AND VL_ESTADIO_PC IS NULL AND VL_CD4_PC IS NULL'
      #9#9#9'ORDER BY DT_MAPA ASC)'
      #9#9'GROUP BY NR_IDADE, FL_SEXO'
      #9#9'))'
      'GROUP BY FL_SEXO, FAIXA'
      'ORDER BY FL_SEXO ASC, CASE WHEN FAIXA = '#39'<1 Ano'#39' THEN '#39'1'#39
      '              WHEN FAIXA = '#39'1-3 Anos'#39' THEN '#39'2'#39
      '              WHEN FAIXA = '#39'4-5 Anos'#39' THEN '#39'3'#39
      '              WHEN FAIXA = '#39'6-9 Anos'#39' THEN '#39'4'#39
      '              WHEN FAIXA = '#39'10-14 Anos'#39' THEN '#39'5'#39
      '              WHEN FAIXA = '#39'15-19 Anos'#39' THEN '#39'6'#39
      '              WHEN FAIXA = '#39'20-24 Anos'#39' THEN '#39'7'#39
      '              WHEN FAIXA = '#39'25-49 Anos'#39' THEN '#39'8'#39
      '              WHEN FAIXA = '#39'>50 Anos'#39' THEN '#39'9'#39
      '              ELSE FAIXA END ASC')
    Left = 8
    Top = 264
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end>
    object qyAcompanhadosFAIXA: TIBStringField
      FieldName = 'FAIXA'
      Required = True
      FixedChar = True
      Size = 10
    end
    object qyAcompanhadosFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      FixedChar = True
      Size = 9
    end
    object qyAcompanhadosPACIENTES_INICIARAM_TARV: TLargeintField
      FieldName = 'PACIENTES_INICIARAM_TARV'
    end
    object qyAcompanhadosPACIENTES_TARV_ANTERIOR: TLargeintField
      FieldName = 'PACIENTES_TARV_ANTERIOR'
    end
    object qyAcompanhadosPACIENTES_PRE_TARV: TLargeintField
      FieldName = 'PACIENTES_PRE_TARV'
    end
    object qyAcompanhadosACOMPANHADOS_CD4_MENOR_350: TLargeintField
      FieldName = 'ACOMPANHADOS_CD4_MENOR_350'
    end
    object qyAcompanhadosACOMPANHADOS_CD4_351_500: TLargeintField
      FieldName = 'ACOMPANHADOS_CD4_351_500'
    end
    object qyAcompanhadosACOMPANHADOS_CD4_MAIOR_500: TLargeintField
      FieldName = 'ACOMPANHADOS_CD4_MAIOR_500'
    end
    object qyAcompanhadosACOMPANHADOS_CD4_NULLO: TLargeintField
      FieldName = 'ACOMPANHADOS_CD4_NULLO'
    end
    object qyAcompanhadosACOMPANHADOS_OMS_I: TLargeintField
      FieldName = 'ACOMPANHADOS_OMS_I'
    end
    object qyAcompanhadosACOMPANHADOS_OMS_II: TLargeintField
      FieldName = 'ACOMPANHADOS_OMS_II'
    end
    object qyAcompanhadosACOMPANHADOS_OMS_III: TLargeintField
      FieldName = 'ACOMPANHADOS_OMS_III'
    end
    object qyAcompanhadosACOMPANHADOS_OMS_IV: TLargeintField
      FieldName = 'ACOMPANHADOS_OMS_IV'
    end
    object qyAcompanhadosACOMPANHADOS_OMS_NULLO: TLargeintField
      FieldName = 'ACOMPANHADOS_OMS_NULLO'
    end
    object qyAcompanhadosACOMPANHADOS_SEM_CD4_E_OMS: TLargeintField
      FieldName = 'ACOMPANHADOS_SEM_CD4_E_OMS'
    end
  end
  object dsAcompanhados: TDataSource
    DataSet = qyAcompanhados
    Left = 40
    Top = 264
  end
  object qyConsolidados: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT FAIXA,'
      
        '    CASE FL_SEXO WHEN '#39'M'#39' THEN '#39'MASCULINO'#39' WHEN '#39'F'#39' THEN '#39'FEMINI' +
        'NO'#39' END FL_SEXO,'
      '    SUM(TOTAL_ATENDIMENTOS) TOTAL_ATENDIMENTOS,'
      '    SUM(TOTAL_1_CONSULTA) TOTAL_1_CONSULTA,'
      '    SUM(PACIENTES_INICIARAM_TARV) PACIENTES_INICIARAM_TARV,'
      '    SUM(PACIENTES_TARV_ANTERIOR) PACIENTES_TARV_ANTERIOR,'
      '    SUM(PACIENTES_PRE_TARV) PACIENTES_PRE_TARV,'
      '    SUM(CONSOLIDADOS_CD4_MENOR_350) CONSOLIDADOS_CD4_MENOR_350,'
      '    SUM(CONSOLIDADOS_CD4_351_500) CONSOLIDADOS_CD4_351_500,'
      '    SUM(CONSOLIDADOS_CD4_MAIOR_500) CONSOLIDADOS_CD4_MAIOR_500,'
      '    SUM(CONSOLIDADOS_CD4_NULLO) CONSOLIDADOS_CD4_NULLO,'
      '    SUM(CONSOLIDADOS_OMS_I) CONSOLIDADOS_OMS_I,'
      '    SUM(CONSOLIDADOS_OMS_II) CONSOLIDADOS_OMS_II,'
      '    SUM(CONSOLIDADOS_OMS_III) CONSOLIDADOS_OMS_III,'
      '    SUM(CONSOLIDADOS_OMS_IV) CONSOLIDADOS_OMS_IV,'
      '    SUM(CONSOLIDADOS_OMS_NULLO) CONSOLIDADOS_OMS_NULLO,'
      '    SUM(CONSOLIDADOS_SEM_CD4_E_OMS) CONSOLIDADOS_SEM_CD4_E_OMS,'
      '    SUM(CV_MENOR_1000) CV_MENOR_1000,'
      '    SUM(CV_MAIOR_IGUAL_1000) CV_MAIOR_IGUAL_1000,'
      '    SUM(CV_IGUAL_LDL) CV_IGUAL_LDL,'
      '    SUM(CV_DETERMINADO) CV_DETERMINADO,'
      '    SUM(CV_INDETERMINADO) CV_INDETERMINADO,'
      '    SUM(CV_NULLO) CV_NULLO,'
      '    SUM(ABANDONO) ABANDONO,'
      '    SUM(OBITOS) OBITOS,'
      '    SUM(TRANSFERIDOS) TRANSFERIDOS'
      'FROM'
      '    (select'
      '        TOTAL_ATENDIMENTOS,'
      '        TOTAL_1_CONSULTA,'
      '        PACIENTES_INICIARAM_TARV,'
      '        PACIENTES_TARV_ANTERIOR,'
      '        PACIENTES_PRE_TARV,'
      '        CONSOLIDADOS_CD4_MENOR_350,'
      '        CONSOLIDADOS_CD4_351_500,'
      '        CONSOLIDADOS_CD4_MAIOR_500,'
      '        CONSOLIDADOS_CD4_NULLO,'
      '        CONSOLIDADOS_OMS_I,'
      '        CONSOLIDADOS_OMS_II,'
      '        CONSOLIDADOS_OMS_III,'
      '        CONSOLIDADOS_OMS_IV,'
      '        CONSOLIDADOS_OMS_NULLO,'
      '        CONSOLIDADOS_SEM_CD4_E_OMS,'
      '        CV_MENOR_1000,'
      '        CV_MAIOR_IGUAL_1000,'
      '        CV_IGUAL_LDL,'
      '        CV_DETERMINADO,'
      '        CV_INDETERMINADO,'
      '        CV_NULLO,'
      '        ABANDONO,'
      '        OBITOS,'
      '        TRANSFERIDOS,'
      '        FL_SEXO,'
      '        CASE'
      '        WHEN NR_IDADE < 1 THEN '#39'<1 Ano'#39
      '        WHEN NR_IDADE >=1 AND NR_IDADE <=3 THEN '#39'1-3 Anos'#39
      '        WHEN NR_IDADE >=4 AND NR_IDADE <=5 THEN '#39'4-5 Anos'#39
      '        WHEN NR_IDADE >=6 AND NR_IDADE <=9 THEN '#39'6-9 Anos'#39
      '        WHEN NR_IDADE >=10 AND NR_IDADE <=14 THEN '#39'10-14 Anos'#39
      '        WHEN NR_IDADE >=15 AND NR_IDADE <=19 THEN '#39'15-19 Anos'#39
      '        WHEN NR_IDADE >=20 AND NR_IDADE <=24 THEN '#39'20-24 Anos'#39
      '        WHEN NR_IDADE >=25 AND NR_IDADE <=49 THEN '#39'25-49 Anos'#39
      '        WHEN NR_IDADE >=50 THEN '#39'>50 Anos'#39
      '        else '#39'N'#227'o Consta'#39
      '        END AS FAIXA'
      '    from'
      '        (SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            COUNT(NR_IDADE) TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                MML.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                MML.FL_SEXO'
      '            FROM MAPA_MEDICO_LINHA MML'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      
        '                MM.DT_MAPA >= :DATA_INICIO AND MM.DT_MAPA <= :DA' +
        'TA_FIM'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            COUNT(NR_IDADE) TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            WHERE'
      
        '                DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRIM' +
        'EIRA_CONSULTA <= :DATA_FIM'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA =1))'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            COUNT(NR_IDADE) PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      
        '                AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIM' +
        'EIRO_TARV <= :DATA_FIM)'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      
        '                AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIM' +
        'EIRO_TARV <= :DATA_FIM'
      '                AND DT_PRIMEIRO_TARV >= DT_PRIMEIRA_CONSULTA'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            COUNT(NR_IDADE) PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND DT_PRIMEIRO_TARV < :DATA_INICIO)'
      
        '                OR (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_' +
        'PRIMEIRA_CONSULTA <= :DATA_FIM'
      
        '                AND DT_PRIMEIRO_TARV >= :DATA_INICIO AND DT_PRIM' +
        'EIRO_TARV <= :DATA_FIM'
      '                AND DT_PRIMEIRO_TARV < DT_PRIMEIRA_CONSULTA'
      '                AND TP_ENTRADA = 2))'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            COUNT(NR_IDADE) PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            WHERE'
      '                ((DT_PRIMEIRA_CONSULTA < :DATA_INICIO)'
      '                OR '
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND DT_PRIMEIRO_TARV IS NULL'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA =1))'
      '                )'
      '                AND DT_PRIMEIRO_TARV IS NULL)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_CD4_PC <= 350'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '                OR'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_CD4_PC <= 350)'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_CD4_PC > 350 AND VL_CD4_PC <= 500)'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_CD4_PC > 350 AND VL_CD4_PC <= 500'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO '
      '                AND VL_CD4_PC > 500)'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_CD4_PC > 500'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_CD4_PC IS NULL)'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_CD4_PC IS NULL'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_ESTADIO_PC = '#39'I'#39')'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_ESTADIO_PC = '#39'I'#39
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA =1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_ESTADIO_PC = '#39'II'#39')'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_ESTADIO_PC = '#39'II'#39
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_ESTADIO_PC = '#39'III'#39')'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_ESTADIO_PC = '#39'III'#39
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_ESTADIO_PC = '#39'IV'#39')'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_ESTADIO_PC = '#39'IV'#39
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_ESTADIO_PC IS NULL)'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_ESTADIO_PC IS NULL'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            COUNT(NR_IDADE) CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      '                (DT_PRIMEIRA_CONSULTA < :DATA_INICIO'
      '                AND VL_ESTADIO_PC IS NULL AND VL_CD4_PC IS NULL)'
      '                OR'
      
        '                (DT_PRIMEIRA_CONSULTA >= :DATA_INICIO AND DT_PRI' +
        'MEIRA_CONSULTA <= :DATA_FIM'
      '                AND VL_ESTADIO_PC IS NULL AND VL_CD4_PC IS NULL'
      '                AND (TP_ENTRADA = 2 OR TP_ENTRADA = 1))'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            COUNT(NR_IDADE) CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      
        '                MM.DT_MAPA >= :DATA_INICIO AND MM.DT_MAPA <= :DA' +
        'TA_FIM'
      
        '                AND (TRIM(VL_CV_UC) SIMILAR TO '#39'[0-9]+'#39' AND VL_C' +
        'V_UC < 1000)'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            COUNT(NR_IDADE) CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      
        '                MM.DT_MAPA >= :DATA_INICIO AND MM.DT_MAPA <= :DA' +
        'TA_FIM'
      
        '                AND (TRIM(VL_CV_UC) SIMILAR TO '#39'[0-9]+'#39' AND VL_C' +
        'V_UC >= 1000)'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            COUNT(NR_IDADE) CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      
        '                MM.DT_MAPA >= :DATA_INICIO AND MM.DT_MAPA <= :DA' +
        'TA_FIM'
      '                AND TRIM(VL_CV_UC) = '#39'<LDL'#39
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            COUNT(NR_IDADE) CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MCEL.NR_IDADE_MESES NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_CRIANCA_EXPOSTA_LINHA MCEL'
      '                ON N.CD_PACIENTE = MCEL.CD_PACIENTE'
      '            INNER JOIN MAPA_CRIANCA_EXPOSTA MCE'
      
        '                ON MCE.CD_MAPA_CRIANCA_EXPOSTA = MCEL.CD_MAPA_CR' +
        'IANCA_EXPOSTA'
      '            WHERE'
      
        '                MCE.DT_MAPA >= :DATA_INICIO AND MCE.DT_MAPA <= :' +
        'DATA_FIM'
      '                AND TRIM(VL_DPI_UC) = '#39'DETERMINADO'#39
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            COUNT(NR_IDADE) CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MCEL.NR_IDADE_MESES NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_CRIANCA_EXPOSTA_LINHA MCEL'
      '                ON N.CD_PACIENTE = MCEL.CD_PACIENTE'
      '            INNER JOIN MAPA_CRIANCA_EXPOSTA MCE'
      
        '                ON MCE.CD_MAPA_CRIANCA_EXPOSTA = MCEL.CD_MAPA_CR' +
        'IANCA_EXPOSTA'
      '            WHERE'
      
        '                MCE.DT_MAPA >= :DATA_INICIO AND MCE.DT_MAPA <= :' +
        'DATA_FIM'
      '                AND TRIM(VL_DPI_UC) = '#39'INDETERMINADO'#39
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            COUNT(NR_IDADE) CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                MML.NR_IDADE_ANOS NR_IDADE,'
      '                FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON N.CD_PACIENTE = MML.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO MM'
      '                ON MM.CD_MAPA = MML.CD_MAPA'
      '            WHERE'
      
        '                MM.DT_MAPA >= :DATA_INICIO AND MM.DT_MAPA <= :DA' +
        'TA_FIM'
      '                AND VL_CV_UC IS NULL'
      '            ORDER BY DT_MAPA ASC)'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            COUNT(NR_IDADE) ABANDONO,'
      '            0 OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      
        '                N.DT_EVOLUCAO_2 >= :DATA_INICIO AND N.DT_EVOLUCA' +
        'O_2 <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 4'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            COUNT(NR_IDADE) OBITOS,'
      '            0 TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      
        '                N.DT_EVOLUCAO_2 >= :DATA_INICIO AND N.DT_EVOLUCA' +
        'O_2 <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 2'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        UNION ALL'
      '        SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_ATENDIMENTOS,'
      '            0 TOTAL_1_CONSULTA,'
      '            0 PACIENTES_INICIARAM_TARV,'
      '            0 PACIENTES_TARV_ANTERIOR,'
      '            0 PACIENTES_PRE_TARV,'
      '            0 CONSOLIDADOS_CD4_MENOR_350,'
      '            0 CONSOLIDADOS_CD4_351_500,'
      '            0 CONSOLIDADOS_CD4_MAIOR_500,'
      '            0 CONSOLIDADOS_CD4_NULLO,'
      '            0 CONSOLIDADOS_OMS_I,'
      '            0 CONSOLIDADOS_OMS_II,'
      '            0 CONSOLIDADOS_OMS_III,'
      '            0 CONSOLIDADOS_OMS_IV,'
      '            0 CONSOLIDADOS_OMS_NULLO,'
      '            0 CONSOLIDADOS_SEM_CD4_E_OMS,'
      '            0 CV_MENOR_1000,'
      '            0 CV_MAIOR_IGUAL_1000,'
      '            0 CV_IGUAL_LDL,'
      '            0 CV_DETERMINADO,'
      '            0 CV_INDETERMINADO,'
      '            0 CV_NULLO,'
      '            0 ABANDONO,'
      '            0 OBITOS,'
      '            COUNT(NR_IDADE) TRANSFERIDOS'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      
        '                N.DT_EVOLUCAO_2 >= :DATA_INICIO AND N.DT_EVOLUCA' +
        'O_2 <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 3'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        ))'
      'GROUP BY FL_SEXO, FAIXA'
      'ORDER BY FL_SEXO ASC, CASE WHEN FAIXA = '#39'<1 Ano'#39' THEN '#39'1'#39
      '              WHEN FAIXA = '#39'1-3 Anos'#39' THEN '#39'2'#39
      '              WHEN FAIXA = '#39'4-5 Anos'#39' THEN '#39'3'#39
      '              WHEN FAIXA = '#39'6-9 Anos'#39' THEN '#39'4'#39
      '              WHEN FAIXA = '#39'10-14 Anos'#39' THEN '#39'5'#39
      '              WHEN FAIXA = '#39'15-19 Anos'#39' THEN '#39'6'#39
      '              WHEN FAIXA = '#39'20-24 Anos'#39' THEN '#39'7'#39
      '              WHEN FAIXA = '#39'25-49 Anos'#39' THEN '#39'8'#39
      '              WHEN FAIXA = '#39'>50 Anos'#39' THEN '#39'9'#39
      '              ELSE FAIXA END ASC')
    Left = 8
    Top = 232
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_INICIO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end>
    object qyConsolidadosFAIXA: TIBStringField
      FieldName = 'FAIXA'
      Required = True
      FixedChar = True
      Size = 10
    end
    object qyConsolidadosFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      FixedChar = True
      Size = 9
    end
    object qyConsolidadosTOTAL_ATENDIMENTOS: TLargeintField
      FieldName = 'TOTAL_ATENDIMENTOS'
    end
    object qyConsolidadosTOTAL_1_CONSULTA: TLargeintField
      FieldName = 'TOTAL_1_CONSULTA'
    end
    object qyConsolidadosPACIENTES_INICIARAM_TARV: TLargeintField
      FieldName = 'PACIENTES_INICIARAM_TARV'
    end
    object qyConsolidadosPACIENTES_TARV_ANTERIOR: TLargeintField
      FieldName = 'PACIENTES_TARV_ANTERIOR'
    end
    object qyConsolidadosPACIENTES_PRE_TARV: TLargeintField
      FieldName = 'PACIENTES_PRE_TARV'
    end
    object qyConsolidadosCONSOLIDADOS_CD4_MENOR_350: TLargeintField
      FieldName = 'CONSOLIDADOS_CD4_MENOR_350'
    end
    object qyConsolidadosCONSOLIDADOS_CD4_351_500: TLargeintField
      FieldName = 'CONSOLIDADOS_CD4_351_500'
    end
    object qyConsolidadosCONSOLIDADOS_CD4_MAIOR_500: TLargeintField
      FieldName = 'CONSOLIDADOS_CD4_MAIOR_500'
    end
    object qyConsolidadosCONSOLIDADOS_CD4_NULLO: TLargeintField
      FieldName = 'CONSOLIDADOS_CD4_NULLO'
    end
    object qyConsolidadosCONSOLIDADOS_OMS_I: TLargeintField
      FieldName = 'CONSOLIDADOS_OMS_I'
    end
    object qyConsolidadosCONSOLIDADOS_OMS_II: TLargeintField
      FieldName = 'CONSOLIDADOS_OMS_II'
    end
    object qyConsolidadosCONSOLIDADOS_OMS_III: TLargeintField
      FieldName = 'CONSOLIDADOS_OMS_III'
    end
    object qyConsolidadosCONSOLIDADOS_OMS_IV: TLargeintField
      FieldName = 'CONSOLIDADOS_OMS_IV'
    end
    object qyConsolidadosCONSOLIDADOS_OMS_NULLO: TLargeintField
      FieldName = 'CONSOLIDADOS_OMS_NULLO'
    end
    object qyConsolidadosCONSOLIDADOS_SEM_CD4_E_OMS: TLargeintField
      FieldName = 'CONSOLIDADOS_SEM_CD4_E_OMS'
    end
    object qyConsolidadosCV_MENOR_1000: TLargeintField
      FieldName = 'CV_MENOR_1000'
    end
    object qyConsolidadosCV_MAIOR_IGUAL_1000: TLargeintField
      FieldName = 'CV_MAIOR_IGUAL_1000'
    end
    object qyConsolidadosCV_IGUAL_LDL: TLargeintField
      FieldName = 'CV_IGUAL_LDL'
    end
    object qyConsolidadosCV_DETERMINADO: TLargeintField
      FieldName = 'CV_DETERMINADO'
    end
    object qyConsolidadosCV_INDETERMINADO: TLargeintField
      FieldName = 'CV_INDETERMINADO'
    end
    object qyConsolidadosCV_NULLO: TLargeintField
      FieldName = 'CV_NULLO'
    end
    object qyConsolidadosABANDONO: TLargeintField
      FieldName = 'ABANDONO'
    end
    object qyConsolidadosOBITOS: TLargeintField
      FieldName = 'OBITOS'
    end
    object qyConsolidadosTRANSFERIDOS: TLargeintField
      FieldName = 'TRANSFERIDOS'
    end
  end
  object frxDBConsolidados: TfrxDBDataset
    UserName = 'frxDBConsolidados'
    CloseDataSource = False
    FieldAliases.Strings = (
      'FAIXA=FAIXA'
      'FL_SEXO=FL_SEXO'
      'TOTAL_ATENDIMENTOS=TOTAL_ATENDIMENTOS'
      'TOTAL_1_CONSULTA=TOTAL_1_CONSULTA'
      'PACIENTES_INICIARAM_TARV=PACIENTES_INICIARAM_TARV'
      'PACIENTES_TARV_ANTERIOR=PACIENTES_TARV_ANTERIOR'
      'PACIENTES_PRE_TARV=PACIENTES_PRE_TARV'
      'CONSOLIDADOS_CD4_MENOR_350=CONSOLIDADOS_CD4_MENOR_350'
      'CONSOLIDADOS_CD4_351_500=CONSOLIDADOS_CD4_351_500'
      'CONSOLIDADOS_CD4_MAIOR_500=CONSOLIDADOS_CD4_MAIOR_500'
      'CONSOLIDADOS_CD4_NULLO=CONSOLIDADOS_CD4_NULLO'
      'CONSOLIDADOS_OMS_I=CONSOLIDADOS_OMS_I'
      'CONSOLIDADOS_OMS_II=CONSOLIDADOS_OMS_II'
      'CONSOLIDADOS_OMS_III=CONSOLIDADOS_OMS_III'
      'CONSOLIDADOS_OMS_IV=CONSOLIDADOS_OMS_IV'
      'CONSOLIDADOS_OMS_NULLO=CONSOLIDADOS_OMS_NULLO'
      'CONSOLIDADOS_SEM_CD4_E_OMS=CONSOLIDADOS_SEM_CD4_E_OMS'
      'CV_MENOR_1000=CV_MENOR_1000'
      'CV_MAIOR_IGUAL_1000=CV_MAIOR_IGUAL_1000'
      'CV_IGUAL_LDL=CV_IGUAL_LDL'
      'CV_DETERMINADO=CV_DETERMINADO'
      'CV_INDETERMINADO=CV_INDETERMINADO'
      'CV_NULLO=CV_NULLO'
      'ABANDONO=ABANDONO'
      'OBITOS=OBITOS'
      'TRANSFERIDOS=TRANSFERIDOS')
    DataSource = dsConsolidados
    BCDToCurrency = False
    Left = 104
    Top = 232
  end
  object dsConsolidados: TDataSource
    DataSet = qyConsolidados
    Left = 40
    Top = 232
  end
  object dsAcumulados: TDataSource
    DataSet = qyAcumulados
    Left = 40
    Top = 200
  end
  object qyAcumulados: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT FAIXA,'
      
        '    CASE FL_SEXO WHEN '#39'M'#39' THEN '#39'MASCULINO'#39' WHEN '#39'F'#39' THEN '#39'FEMINI' +
        'NO'#39' END FL_SEXO,'
      '    SUM(TOTAL_IDENTIFICADOS) TOTAL_IDENTIFICADOS,'
      #9'SUM(PCTS_COM_PROCESSO) PCTS_COM_PROCESSO,'
      #9'SUM(PCTS_ATIVOS) PCTS_ATIVOS,'
      #9'SUM(PCTS_ABANDONO) PCTS_ABANDONO,'
      #9'SUM(PCTS_OBITO) PCTS_OBITO,'
      #9'SUM(PCTS_TRANSFERIDOS) PCTS_TRANSFERIDOS,'
      #9'SUM(PCTS_COM_TARV) PCTS_COM_TARV,'
      #9'SUM(PCTS_PRE_TARV) PCTS_PRE_TARV'
      'FROM'
      '    (select'
      '        TOTAL_IDENTIFICADOS,'
      #9#9'PCTS_COM_PROCESSO,'
      #9#9'PCTS_ATIVOS,'
      #9#9'PCTS_ABANDONO,'
      #9#9'PCTS_OBITO,'
      #9#9'PCTS_TRANSFERIDOS,'
      #9#9'PCTS_COM_TARV,'
      #9#9'PCTS_PRE_TARV,'
      '        FL_SEXO,'
      '        CASE'
      '        WHEN NR_IDADE < 1 THEN '#39'<1 Ano'#39
      '        WHEN NR_IDADE >=1 AND NR_IDADE <=3 THEN '#39'1-3 Anos'#39
      '        WHEN NR_IDADE >=4 AND NR_IDADE <=5 THEN '#39'4-5 Anos'#39
      '        WHEN NR_IDADE >=6 AND NR_IDADE <=9 THEN '#39'6-9 Anos'#39
      '        WHEN NR_IDADE >=10 AND NR_IDADE <=14 THEN '#39'10-14 Anos'#39
      '        WHEN NR_IDADE >=15 AND NR_IDADE <=19 THEN '#39'15-19 Anos'#39
      '        WHEN NR_IDADE >=20 AND NR_IDADE <=24 THEN '#39'20-24 Anos'#39
      '        WHEN NR_IDADE >=25 AND NR_IDADE <=49 THEN '#39'25-49 Anos'#39
      '        WHEN NR_IDADE >=50 THEN '#39'>50 Anos'#39
      '        else '#39'N'#227'o Consta'#39
      '        END AS FAIXA'
      '    from'
      '        (SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            COUNT(NR_IDADE) TOTAL_IDENTIFICADOS,'
      #9#9#9'0 PCTS_COM_PROCESSO,'
      #9#9#9'0 PCTS_ATIVOS,'
      #9#9#9'0 PCTS_ABANDONO,'
      #9#9#9'0 PCTS_OBITO,'
      #9#9#9'0 PCTS_TRANSFERIDOS,'
      #9#9#9'0 PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      '                N.DT_NOTIFICACAO <= :DATA_FIM'
      '                AND N.TP_ENTRADA <> 5'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      #9#9#9'0 TOTAL_IDENTIFICADOS,'
      '            COUNT(NR_IDADE) PCTS_COM_PROCESSO,'
      #9#9#9'0 PCTS_ATIVOS,'
      #9#9#9'0 PCTS_ABANDONO,'
      #9#9#9'0 PCTS_OBITO,'
      #9#9#9'0 PCTS_TRANSFERIDOS,'
      #9#9#9'0 PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      '                N.DT_NOTIFICACAO <= :DATA_FIM'
      '                AND N.TP_ENTRADA <> 5'
      #9#9#9#9'AND P.NR_PROCESSO IS NOT NULL'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      #9#9#9'0 TOTAL_IDENTIFICADOS,'
      '            0 PCTS_COM_PROCESSO,'
      #9#9#9'COUNT(NR_IDADE) PCTS_ATIVOS,'
      #9#9#9'0 PCTS_ABANDONO,'
      #9#9#9'0 PCTS_OBITO,'
      #9#9#9'0 PCTS_TRANSFERIDOS,'
      #9#9#9'0 PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      '                N.DT_NOTIFICACAO <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 1'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      #9#9#9'0 TOTAL_IDENTIFICADOS,'
      '            0 PCTS_COM_PROCESSO,'
      #9#9#9'0 PCTS_ATIVOS,'
      #9#9#9'COUNT(NR_IDADE) PCTS_ABANDONO,'
      #9#9#9'0 PCTS_OBITO,'
      #9#9#9'0 PCTS_TRANSFERIDOS,'
      #9#9#9'0 PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      '                N.DT_EVOLUCAO_2 <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 4'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      #9#9#9'0 TOTAL_IDENTIFICADOS,'
      '            0 PCTS_COM_PROCESSO,'
      #9#9#9'0 PCTS_ATIVOS,'
      #9#9#9'0 PCTS_ABANDONO,'
      #9#9#9'COUNT(NR_IDADE) PCTS_OBITO,'
      #9#9#9'0 PCTS_TRANSFERIDOS,'
      #9#9#9'0 PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      '                N.DT_EVOLUCAO_2 <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 2'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      #9#9#9'0 TOTAL_IDENTIFICADOS,'
      '            0 PCTS_COM_PROCESSO,'
      #9#9#9'0 PCTS_ATIVOS,'
      #9#9#9'0 PCTS_ABANDONO,'
      #9#9#9'0 PCTS_OBITO,'
      #9#9#9'COUNT(NR_IDADE) PCTS_TRANSFERIDOS,'
      #9#9#9'0 PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            WHERE'
      '                N.DT_EVOLUCAO_2 <= :DATA_FIM'
      '                AND N.CD_EVOLUCAO = 3'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_IDENTIFICADOS,'
      '            0 PCTS_COM_PROCESSO,'
      '            0 PCTS_ATIVOS,'
      '            0 PCTS_ABANDONO,'
      '            0 PCTS_OBITO,'
      '            0 PCTS_TRANSFERIDOS,'
      '            COUNT(NR_IDADE) PCTS_COM_TARV,'
      #9#9#9'0 PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON MML.CD_PACIENTE = N.CD_PACIENTE'
      '            WHERE'
      '                MML.DT_ULTIMA_CONSULTA <= :DATA_FIM'
      
        '                AND (N.CD_EVOLUCAO = 1 or N.CD_EVOLUCAO IS NULL ' +
        ')'
      '                AND MML.DT_PRIMEIRO_TARV IS NOT NULL'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      #9#9'UNION ALL'
      #9#9'SELECT'
      '            NR_IDADE,'
      '            FL_SEXO,'
      '            0 TOTAL_IDENTIFICADOS,'
      '            0 PCTS_COM_PROCESSO,'
      '            0 PCTS_ATIVOS,'
      '            0 PCTS_ABANDONO,'
      '            0 PCTS_OBITO,'
      '            0 PCTS_TRANSFERIDOS,'
      '            0 PCTS_COM_TARV,'
      #9#9#9'COUNT(NR_IDADE) PCTS_PRE_TARV'
      '        FROM'
      '            (SELECT DISTINCT'
      '                N.CD_PACIENTE,'
      '                P.IDADE NR_IDADE,'
      '                P.TP_SEXO FL_SEXO'
      '            FROM notificacao N'
      '            INNER JOIN PACIENTE P'
      '                ON N.CD_PACIENTE = P.CD_PACIENTE'
      '            INNER JOIN MAPA_MEDICO_LINHA MML'
      '                ON MML.CD_PACIENTE = N.CD_PACIENTE'
      '            WHERE'
      '                MML.DT_ULTIMA_CONSULTA <= :DATA_FIM'
      
        '                AND (N.CD_EVOLUCAO = 1 or N.CD_EVOLUCAO IS NULL ' +
        ')'
      '                AND MML.DT_PRIMEIRO_TARV IS NULL'
      '            )'
      '        GROUP BY NR_IDADE, FL_SEXO'
      '        ))'
      'GROUP BY FL_SEXO, FAIXA'
      'ORDER BY FL_SEXO ASC, CASE WHEN FAIXA = '#39'<1 Ano'#39' THEN '#39'1'#39
      '              WHEN FAIXA = '#39'1-3 Anos'#39' THEN '#39'2'#39
      '              WHEN FAIXA = '#39'4-5 Anos'#39' THEN '#39'3'#39
      '              WHEN FAIXA = '#39'6-9 Anos'#39' THEN '#39'4'#39
      '              WHEN FAIXA = '#39'10-14 Anos'#39' THEN '#39'5'#39
      '              WHEN FAIXA = '#39'15-19 Anos'#39' THEN '#39'6'#39
      '              WHEN FAIXA = '#39'20-24 Anos'#39' THEN '#39'7'#39
      '              WHEN FAIXA = '#39'25-49 Anos'#39' THEN '#39'8'#39
      '              WHEN FAIXA = '#39'>50 Anos'#39' THEN '#39'9'#39
      '              ELSE FAIXA END ASC')
    Left = 8
    Top = 200
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'DATA_FIM'
        ParamType = ptUnknown
      end>
    object qyAcumuladosFAIXA: TIBStringField
      FieldName = 'FAIXA'
      Required = True
      FixedChar = True
      Size = 10
    end
    object qyAcumuladosFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      FixedChar = True
      Size = 9
    end
    object qyAcumuladosTOTAL_IDENTIFICADOS: TLargeintField
      FieldName = 'TOTAL_IDENTIFICADOS'
    end
    object qyAcumuladosPCTS_COM_PROCESSO: TLargeintField
      FieldName = 'PCTS_COM_PROCESSO'
    end
    object qyAcumuladosPCTS_ATIVOS: TLargeintField
      FieldName = 'PCTS_ATIVOS'
    end
    object qyAcumuladosPCTS_ABANDONO: TLargeintField
      FieldName = 'PCTS_ABANDONO'
    end
    object qyAcumuladosPCTS_OBITO: TLargeintField
      FieldName = 'PCTS_OBITO'
    end
    object qyAcumuladosPCTS_TRANSFERIDOS: TLargeintField
      FieldName = 'PCTS_TRANSFERIDOS'
    end
    object qyAcumuladosPCTS_COM_TARV: TLargeintField
      FieldName = 'PCTS_COM_TARV'
    end
    object qyAcumuladosPCTS_PRE_TARV: TLargeintField
      FieldName = 'PCTS_PRE_TARV'
    end
  end
  object frxDBAcumulados: TfrxDBDataset
    UserName = 'frxDBAcumulados'
    CloseDataSource = False
    FieldAliases.Strings = (
      'FAIXA=FAIXA'
      'FL_SEXO=FL_SEXO'
      'TOTAL_IDENTIFICADOS=TOTAL_IDENTIFICADOS'
      'PCTS_COM_PROCESSO=PCTS_COM_PROCESSO'
      'PCTS_ATIVOS=PCTS_ATIVOS'
      'PCTS_ABANDONO=PCTS_ABANDONO'
      'PCTS_OBITO=PCTS_OBITO'
      'PCTS_TRANSFERIDOS=PCTS_TRANSFERIDOS'
      'PCTS_COM_TARV=PCTS_COM_TARV'
      'PCTS_PRE_TARV=PCTS_PRE_TARV')
    DataSource = dsAcumulados
    BCDToCurrency = False
    Left = 104
    Top = 200
  end
end
