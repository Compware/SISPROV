object frmRelGestante: TfrmRelGestante
  Left = 312
  Top = 128
  Width = 553
  Height = 566
  Caption = 'Relat'#243'rio de Gestantes'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 545
    Height = 81
    Align = alTop
    TabOrder = 0
    object lbStatus: TLabel
      Left = 775
      Top = 48
      Width = 3
      Height = 13
    end
    object Label6: TLabel
      Left = 419
      Top = 36
      Width = 6
      Height = 13
      Caption = '0'
      Visible = False
    end
    object ckComProcesso: TCheckBox
      Left = 167
      Top = 30
      Width = 97
      Height = 17
      Caption = 'Com Processo'
      TabOrder = 0
    end
    object btPesquisar: TBitBtn
      Left = 167
      Top = 51
      Width = 75
      Height = 25
      Caption = 'Pesquisar'
      TabOrder = 1
      OnClick = btPesquisarClick
    end
    object GroupBox1: TGroupBox
      Left = 12
      Top = 9
      Width = 151
      Height = 69
      TabOrder = 2
      object Label1: TLabel
        Left = 11
        Top = 18
        Width = 21
        Height = 13
        Caption = 'De:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 12
        Top = 41
        Width = 24
        Height = 13
        Caption = 'At'#233':'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object dtDe: TDateTimePicker
        Left = 40
        Top = 15
        Width = 105
        Height = 21
        Date = 42429.883027557870000000
        Time = 42429.883027557870000000
        TabOrder = 0
      end
      object dtAte: TDateTimePicker
        Left = 40
        Top = 38
        Width = 105
        Height = 21
        Date = 42429.883027557870000000
        Time = 42429.883027557870000000
        TabOrder = 1
      end
    end
    object ckPeriodo: TCheckBox
      Left = 18
      Top = 4
      Width = 139
      Height = 17
      Caption = 'Per'#237'odo Inicio'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object btCancelar: TBitBtn
      Left = 272
      Top = 32
      Width = 45
      Height = 45
      TabOrder = 4
      OnClick = btCancelarClick
      Glyph.Data = {
        
        FF7FFF7FFF7F}
      NumGlyphs = 2
    end
    object btEditar: TBitBtn
      Left = 317
      Top = 32
      Width = 45
      Height = 45
      TabOrder = 5
      OnClick = btEditarClick
      Glyph.Data = {
        
        FF7FFF7FFF7F}
      NumGlyphs = 2
    end
    object btGravar: TBitBtn
      Left = 363
      Top = 32
      Width = 45
      Height = 45
      TabOrder = 6
      OnClick = btGravarClick
      Glyph.Data = {
        
        FF7FFF7FFF7F}
      NumGlyphs = 2
    end
    object BitBtn1: TBitBtn
      Left = 408
      Top = 32
      Width = 45
      Height = 45
      TabOrder = 7
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        FF7FFF7FFF7F}
      NumGlyphs = 2
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 81
    Width = 545
    Height = 451
    Align = alClient
    TabOrder = 1
    object grDados: TdxDBGrid
      Left = 1
      Top = 1
      Width = 543
      Height = 311
      Bands = <
        item
          Caption = 'Gestantes'
          DisableCustomizing = True
          DisableDragging = True
          Width = 500
        end>
      DefaultLayout = False
      HeaderPanelRowCount = 1
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alClient
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      BandColor = 8404992
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWhite
      BandFont.Height = -13
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = [fsBold]
      DataSource = dsGestantes
      Filter.AutoDataSetFilter = True
      Filter.Criteria = {00000000}
      GroupPanelColor = 12615680
      HeaderColor = 15983825
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -11
      HeaderFont.Name = 'MS Sans Serif'
      HeaderFont.Style = [fsBold]
      OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoStoreToIniFile, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoUseBookmarks]
      OptionsView = [edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      ShowBands = True
      ShowPreviewGrid = False
      ShowRowFooter = True
      object grDadosN_Processo: TdxDBGridColumn
        Caption = 'Nr. Processo'
        ReadOnly = True
        Width = 80
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NR_PROCESSO'
      end
      object grDadosNMPaciente: TdxDBGridColumn
        Caption = 'Paciente'
        ReadOnly = True
        Width = 200
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NM_PACIENTE'
      end
      object grDadosFL_Gestante: TdxDBGridCheckColumn
        Caption = 'Gestante'
        ReadOnly = True
        Width = 59
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FL_GESTANTE'
        ValueChecked = '1'
        ValueUnchecked = '0'
      end
      object grDadosNR_Meses: TdxDBGridColumn
        Caption = 'Meses'
        ReadOnly = True
        Width = 45
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NR_MESES'
      end
      object grDadosDT_Notificacao: TdxDBGridDateColumn
        Caption = 'Data Inicio'
        ReadOnly = True
        Width = 70
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DT_INICIO_GESTACAO'
      end
      object grDadosDT_FIM_GESTACAO: TdxDBGridDateColumn
        Caption = 'Data Fim'
        ReadOnly = True
        Width = 70
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DT_FIM_GESTACAO'
      end
    end
    inline frameControleGestante1: TframeControleGestante
      Left = 1
      Top = 312
      Width = 543
      Height = 138
      Align = alBottom
      TabOrder = 1
      inherited Panel1: TPanel
        Left = 513
        Height = 138
      end
      inherited grGestante: TdxDBGrid
        Width = 513
        Height = 138
        Bands = <
          item
            Caption = 'Gesta'#231#245'es'
            Width = 238
          end>
        Filter.Criteria = {00000000}
      end
      inherited qyGestante: TIBDataSet
        DataSource = dsGestantes
      end
    end
  end
  object dsGestantes: TDataSource
    DataSet = qyGestantes
    Left = 368
    Top = 200
  end
  object qyGestantes: TIBDataSet
    Database = DM.DB
    Transaction = DM.TS
    AfterOpen = qyGestantesAfterOpen
    BeforeScroll = qyGestantesBeforeScroll
    OnCalcFields = qyGestantesCalcFields
    BufferChunks = 1000
    CachedUpdates = False
    SelectSQL.Strings = (
      'SELECT'
      #9'P.NR_PROCESSO,'
      #9'P.NM_PACIENTE,'
      #9'P.FL_GESTANTE,'
      #9'P.CD_PACIENTE,'
      #9'G.NR_MESES,'
      #9'G.DT_INICIO_GESTACAO,'
      #9'G.DT_FIM_GESTACAO'
      'FROM PACIENTE P'
      #9'LEFT OUTER JOIN GESTANTE G'
      #9'ON P.CD_PACIENTE = G.CD_PACIENTE'
      'WHERE'
      #9'(G.CD_GESTANTE IN'
      #9#9'(SELECT first 1 CD_GESTANTE FROM GESTANTE G2'
      
        #9#9#9'WHERE G2.CD_PACIENTE = P.CD_PACIENTE order by DT_INICIO_GESTA' +
        'CAO desc)'
      
        #9' OR (P.FL_GESTANTE = 1 and P.CD_PACIENTE not in (SELECT CD_PACI' +
        'ENTE FROM GESTANTE)))')
    Left = 336
    Top = 200
    object qyGestantesNR_PROCESSO: TIntegerField
      FieldName = 'NR_PROCESSO'
      Origin = 'PACIENTE.NR_PROCESSO'
    end
    object qyGestantesNM_PACIENTE: TIBStringField
      FieldName = 'NM_PACIENTE'
      Origin = 'PACIENTE.NM_PACIENTE'
      Required = True
      Size = 50
    end
    object qyGestantesFL_GESTANTE: TIntegerField
      FieldName = 'FL_GESTANTE'
      Origin = 'PACIENTE.FL_GESTANTE'
    end
    object qyGestantesNR_MESES: TIntegerField
      FieldName = 'NR_MESES'
      Origin = 'GESTANTE.NR_MESES'
    end
    object qyGestantesDT_INICIO_GESTACAO: TDateField
      FieldName = 'DT_INICIO_GESTACAO'
      Origin = 'GESTANTE.DT_INICIO_GESTACAO'
    end
    object qyGestantesDT_FIM_GESTACAO: TDateField
      FieldName = 'DT_FIM_GESTACAO'
      Origin = 'GESTANTE.DT_FIM_GESTACAO'
    end
    object qyGestantesCD_PACIENTE: TIntegerField
      FieldName = 'CD_PACIENTE'
      Origin = 'PACIENTE.CD_PACIENTE'
      Required = True
    end
    object qyGestantesCODIGO: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'CODIGO'
      Calculated = True
    end
  end
  object fxGestante: TfrxReport
    Version = '4.15.6'
    DataSet = fxdbUnidade
    DataSetName = 'Unidade'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 41894.334857743100000000
    ReportOptions.LastChange = 42579.400878900470000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      ''
      ''
      'procedure Memo125OnAfterData(Sender: TfrxComponent);'
      'begin                                                     '
      '  if (<Gestantes."FL_GESTANTE"> = 1) then '
      '    Memo125.Text := '#39'S'#39' '
      '  else '
      '    Memo125.Text := '#39'N'#39'   '
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 264
    Top = 248
    Datasets = <
      item
        DataSet = fxdbGestantes
        DataSetName = 'Gestantes'
      end
      item
        DataSet = fxdbUnidade
        DataSetName = 'Unidade'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      OnAfterPrint = 'Page1OnAfterPrint'
      object ReportTitle1: TfrxReportTitle
        Height = 200.315090000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture1: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo1: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'RELAT'#195#8220'RIO GESTANTE')
          ParentFont = False
        end
        object Memo44: TfrxMemoView
          Left = 16.118120000000000000
          Top = 158.740260000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Munic'#195#173'pio')
          ParentFont = False
        end
        object Memo45: TfrxMemoView
          Left = 16.118120000000000000
          Top = 181.417440000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Prov'#195#173'ncia')
          ParentFont = False
        end
        object Memo46: TfrxMemoView
          Left = 264.567100000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object Memo49: TfrxMemoView
          Left = 78.370130000000000000
          Top = 158.740260000000000000
          Width = 173.858380000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_MUNICIPIO'
          DataSet = fxdbUnidade
          DataSetName = 'Unidade'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Unidade."DS_MUNICIPIO"]')
          ParentFont = False
        end
        object Memo50: TfrxMemoView
          Left = 77.370130000000000000
          Top = 181.417440000000000000
          Width = 173.858380000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_PROVINCIA'
          DataSet = fxdbUnidade
          DataSetName = 'Unidade'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Unidade."DS_PROVINCIA"]')
          ParentFont = False
        end
        object Memo51: TfrxMemoView
          Left = 317.480520000000000000
          Top = 158.740260000000000000
          Width = 207.874150000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = fxdbUnidade
          DataSetName = 'Unidade'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Unidade."DS_UNIDADE"]')
          ParentFont = False
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 415.748300000000000000
        Width = 1046.929810000000000000
        object Shape3: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo9: TfrxMemoView
          Left = 84.929190000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 11.338590000000000000
          Top = 1.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 998.795920000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Left = 943.882500000000000000
          Top = 1.779530000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        Height = 26.574830000000000000
        Top = 328.819110000000000000
        Width = 1046.929810000000000000
        DataSet = fxdbGestantes
        DataSetName = 'Gestantes'
        RowCount = 0
        object Shape2: TfrxShapeView
          Width = 657.638049130000000000
          Height = 26.456692910000000000
          ShowHint = False
        end
        object Memo23: TfrxMemoView
          Left = 462.102660000000000000
          Top = 5.669295000000000000
          Width = 90.708720000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DT_INICIO_GESTACAO'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[Gestantes."DT_INICIO_GESTACAO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line1: TfrxLineView
          Left = 340.433161180000000000
          Top = 0.118120000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Line7: TfrxLineView
          Left = 556.590910000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo125: TfrxMemoView
          Left = 345.937230000000000000
          Top = 3.779530000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          OnAfterData = 'Memo125OnAfterData'
          ShowHint = False
          DataField = 'FL_GESTANTE'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[Gestantes."FL_GESTANTE"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          Left = 563.709030000000000000
          Top = 5.669295000000000000
          Width = 90.708720000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DT_FIM_GESTACAO'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[Gestantes."DT_FIM_GESTACAO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo6: TfrxMemoView
          Left = 406.134248820000000000
          Top = 3.779530000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'NR_MESES'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[Gestantes."NR_MESES"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line5: TfrxLineView
          Left = 460.323130000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo15: TfrxMemoView
          Top = 3.779530000000000000
          Width = 30.236240000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'CODIGO'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[Gestantes."CODIGO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line8: TfrxLineView
          Left = 34.015770000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo17: TfrxMemoView
          Left = 37.795300000000000000
          Top = 3.779530000000000000
          Width = 71.811023620000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'NR_PROCESSO'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8 = (
            '[Gestantes."NR_PROCESSO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line10: TfrxLineView
          Left = 111.385900000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Line2: TfrxLineView
          Left = 400.850650000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo19: TfrxMemoView
          Left = 112.385900000000000000
          Top = 3.779530000000000000
          Width = 226.771653540000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'NM_PACIENTE'
          DataSet = fxdbGestantes
          DataSetName = 'Gestantes'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Gestantes."NM_PACIENTE"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object PageHeader1: TfrxPageHeader
        Height = 27.456710000000000000
        Top = 241.889920000000000000
        Width = 1046.929810000000000000
        object Shape1: TfrxShapeView
          Top = 0.779530000000000000
          Width = 657.638049130000000000
          Height = 26.456710000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo4: TfrxMemoView
          Left = 561.457020000000000000
          Top = 7.677180000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Final Gesta'#195#167#195#163'o')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Left = 464.268090000000000000
          Top = 7.677180000000000000
          Width = 83.149660000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Inicio Gesta'#195#167#195#163'o')
          ParentFont = False
        end
        object Line21: TfrxLineView
          Left = 556.590910000000000000
          Top = 0.779530000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Line34: TfrxLineView
          Left = 340.433161180000000000
          Top = 0.755905510000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo2: TfrxMemoView
          Left = 343.937230000000000000
          Top = 7.559060000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Gestante')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Left = 400.850650000000000000
          Top = 0.779530000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo7: TfrxMemoView
          Left = 404.134248820000000000
          Top = 7.338590000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Meses')
          ParentFont = False
        end
        object Line4: TfrxLineView
          Left = 460.323130000000000000
          Top = 0.755905510000000000
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo8: TfrxMemoView
          Left = 1.779530000000000000
          Top = 7.559060000000000000
          Width = 30.236220470000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Nr.')
          ParentFont = False
        end
        object Line6: TfrxLineView
          Left = 34.015770000000000000
          Top = 0.755905511811023600
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo16: TfrxMemoView
          Left = 36.574830000000000000
          Top = 7.559060000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Nr. Processo')
          ParentFont = False
        end
        object Line9: TfrxLineView
          Left = 111.385900000000000000
          Top = 0.755905511811023600
          Height = 26.456710000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo18: TfrxMemoView
          Left = 113.385900000000000000
          Top = 7.559060000000000000
          Width = 222.992270000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Nome Paciente')
          ParentFont = False
        end
      end
    end
  end
  object fxdbGestantes: TfrxDBDataset
    UserName = 'Gestantes'
    CloseDataSource = False
    DataSource = dsGestantes
    BCDToCurrency = False
    Left = 264
    Top = 294
  end
  object fxdbUnidade: TfrxDBDataset
    UserName = 'Unidade'
    CloseDataSource = False
    DataSource = dsUnidade
    BCDToCurrency = False
    Left = 264
    Top = 270
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 100
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    CheckboxAsShape = False
    Left = 264
    Top = 320
  end
  object qyUnidade: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'SELECT'
      #9'DS_UNIDADE,'
      #9'DS_MUNICIPIO,'
      #9'DS_PROVINCIA'
      'FROM UNIDADE  '
      '  left outer join   PROVINCIA on'
      '  UNIDADE.CD_PROVINCIA    = PROVINCIA.CD_PROVINCIA'
      '  left outer join   MUNICIPIO on'
      '  UNIDADE.CD_MUNICIPIO  =  MUNICIPIO.CD_MUNICIPIO and'
      '  UNIDADE.CD_PROVINCIA =  MUNICIPIO.CD_PROVINCIA'
      'WHERE UNIDADE.CD_UNIDADE = :CD_UNIDADE')
    Left = 336
    Top = 233
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'CD_UNIDADE'
        ParamType = ptUnknown
      end>
    object qyUnidadeDS_UNIDADE: TIBStringField
      FieldName = 'DS_UNIDADE'
      Origin = 'UNIDADE.DS_UNIDADE'
      Required = True
      Size = 50
    end
    object qyUnidadeDS_MUNICIPIO: TIBStringField
      FieldName = 'DS_MUNICIPIO'
      Origin = 'MUNICIPIO.DS_MUNICIPIO'
      Size = 50
    end
    object qyUnidadeDS_PROVINCIA: TIBStringField
      FieldName = 'DS_PROVINCIA'
      Origin = 'PROVINCIA.DS_PROVINCIA'
      Size = 50
    end
  end
  object dsUnidade: TDataSource
    DataSet = qyUnidade
    Left = 368
    Top = 233
  end
end
