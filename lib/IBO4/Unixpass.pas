...
function CreateInterbasePassword( const aPassword: AnsiString ): AnsiString;
function UnixPassword( const aPassword,aSalt: AnsiString ): AnsiString;
...
function CreateInterbasePassword( const aPassword: AnsiString ): AnsiString;
begin
  Result := {ibo}Copy( UnixPassword( aPassword, '9z' ), 3, 99 );
  Result := {ibo}Copy( UnixPassword( Result,    '9z' ), 3, 99 );
end;
...
procedure TransposeBlock(   var aBlock: TBlock;
                          const aOrder: TBlock;
...
// rotate block data, basically moves all data in each half of the
// block to the left one spot
...
procedure TransformBlock( const aTotalPasses: Integer;
                          const aBlock,aNewTranspose: TBlock;
...
begin
  ...
  for i := 47 downto 0 do
    ...
    begin
        ...
        ShiftSpot := ShiftSpot {%H-}+ ShiftSelector.Data[6*i + 1] shl 3;
        ShiftSpot := ShiftSpot {%H-}+ ShiftSelector.Data[6*i + 2] shl 2;
        ShiftSpot := ShiftSpot {%H-}+ ShiftSelector.Data[6*i + 3] shl 1;
        ShiftSpot := ShiftSpot + ShiftSelector.Data[6*i + 4];
        ShiftSpot := ShiftSpot {%H-}+ ShiftSelector.Data[6*i + 5] shl 4;
...
begin
  TransposeBlock(aBlock, InitialTranspose1, 64);
    ...
    TransformBlock(TotalPasses, aBlock, aNewTranspose, aKey, TransformedBlock{%H-});
...
var
    ...
    i,j,pwIndex,npwIndex,total,spot,holder: Integer;
    OneChar: integer;
begin
    Result := '';
    if {ibo}Length(aSalt) <> 2 then Exit; // otherwise will blow up

    Result := '1234567890123'; // need 13 characters so we can use as array
    total := {ibo}Length(aPassword);
  ...
  for i := 1 to {ibo}Length(aSalt) do    // using salt to set up transpose key
    begin
        OneChar := Ord(aSalt[i]);
        Result[i] := ansichar( Chr( OneChar ));
    ...
    begin
        ...
        if OneChar > Ord('Z') then
            OneChar := OneChar + 6;
        Result[pwIndex + 1] := ansichar( Chr(OneChar));
        Inc(pwIndex);
    end;
