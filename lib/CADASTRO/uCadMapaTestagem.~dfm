inherited frmCadMapaTestagem: TfrmCadMapaTestagem
  Left = 89
  Top = 114
  Caption = 'Mapa de Testagem'
  ClientHeight = 570
  ClientWidth = 1235
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited sbPadrao: TStatusBar
    Top = 551
    Width = 1235
    Panels = <
      item
        Style = psOwnerDraw
        Width = 900
      end
      item
        Width = 300
      end
      item
        Width = 50
      end>
  end
  inherited Panel1: TPanel
    Width = 1235
    inherited btPesquisar: TBitBtn
      OnClick = btPesquisarClick
    end
    inherited btfechar: TBitBtn
      Left = 1187
    end
  end
  inherited Panel2: TPanel
    Width = 1235
    Height = 499
    inherited pgGeral: TPageControl
      Width = 1233
      Height = 497
      inherited tabLista: TTabSheet
        inherited Panel3: TPanel
          Width = 1225
          Height = 469
          inherited grDados: TdxDBGrid
            Width = 1223
            Height = 467
            KeyField = 'CD_MAPA_TESTAGEM'
            ParentFont = False
            BandColor = clHighlight
            Filter.Criteria = {00000000}
            GroupPanelColor = clHighlight
            HeaderFont.Style = [fsBold]
            OptionsBehavior = [edgoAutoSearch, edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoStoreToIniFile, edgoTabThrough, edgoVertThrough]
            object gcCD_MAPA_TESTAGEM: TdxDBGridColumn
              Caption = 'C'#243'digo'
              BandIndex = 0
              RowIndex = 0
              FieldName = 'CD_MAPA_TESTAGEM'
            end
            object gcDT_MAPA: TdxDBGridColumn
              Caption = 'Data'
              Width = 99
              BandIndex = 0
              RowIndex = 0
              FieldName = 'DT_MAPA'
            end
          end
        end
      end
      inherited tabGeral: TTabSheet
        inherited Panel4: TPanel
          Width = 1225
          Height = 469
          object Panel5: TPanel
            Left = 1
            Top = 1
            Width = 1223
            Height = 40
            Align = alTop
            TabOrder = 0
            object Label4: TLabel
              Left = 6
              Top = 12
              Width = 32
              Height = 13
              Caption = 'Data:'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object btConsultar: TBitBtn
              Left = 199
              Top = 4
              Width = 98
              Height = 28
              BiDiMode = bdRightToLeft
              Caption = 'Consultar'
              ParentBiDiMode = False
              TabOrder = 0
              Visible = False
              OnClick = btConsultarClick
              Glyph.Data = {
                
                5152949494969696A7A7A7A7A7A7525152525152525152525152}
              NumGlyphs = 2
            end
            object dtMapa: TDBDateEdit
              Left = 44
              Top = 9
              Width = 153
              Height = 21
              DataField = 'DT_MAPA'
              DataSource = dsControle
              DialogTitle = 'Data Mapa'
              NumGlyphs = 2
              TabOrder = 1
            end
          end
          object dxDBGrid1: TdxDBGrid
            Left = 1
            Top = 41
            Width = 1193
            Height = 427
            Bands = <
              item
                Caption = 'Registro'
              end
              item
                Caption = 'Utente'
                Width = 46
              end
              item
                Caption = 'Testagem'
                Width = 269
              end
              item
                Caption = 'Outros'
              end
              item
                Caption = 'Digitador'
              end>
            DefaultLayout = False
            HeaderPanelRowCount = 1
            KeyField = 'CD_MAPA_TESTAGEM_PACIENTE'
            SummaryGroups = <>
            SummarySeparator = ', '
            Align = alClient
            BorderStyle = bsNone
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 1
            BandColor = 8404992
            BandFont.Charset = DEFAULT_CHARSET
            BandFont.Color = clWhite
            BandFont.Height = -11
            BandFont.Name = 'MS Sans Serif'
            BandFont.Style = [fsBold]
            DataSource = dsMapaPaciente
            Filter.Criteria = {00000000}
            HeaderColor = 15983825
            HeaderFont.Charset = DEFAULT_CHARSET
            HeaderFont.Color = clWindowText
            HeaderFont.Height = -11
            HeaderFont.Name = 'MS Sans Serif'
            HeaderFont.Style = [fsBold]
            OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoStoreToIniFile, edgoTabs, edgoTabThrough, edgoVertThrough]
            OptionsDB = [edgoCanAppend, edgoCancelOnExit, edgoCanNavigation, edgoUseBookmarks]
            OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoUseBitmap]
            PreviewFont.Charset = DEFAULT_CHARSET
            PreviewFont.Color = clBlue
            PreviewFont.Height = -11
            PreviewFont.Name = 'MS Sans Serif'
            PreviewFont.Style = []
            ShowBands = True
            ShowRowFooter = True
            object gcCODIGO: TdxDBGridColumn
              Caption = 'C'#243'd.'
              DisableEditor = True
              HeaderAlignment = taCenter
              Width = 35
              BandIndex = 0
              RowIndex = 0
              FieldName = 'CODIGO'
            end
            object gcLIVRO: TdxDBGridColumn
              Caption = 'Livro'
              BandIndex = 0
              RowIndex = 0
              FieldName = 'NR_LIVRO'
            end
            object gcNR_REGISTRO: TdxDBGridColumn
              Caption = 'Nr.'
              HeaderAlignment = taCenter
              BandIndex = 0
              RowIndex = 0
              FieldName = 'NR_REGISTRO'
            end
            object gcDT_REGISTRO: TdxDBGridDateColumn
              Caption = 'Registro'
              Color = clScrollBar
              DisableEditor = True
              ReadOnly = True
              BandIndex = 0
              RowIndex = 0
              FieldName = 'DT_REGISTRO'
              UseEditMask = True
            end
            object gcNM_PACIENTE: TdxDBGridColumn
              Caption = 'Utente'
              HeaderAlignment = taCenter
              Width = 200
              BandIndex = 1
              RowIndex = 0
              FieldName = 'NM_PACIENTE'
            end
            object gcFL_SEXO: TdxDBGridPickColumn
              Caption = 'Sexo'
              Width = 34
              BandIndex = 1
              RowIndex = 0
              FieldName = 'FL_SEXO'
              Items.Strings = (
                'Masculino'
                'Feminino')
            end
            object gcNR_IDADE: TdxDBGridSpinColumn
              Caption = 'Idade'
              HeaderAlignment = taCenter
              BandIndex = 1
              RowIndex = 0
              FieldName = 'NR_IDADE'
              MaxValue = 150.000000000000000000
            end
            object gcNR_IDADE_MESES: TdxDBGridColumn
              Caption = 'Meses'
              Width = 50
              BandIndex = 1
              RowIndex = 0
              FieldName = 'NR_IDADE_MESES'
            end
            object gcFL_ACONSELHADO: TdxDBGridCheckColumn
              Alignment = taCenter
              Caption = 'Aconselhado'
              HeaderAlignment = taCenter
              Width = 80
              BandIndex = 1
              RowIndex = 0
              FieldName = 'FL_ACONSELHADO'
              ValueChecked = 'S'
              ValueGrayed = 'N'
              ValueUnchecked = 'N'
            end
            object gcFL_GESTANTE_NOVO: TdxDBGridCheckColumn
              Alignment = taCenter
              Caption = 'Gestante'
              HeaderAlignment = taCenter
              Width = 60
              BandIndex = 1
              RowIndex = 0
              FieldName = 'FL_GESTANTE_NOVO'
              ValueChecked = 'S'
              ValueGrayed = 'N'
              ValueUnchecked = 'N'
            end
            object gcFL_RESULTADO_NEGATIVO: TdxDBGridCheckColumn
              Alignment = taCenter
              Caption = 'Negativo'
              HeaderAlignment = taCenter
              Width = 72
              BandIndex = 2
              RowIndex = 0
              FieldName = 'FL_RESULTADO_NEGATIVO'
              ValueChecked = 'S'
              ValueGrayed = 'N'
              ValueUnchecked = 'N'
            end
            object gcFL_RESULTADO_POSITIVO: TdxDBGridCheckColumn
              Alignment = taCenter
              Caption = 'Positivo'
              HeaderAlignment = taCenter
              Width = 58
              BandIndex = 2
              RowIndex = 0
              FieldName = 'FL_RESULTADO_POSITIVO'
              ValueChecked = 'S'
              ValueGrayed = 'N'
              ValueUnchecked = 'N'
            end
            object gcFL_RESULTADO_INDETERMINADO: TdxDBGridCheckColumn
              Alignment = taCenter
              Caption = 'Indeterminado'
              HeaderAlignment = taCenter
              Width = 91
              BandIndex = 2
              RowIndex = 0
              FieldName = 'FL_RESULTADO_INDETERMINADO'
              ValueChecked = 'S'
              ValueGrayed = 'N'
              ValueUnchecked = 'N'
            end
            object gcNR_NOTIFICACAO: TdxDBGridButtonColumn
              Alignment = taCenter
              Caption = 'Not.'
              HeaderAlignment = taCenter
              Width = 55
              BandIndex = 2
              RowIndex = 0
              FieldName = 'NR_NOTIFICACAO'
              Buttons = <
                item
                  Default = True
                end>
              OnButtonClick = gcNR_NOTIFICACAOButtonClick
            end
            object gcDS_OBSERVACAO: TdxDBGridMemoColumn
              Caption = 'Observa'#231#227'o'
              HeaderAlignment = taCenter
              Width = 200
              BandIndex = 3
              RowIndex = 0
              FieldName = 'DS_OBSERVACAO'
            end
            object gcCD_PROVINCIA: TdxDBGridColumn
              Caption = 'C'#243'd.'
              DisableEditor = True
              Width = 35
              BandIndex = 3
              RowIndex = 0
              FieldName = 'CD_PROVINCIA'
            end
            object gcDS_PROVINCIA: TdxDBGridButtonColumn
              Caption = 'Provincia'
              DisableEditor = True
              HeaderAlignment = taCenter
              Width = 200
              BandIndex = 3
              RowIndex = 0
              FieldName = 'DS_PROVINCIA'
              Buttons = <
                item
                  Default = True
                  Glyph.Data = {
                    
                    CBCAE3F2F1F7FDF9FFFFF9FFFFFF}
                  Visible = False
                end>
              Glyph.Data = {
                
                CBCAE3F2F1F7FDF9FFFFF9FFFFFF}
              OnButtonClick = gcDS_PROVINCIAButtonClick
            end
            object gcCD_MUNICIPIO: TdxDBGridColumn
              Caption = 'C'#243'd.'
              DisableEditor = True
              Width = 35
              BandIndex = 3
              RowIndex = 0
              FieldName = 'CD_MUNICIPIO'
            end
            object gcDS_MUNICIPIO: TdxDBGridButtonColumn
              Caption = 'Munic'#237'pio'
              DisableEditor = True
              HeaderAlignment = taCenter
              Width = 200
              BandIndex = 3
              RowIndex = 0
              FieldName = 'DS_MUNICIPIO'
              Buttons = <
                item
                  Default = True
                  Glyph.Data = {
                    
                    CBCAE3F2F1F7FDF9FFFFF9FFFFFF}
                  Visible = False
                end>
              Glyph.Data = {
                
                CBCAE3F2F1F7FDF9FFFFF9FFFFFF}
              OnButtonClick = gcDS_MUNICIPIOButtonClick
            end
            object gcNM_INVESTIGADOR: TdxDBGridButtonColumn
              Caption = 'Investigador'
              HeaderAlignment = taCenter
              BandIndex = 3
              RowIndex = 0
              FieldName = 'NM_INVESTIGADOR'
              Buttons = <
                item
                  Default = True
                  Glyph.Data = {
                    
                    CBCAE3F2F1F7FDF9FFFFF9FFFFFF}
                  Kind = bkGlyph
                end>
              EditButtonStyle = ebsGlyph
              OnButtonClick = gcNM_INVESTIGADORButtonClick
            end
            object gcDS_LOCAL: TdxDBGridColumn
              Caption = 'Se'#231#227'o'
              HeaderAlignment = taCenter
              Width = 100
              BandIndex = 3
              RowIndex = 0
              FieldName = 'DS_LOCAL'
            end
            object gcDS_SALA: TdxDBGridColumn
              Caption = 'Sala'
              HeaderAlignment = taCenter
              Width = 100
              BandIndex = 3
              RowIndex = 0
              FieldName = 'DS_SALA'
            end
            object gcNM_DIGITADOR: TdxDBGridColumn
              Caption = 'Digitador'
              HeaderAlignment = taCenter
              BandIndex = 4
              RowIndex = 0
              FieldName = 'NM_DIGITADOR'
            end
            object gcDT_INGRESSO_DIG: TdxDBGridDateColumn
              Caption = 'Ingresso'
              HeaderAlignment = taCenter
              BandIndex = 4
              RowIndex = 0
              FieldName = 'DT_INGRESSO_DIG'
              UseEditMask = True
            end
            object gcDS_OBSERVACAO_DIG: TdxDBGridColumn
              Caption = 'Observa'#231#227'o'
              HeaderAlignment = taCenter
              Width = 200
              BandIndex = 4
              RowIndex = 0
              FieldName = 'DS_OBSERVACAO_DIG'
            end
          end
          object Panel6: TPanel
            Left = 1194
            Top = 41
            Width = 30
            Height = 427
            Align = alRight
            TabOrder = 2
            object btIns: TSpeedButton
              Left = 4
              Top = 8
              Width = 23
              Height = 22
              Hint = 'Incluir'
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              NumGlyphs = 2
              ParentShowHint = False
              ShowHint = True
              OnClick = btInsClick
            end
            object btDel: TSpeedButton
              Left = 4
              Top = 32
              Width = 23
              Height = 22
              Hint = 'Excluir'
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              NumGlyphs = 2
              ParentShowHint = False
              ShowHint = True
              OnClick = btDelClick
            end
          end
        end
      end
      object tabExport: TTabSheet
        Caption = 'Export'
        ImageIndex = 2
        DesignSize = (
          1225
          469)
        object grExport: TdxDBGrid
          Left = 0
          Top = 0
          Width = 1081
          Height = 469
          Bands = <
            item
              Caption = 'Registro'
              Width = 217
            end
            item
              Caption = 'Utente'
              Width = 46
            end
            item
              Caption = 'Testagem'
              Width = 269
            end
            item
              Caption = 'Outros'
              Width = 714
            end
            item
              Caption = 'Digitador'
            end>
          DefaultLayout = False
          HeaderPanelRowCount = 1
          KeyField = 'CD_MAPA_TESTAGEM_PACIENTE'
          ShowGroupPanel = True
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alClient
          BorderStyle = bsNone
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ParentShowHint = False
          PopupMenu = PopupMenu1
          ShowHint = True
          TabOrder = 0
          BandColor = 8404992
          BandFont.Charset = DEFAULT_CHARSET
          BandFont.Color = clWhite
          BandFont.Height = -11
          BandFont.Name = 'MS Sans Serif'
          BandFont.Style = [fsBold]
          DataSource = dsExport
          Filter.Active = True
          Filter.AutoDataSetFilter = True
          Filter.Criteria = {00000000}
          GroupPanelColor = 12615680
          HeaderColor = 15983825
          HeaderFont.Charset = DEFAULT_CHARSET
          HeaderFont.Color = clWindowText
          HeaderFont.Height = -11
          HeaderFont.Name = 'MS Sans Serif'
          HeaderFont.Style = [fsBold]
          OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoStoreToIniFile, edgoTabs, edgoTabThrough, edgoVertThrough]
          OptionsDB = [edgoCanAppend, edgoCancelOnExit, edgoCanDelete, edgoCanInsert, edgoCanNavigation, edgoUseBookmarks]
          OptionsView = [edgoBandHeaderWidth, edgoIndicator, edgoRowSelect, edgoUseBitmap]
          PreviewFont.Charset = DEFAULT_CHARSET
          PreviewFont.Color = clBlue
          PreviewFont.Height = -11
          PreviewFont.Name = 'MS Sans Serif'
          PreviewFont.Style = []
          ShowBands = True
          ShowRowFooter = True
          OnCustomDraw = grExportCustomDraw
          object grExportColumn22: TdxDBGridColumn
            Caption = 'Todos'
            Sorted = csUp
            Visible = False
            Width = 60
            BandIndex = 0
            RowIndex = 0
            FieldName = 'TODOS'
            GroupIndex = 0
          end
          object dxDBGridColumn1: TdxDBGridColumn
            Caption = 'C'#243'd.'
            DisableEditor = True
            HeaderAlignment = taCenter
            Width = 51
            BandIndex = 0
            RowIndex = 0
            FieldName = 'CD_MAPA_TESTAGEM_PACIENTE'
          end
          object dxDBGridColumn2: TdxDBGridColumn
            Caption = 'Livro'
            Width = 53
            BandIndex = 0
            RowIndex = 0
            FieldName = 'NR_LIVRO'
          end
          object grExportColumn27: TdxDBGridColumn
            Caption = 'Nr.'
            HeaderAlignment = taCenter
            Width = 40
            BandIndex = 0
            RowIndex = 0
            FieldName = 'NR_REGISTRO'
          end
          object dxDBGridDateColumn1: TdxDBGridDateColumn
            Caption = 'Registro'
            Width = 72
            BandIndex = 0
            RowIndex = 0
            FieldName = 'DT_REGISTRO'
          end
          object dxDBGridColumn3: TdxDBGridColumn
            Caption = 'Utente'
            HeaderAlignment = taCenter
            Width = 156
            BandIndex = 1
            RowIndex = 0
            FieldName = 'NM_PACIENTE'
          end
          object dxDBGridPickColumn1: TdxDBGridPickColumn
            Caption = 'Sexo'
            Width = 51
            BandIndex = 1
            RowIndex = 0
            FieldName = 'FL_SEXO'
            Items.Strings = (
              'Masculino'
              'Feminino')
          end
          object dxDBGridSpinColumn1: TdxDBGridSpinColumn
            Caption = 'Idade'
            HeaderAlignment = taCenter
            Width = 55
            BandIndex = 1
            RowIndex = 0
            FieldName = 'NR_IDADE'
            MaxValue = 150.000000000000000000
          end
          object dxDBGridColumn4: TdxDBGridColumn
            Caption = 'Meses'
            Width = 59
            BandIndex = 1
            RowIndex = 0
            FieldName = 'NR_IDADE_MESES'
          end
          object dxDBGridCheckColumn1: TdxDBGridCheckColumn
            Alignment = taCenter
            Caption = 'Aconselhado'
            HeaderAlignment = taCenter
            Width = 96
            BandIndex = 1
            RowIndex = 0
            FieldName = 'FL_ACONSELHADO'
            ValueChecked = 'S'
            ValueGrayed = 'N'
            ValueUnchecked = 'N'
          end
          object dxDBGridCheckColumn2: TdxDBGridCheckColumn
            Alignment = taCenter
            Caption = 'Gestante'
            HeaderAlignment = taCenter
            Width = 74
            BandIndex = 1
            RowIndex = 0
            FieldName = 'FL_GESTANTE_NOVO'
            ValueChecked = 'S'
            ValueGrayed = 'N'
            ValueUnchecked = 'N'
          end
          object dxDBGridCheckColumn3: TdxDBGridCheckColumn
            Alignment = taCenter
            Caption = 'Negativo'
            HeaderAlignment = taCenter
            Width = 74
            BandIndex = 2
            RowIndex = 0
            FieldName = 'FL_RESULTADO_NEGATIVO'
            ValueChecked = 'S'
            ValueGrayed = 'N'
            ValueUnchecked = 'N'
          end
          object dxDBGridCheckColumn4: TdxDBGridCheckColumn
            Alignment = taCenter
            Caption = 'Positivo'
            HeaderAlignment = taCenter
            Width = 68
            BandIndex = 2
            RowIndex = 0
            FieldName = 'FL_RESULTADO_POSITIVO'
            ValueChecked = 'S'
            ValueGrayed = 'N'
            ValueUnchecked = 'N'
          end
          object dxDBGridCheckColumn5: TdxDBGridCheckColumn
            Alignment = taCenter
            Caption = 'Indeterminado'
            HeaderAlignment = taCenter
            Width = 103
            BandIndex = 2
            RowIndex = 0
            FieldName = 'FL_RESULTADO_INDETERMINADO'
            ValueChecked = 'S'
            ValueGrayed = 'N'
            ValueUnchecked = 'N'
          end
          object dxDBGridButtonColumn1: TdxDBGridButtonColumn
            Alignment = taCenter
            Caption = 'Not.'
            HeaderAlignment = taCenter
            Width = 59
            BandIndex = 2
            RowIndex = 0
            FieldName = 'NR_NOTIFICACAO'
            Buttons = <
              item
                Default = True
              end>
            OnButtonClick = gcNR_NOTIFICACAOButtonClick
          end
          object dxDBGridMemoColumn1: TdxDBGridMemoColumn
            Caption = 'Observa'#231#227'o'
            HeaderAlignment = taCenter
            Width = 217
            BandIndex = 3
            RowIndex = 0
            FieldName = 'DS_OBSERVACAO'
          end
          object dxDBGridColumn5: TdxDBGridColumn
            Caption = 'C'#243'd.'
            Width = 53
            BandIndex = 3
            RowIndex = 0
            FieldName = 'CD_MUNICIPIO'
          end
          object dxDBGridButtonColumn2: TdxDBGridColumn
            Caption = 'Municipio'
            HeaderAlignment = taCenter
            ReadOnly = True
            Width = 217
            BandIndex = 3
            RowIndex = 0
            FieldName = 'DS_MUNICIPIO'
          end
          object grExportColumn23: TdxDBGridColumn
            Caption = 'C'#243'd.'
            Width = 53
            BandIndex = 3
            RowIndex = 0
            FieldName = 'CD_PROVINCIA'
          end
          object grExportColumn24: TdxDBGridColumn
            Caption = 'Provincia'
            HeaderAlignment = taCenter
            Width = 217
            BandIndex = 3
            RowIndex = 0
            FieldName = 'DS_PROVINCIA'
          end
          object dxDBGridColumn6: TdxDBGridColumn
            Caption = 'Investigador'
            HeaderAlignment = taCenter
            Width = 100
            BandIndex = 3
            RowIndex = 0
            FieldName = 'NM_INVESTIGADOR'
          end
          object dxDBGridColumn7: TdxDBGridColumn
            Caption = 'Se'#231#227'o'
            HeaderAlignment = taCenter
            Width = 100
            BandIndex = 3
            RowIndex = 0
            FieldName = 'DS_LOCAL'
          end
          object grExportColumn25: TdxDBGridColumn
            Caption = 'Sala'
            HeaderAlignment = taCenter
            Width = 100
            BandIndex = 3
            RowIndex = 0
            FieldName = 'DS_SALA'
          end
          object dxDBGridColumn8: TdxDBGridColumn
            Caption = 'Digitador'
            HeaderAlignment = taCenter
            Width = 100
            BandIndex = 4
            RowIndex = 0
            FieldName = 'NM_DIGITADOR'
          end
          object dxDBGridDateColumn2: TdxDBGridDateColumn
            Caption = 'Ingresso'
            HeaderAlignment = taCenter
            Width = 71
            BandIndex = 4
            RowIndex = 0
            FieldName = 'DT_INGRESSO_DIG'
          end
          object dxDBGridColumn9: TdxDBGridColumn
            Caption = 'Observa'#231#227'o'
            HeaderAlignment = taCenter
            Width = 200
            BandIndex = 4
            RowIndex = 0
            FieldName = 'DS_OBSERVACAO_DIG'
          end
        end
        object pnCampos: TPanel
          Left = 1081
          Top = 0
          Width = 144
          Height = 469
          Align = alRight
          TabOrder = 1
          Visible = False
          object ckCampos: TCheckListBox
            Left = 1
            Top = 1
            Width = 142
            Height = 467
            OnClickCheck = ckCamposClickCheck
            Align = alClient
            ItemHeight = 13
            TabOrder = 0
          end
        end
        object btCampos: TBitBtn
          Left = 1120
          Top = 4
          Width = 75
          Height = 20
          Anchors = [akTop, akRight]
          Caption = 'Campos'
          TabOrder = 2
          OnClick = btCamposClick
        end
      end
    end
  end
  inherited dsControle: TDataSource
    DataSet = TB_MAPA_TESTAGEM
    Left = 437
    Top = 205
  end
  inherited dsGrid: TDataSource
    DataSet = qyGrid
    Left = 397
    Top = 205
  end
  object qyGrid: TIBQuery [5]
    Database = DM.DB
    Transaction = DM.TS
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select '
      ' MAPA_TESTAGEM.CD_MAPA_TESTAGEM,  '
      ' MAPA_TESTAGEM.CD_MUNICIPIO, '
      ' MAPA_TESTAGEM.CD_PROVINCIA, '
      ' MAPA_TESTAGEM.CD_UNIDADE, '
      ' MAPA_TESTAGEM.DS_LOCAL, '
      ' MAPA_TESTAGEM.DT_MAPA'
      'from MAPA_TESTAGEM'
      '')
    Left = 354
    Top = 206
    object qyGridCD_MAPA_TESTAGEM: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM'
      Origin = 'MAPA_TESTAGEM.CD_MAPA_TESTAGEM'
      Required = True
    end
    object qyGridDT_MAPA: TDateField
      FieldName = 'DT_MAPA'
      Origin = 'MAPA_TESTAGEM.DT_MAPA'
    end
  end
  inherited PopupMenu1: TPopupMenu
    Left = 317
    Top = 213
    object N1: TMenuItem
      Caption = '-'
    end
    object Excel1: TMenuItem
      Caption = 'Excel'
      OnClick = Excel1Click
    end
    object Imprimir1: TMenuItem
      Caption = 'Imprimir'
      OnClick = Imprimir1Click
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object Editar1: TMenuItem
      Caption = 'Editar'
      OnClick = Editar1Click
    end
  end
  object dsMapaPaciente: TDataSource
    AutoEdit = False
    DataSet = TB_MAPA_TESTAGEM_PACIENTE
    OnStateChange = dsControleStateChange
    Left = 469
    Top = 197
  end
  object fxMapa: TfrxReport
    Version = '4.15.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 41894.334857743100000000
    ReportOptions.LastChange = 42606.495912523150000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 552
    Top = 224
    Datasets = <
      item
        DataSet = fxdbMapa
        DataSetName = 'Mapa'
      end
      item
        DataSet = fxdbPaciente
        DataSetName = 'Paciente'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      OnAfterPrint = 'Page1OnAfterPrint'
      object ReportTitle1: TfrxReportTitle
        Height = 200.315090000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Picture1: TfrxPictureView
          Align = baCenter
          Left = 478.090910000000000000
          Top = 3.779530000000000000
          Width = 90.747990000000000000
          Height = 62.629870000000000000
          ShowHint = False
          Center = True
          Picture.Data = {
            
            0000000049454E44AE426082}
          HightQuality = False
          Transparent = True
          TransparentColor = clWhite
        end
        object Memo1: TfrxMemoView
          Align = baCenter
          Left = 394.960885000000000000
          Top = 63.472480000000000000
          Width = 257.008040000000000000
          Height = 52.913420000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            '                Rep'#195#186'blica de Angola'
            '                 Minist'#195#169'rio da Sa'#195#186'de'
            'Instituto Nacional de Luta Contra o Sida')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Align = baCenter
          Left = 343.937230000000000000
          Top = 118.504020000000000000
          Width = 359.055350000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'REGISTO DI'#195#129'RIO DE TESTES R'#195#129'PIDOS')
          ParentFont = False
        end
        object Memo44: TfrxMemoView
          Left = 15.118120000000000000
          Top = 158.740260000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Munic'#195#173'pio')
          ParentFont = False
        end
        object Memo45: TfrxMemoView
          Left = 15.118120000000000000
          Top = 181.417440000000000000
          Width = 56.692950000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Prov'#195#173'ncia')
          ParentFont = False
        end
        object Memo46: TfrxMemoView
          Left = 264.567100000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Unidade')
          ParentFont = False
        end
        object Memo48: TfrxMemoView
          Left = 544.252320000000000000
          Top = 158.740260000000000000
          Width = 49.133890000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'Data')
          ParentFont = False
        end
        object Memo49: TfrxMemoView
          Left = 79.370130000000000000
          Top = 158.740260000000000000
          Width = 177.637910000000000000
          Height = 22.677180000000000000
          ShowHint = False
          DataField = 'DS_MUNICIPIO'
          DataSet = fxdbMapa
          DataSetName = 'Mapa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Mapa."DS_MUNICIPIO"]')
          ParentFont = False
        end
        object Memo50: TfrxMemoView
          Left = 79.370130000000000000
          Top = 181.417440000000000000
          Width = 173.858380000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_PROVINCIA'
          DataSet = fxdbMapa
          DataSetName = 'Mapa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Mapa."DS_PROVINCIA"]')
          ParentFont = False
        end
        object Memo51: TfrxMemoView
          Left = 317.480520000000000000
          Top = 158.740260000000000000
          Width = 207.874150000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DS_UNIDADE'
          DataSet = fxdbMapa
          DataSetName = 'Mapa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Mapa."DS_UNIDADE"]')
          ParentFont = False
        end
        object Memo53: TfrxMemoView
          Left = 597.165740000000000000
          Top = 158.740260000000000000
          Width = 86.929190000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DataField = 'DT_MAPA'
          DataSet = fxdbMapa
          DataSetName = 'Mapa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[Mapa."DT_MAPA"]')
          ParentFont = False
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 18.897650000000000000
        Top = 472.441250000000000000
        Width = 1046.929810000000000000
        object Shape3: TfrxShapeView
          Width = 1046.929810000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = 15790320
        end
        object Memo9: TfrxMemoView
          Left = 84.929190000000000000
          Top = 2.000000000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'dd/mm/yyyy 00:00'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[DATE]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 11.338590000000000000
          Top = 2.000000000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Data/Hora:')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 159.740260000000000000
          Top = 2.000000000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          ShowHint = False
          DisplayFormat.FormatStr = 'hh:mm am/pm'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[TIME]')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 1002.575450000000000000
          Top = 2.000000000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8 = (
            '[PAGE#]')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Left = 947.662030000000000000
          Top = 2.000000000000000000
          Width = 52.913420000000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8 = (
            'Pagina:')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        Height = 30.236240000000000000
        Top = 381.732530000000000000
        Width = 1046.929810000000000000
        DataSet = fxdbPaciente
        DataSetName = 'Paciente'
        RowCount = 0
        object Memo23: TfrxMemoView
          Width = 30.236218030000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'CODIGO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."CODIGO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo29: TfrxMemoView
          Left = 79.369998190000000000
          Width = 139.842519690000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'NM_PACIENTE'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."NM_PACIENTE"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo24: TfrxMemoView
          Left = 219.212481260000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'FL_SEXO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."FL_SEXO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo26: TfrxMemoView
          Left = 257.007734880000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'NR_IDADE_MESES'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."NR_IDADE_MESES"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo28: TfrxMemoView
          Left = 313.700660470000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'FL_RESULTADO_POSITIVO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."FL_RESULTADO_POSITIVO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          Left = 332.598286060000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'FL_RESULTADO_NEGATIVO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."FL_RESULTADO_NEGATIVO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo34: TfrxMemoView
          Left = 351.495911650000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'FL_RESULTADO_INDETERMINADO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."FL_RESULTADO_INDETERMINADO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo43: TfrxMemoView
          Left = 275.905372680000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'FL_ACONSELHADO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."FL_ACONSELHADO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo19: TfrxMemoView
          Left = 238.110097090000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'NR_IDADE'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."NR_IDADE"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo20: TfrxMemoView
          Left = 434.645557010000000000
          Width = 113.385826770000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'DS_OBSERVACAO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."DS_OBSERVACAO"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          Left = 548.031356930000000000
          Width = 120.944881890000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'NM_INVESTIGADOR'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."NM_INVESTIGADOR"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          Left = 294.803340000000000000
          Width = 18.897637800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'FL_GESTANTE_NOVO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."FL_GESTANTE_NOVO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          Left = 30.236240000000000000
          Width = 49.133860710000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."NR_LIVRO"]'
            '/ [Paciente."NR_REGISTRO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo41: TfrxMemoView
          Left = 668.976810000000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."DS_LOCAL"]'
            '/ [Paciente."DS_SALA"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo52: TfrxMemoView
          Left = 370.393940000000000000
          Width = 64.251997800000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'NR_NOTIFICACAO'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            '[Paciente."NR_NOTIFICACAO"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo58: TfrxMemoView
          Left = 725.669760000000000000
          Width = 120.944881890000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'NM_DIGITADOR'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."NM_DIGITADOR"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          Left = 846.614720000000000000
          Width = 60.472401890000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'DT_INGRESSO_DIG'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."DT_INGRESSO_DIG"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo37: TfrxMemoView
          Left = 907.087200000000000000
          Width = 139.842531890000000000
          Height = 30.236220470000000000
          ShowHint = False
          DataField = 'DS_OBSERVACAO_DIG'
          DataSet = fxdbPaciente
          DataSetName = 'Paciente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8 = (
            '[Paciente."DS_OBSERVACAO_DIG"]')
          ParagraphGap = 3.000000000000000000
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object PageHeader1: TfrxPageHeader
        Height = 80.370130000000000000
        Top = 241.889920000000000000
        Width = 1046.929810000000000000
        object Shape1: TfrxShapeView
          Left = 0.338590000000000000
          Top = 0.779530000000000000
          Width = 1046.929346220000000000
          Height = 79.370130000000000000
          ShowHint = False
          Color = 13750737
        end
        object Memo8: TfrxMemoView
          Left = 313.700670240000000000
          Top = 0.755905510000000000
          Width = 120.944886770000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Resultado do Teste')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo15: TfrxMemoView
          Left = 332.598295830000000000
          Top = 15.897650000000000000
          Width = 18.897637800000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Negativo')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo4: TfrxMemoView
          Left = 79.369998190000000000
          Top = 15.874015750000000000
          Width = 139.842519690000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Nome')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          Left = 219.212481260000000000
          Top = 15.874025510000000000
          Width = 18.897637800000000000
          Height = 64.251958740000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Sexo')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          Top = 0.677180000000000000
          Width = 30.236240000000000000
          Height = 79.370130000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8 = (
            'C'#195#179'd.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          Left = 313.700670240000000000
          Top = 15.874015750000000000
          Width = 18.897637800000000000
          Height = 64.251968500000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Positivo')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          Left = 294.803032440000000000
          Top = 15.874015750000000000
          Width = 18.897650000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Gestante')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo54: TfrxMemoView
          Left = 238.110119060000000000
          Top = 15.874025510000000000
          Width = 18.897637800000000000
          Height = 64.251958740000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Idade')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo55: TfrxMemoView
          Left = 275.905394650000000000
          Top = 16.015770000000000000
          Width = 18.897650000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Aconselhado')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          Left = 434.645557010000000000
          Top = 15.874015750000000000
          Width = 113.385826770000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Observa'#195#167#195#163'o')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo18: TfrxMemoView
          Left = 548.031356930000000000
          Top = 15.874015750000000000
          Width = 120.944960000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Investigador')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo6: TfrxMemoView
          Left = 351.495921420000000000
          Top = 15.874015750000000000
          Width = 18.897637800000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -8
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Indeterminado')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo16: TfrxMemoView
          Left = 30.236240000000000000
          Top = 15.874025510000000000
          Width = 49.133860710000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Livro'
            '/ Nr.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo27: TfrxMemoView
          Left = 30.236240000000000000
          Top = 0.755905510000000000
          Width = 49.133821650000000000
          Height = 15.118110240000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Registro')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          Left = 79.370130000000000000
          Top = 0.755905510000000000
          Width = 234.330708660000000000
          Height = 15.118110240000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Utente')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          Left = 257.007756850000000000
          Top = 15.874015750000000000
          Width = 18.897637800000000000
          Height = 64.251958740000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Meses')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo38: TfrxMemoView
          Left = 434.645950000000000000
          Top = 0.755905510000000000
          Width = 291.023622050000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Outros')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo39: TfrxMemoView
          Left = 668.976810000000000000
          Top = 15.874015750000000000
          Width = 56.692950000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Se'#195#167#195#163'o'
            '/ Sala')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo47: TfrxMemoView
          Left = 370.393940000000000000
          Top = 15.874015750000000000
          Width = 64.252010000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Notifica'#195#167#195#163'o')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo56: TfrxMemoView
          Left = 725.669760000000000000
          Top = 0.755905510000000000
          Width = 321.259935280000000000
          Height = 15.118120000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Digitador')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo57: TfrxMemoView
          Left = 725.669760000000000000
          Top = 15.874015750000000000
          Width = 120.944960000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Digitador')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo22: TfrxMemoView
          Left = 846.614720000000000000
          Top = 15.874015750000000000
          Width = 60.472480000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Ingresso')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          Left = 907.087200000000000000
          Top = 15.874015750000000000
          Width = 139.842610000000000000
          Height = 64.252010000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8 = (
            'Observa'#195#167#195#163'o')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object fxdbMapa: TfrxDBDataset
    UserName = 'Mapa'
    CloseDataSource = False
    DataSource = dsControle
    BCDToCurrency = False
    Left = 552
    Top = 254
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 95
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    CheckboxAsShape = False
    Left = 608
    Top = 272
  end
  object fxdbPaciente: TfrxDBDataset
    UserName = 'Paciente'
    CloseDataSource = False
    DataSource = dsMapaPaciente
    BCDToCurrency = False
    Left = 552
    Top = 286
  end
  object TB_MAPA_TESTAGEM: TIBDataSet
    Database = DM.DB
    Transaction = DM.TS
    AfterInsert = TB_MAPA_TESTAGEMAfterInsert
    AfterOpen = TB_MAPA_TESTAGEMAfterOpen
    BeforeClose = TB_MAPA_TESTAGEMBeforeClose
    BeforeDelete = TB_MAPA_TESTAGEMBeforeDelete
    BeforePost = TB_MAPA_TESTAGEMBeforePost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from "MAPA_TESTAGEM"'
      'where'
      '  "CD_MAPA_TESTAGEM" = :"OLD_CD_MAPA_TESTAGEM"')
    InsertSQL.Strings = (
      'insert into "MAPA_TESTAGEM"'
      '('
      '  "CD_MAPA_TESTAGEM"'
      ', "CD_PROVINCIA"'
      ', "CD_MUNICIPIO"'
      ', "CD_UNIDADE"'
      ', "DT_MAPA"'
      ', "DS_LOCAL"'
      ', "NR_LIVRO"'
      ')'
      'values'
      '('
      '  :"CD_MAPA_TESTAGEM"'
      ', :"CD_PROVINCIA"'
      ', :"CD_MUNICIPIO"'
      ', :"CD_UNIDADE"'
      ', :"DT_MAPA"'
      ', :"DS_LOCAL"'
      ', :"NR_LIVRO"'
      ')')
    SelectSQL.Strings = (
      'select '
      ' MAPA_TESTAGEM.CD_MAPA_TESTAGEM,  '
      ' MAPA_TESTAGEM.CD_MUNICIPIO, '
      ' MAPA_TESTAGEM.CD_PROVINCIA, '
      ' MAPA_TESTAGEM.CD_UNIDADE, '
      ' MAPA_TESTAGEM.DS_LOCAL, '
      ' MAPA_TESTAGEM.DT_MAPA,'
      ' MAPA_TESTAGEM.NR_LIVRO,'
      ' UNIDADE.DS_UNIDADE,'
      ' MUNICIPIO.DS_MUNICIPIO,'
      ' PROVINCIA.DS_PROVINCIA'
      'from MAPA_TESTAGEM'
      '  left outer join   UNIDADE on'
      '  MAPA_TESTAGEM.CD_MUNICIPIO  =  UNIDADE.CD_MUNICIPIO and'
      '  MAPA_TESTAGEM.CD_PROVINCIA =  UNIDADE.CD_PROVINCIA and'
      '  MAPA_TESTAGEM.CD_UNIDADE    =  UNIDADE.CD_UNIDADE  '
      '  left outer join   PROVINCIA on'
      '  MAPA_TESTAGEM.CD_PROVINCIA    = PROVINCIA.CD_PROVINCIA'
      '  left outer join   MUNICIPIO on'
      '  MAPA_TESTAGEM.CD_MUNICIPIO  =  MUNICIPIO.CD_MUNICIPIO and'
      '  MAPA_TESTAGEM.CD_PROVINCIA =  MUNICIPIO.CD_PROVINCIA '
      'where'
      '       MAPA_TESTAGEM.DT_MAPA = :DT_MAPA')
    ModifySQL.Strings = (
      'update "MAPA_TESTAGEM"'
      'set'
      ' "CD_MAPA_TESTAGEM" = :"CD_MAPA_TESTAGEM"'
      ', "CD_PROVINCIA" = :"CD_PROVINCIA"'
      ', "CD_MUNICIPIO" = :"CD_MUNICIPIO"'
      ', "CD_UNIDADE" = :"CD_UNIDADE"'
      ', "DT_MAPA" = :"DT_MAPA"'
      ', "DS_LOCAL" = :"DS_LOCAL"'
      ', "NR_LIVRO" = :"NR_LIVRO"'
      'where'
      '  "CD_MAPA_TESTAGEM" = :"OLD_CD_MAPA_TESTAGEM"')
    GeneratorField.Field = 'CD_MAPA_TESTAGEM'
    GeneratorField.Generator = 'GEN_MAPA_TESTAGEM'
    Left = 1048
    Top = 235
    object TB_MAPA_TESTAGEMCD_MAPA_TESTAGEM: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM'
      Origin = 'MAPA_TESTAGEM.CD_MAPA_TESTAGEM'
      Required = True
    end
    object TB_MAPA_TESTAGEMCD_MUNICIPIO: TIntegerField
      FieldName = 'CD_MUNICIPIO'
      Origin = 'MAPA_TESTAGEM.CD_MUNICIPIO'
      Required = True
    end
    object TB_MAPA_TESTAGEMCD_PROVINCIA: TIntegerField
      FieldName = 'CD_PROVINCIA'
      Origin = 'MAPA_TESTAGEM.CD_PROVINCIA'
      Required = True
    end
    object TB_MAPA_TESTAGEMCD_UNIDADE: TIntegerField
      FieldName = 'CD_UNIDADE'
      Origin = 'MAPA_TESTAGEM.CD_UNIDADE'
      Required = True
    end
    object TB_MAPA_TESTAGEMDS_LOCAL: TIBStringField
      FieldName = 'DS_LOCAL'
      Origin = 'MAPA_TESTAGEM.DS_LOCAL'
      Size = 30
    end
    object TB_MAPA_TESTAGEMDT_MAPA: TDateField
      FieldName = 'DT_MAPA'
      Origin = 'MAPA_TESTAGEM.DT_MAPA'
      OnValidate = TB_MAPA_TESTAGEMDT_MAPAValidate
    end
    object TB_MAPA_TESTAGEMDS_UNIDADE: TIBStringField
      FieldName = 'DS_UNIDADE'
      Origin = 'UNIDADE.DS_UNIDADE'
      Size = 50
    end
    object TB_MAPA_TESTAGEMDS_MUNICIPIO: TIBStringField
      FieldName = 'DS_MUNICIPIO'
      Origin = 'MUNICIPIO.DS_MUNICIPIO'
      Size = 50
    end
    object TB_MAPA_TESTAGEMDS_PROVINCIA: TIBStringField
      FieldName = 'DS_PROVINCIA'
      Origin = 'PROVINCIA.DS_PROVINCIA'
      Size = 50
    end
    object TB_MAPA_TESTAGEMNR_LIVRO: TIBStringField
      FieldName = 'NR_LIVRO'
      Origin = 'MAPA_TESTAGEM.NR_LIVRO'
      Size = 15
    end
  end
  object TB_MAPA_TESTAGEM_PACIENTE: TIBDataSet
    Database = DM.DB
    Transaction = DM.TS
    AfterInsert = TB_MAPA_TESTAGEM_PACIENTEAfterInsert
    BeforePost = TB_MAPA_TESTAGEM_PACIENTEBeforePost
    OnCalcFields = TB_MAPA_TESTAGEM_PACIENTECalcFields
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from "MAPA_TESTAGEM_PACIENTE"'
      'where'
      '  "CD_MAPA_TESTAGEM_PACIENTE" = :"OLD_CD_MAPA_TESTAGEM_PACIENTE"')
    InsertSQL.Strings = (
      'insert into "MAPA_TESTAGEM_PACIENTE"'
      '('
      '  "CD_MAPA_TESTAGEM_PACIENTE"'
      ', "CD_MAPA_TESTAGEM"'
      ', "NM_PACIENTE"'
      ', "FL_SEXO"'
      ', "NR_IDADE"'
      ', "FL_ACONSELHADO"'
      ', "FL_GESTANTE_NOVO"'
      ', "FL_GESTANTE_RETORNO"'
      ', "FL_RESULTADO_NEGATIVO"'
      ', "FL_RESULTADO_POSITIVO"'
      ', "FL_RESULTADO_INDETERMINADO"'
      ', "DS_OBSERVACAO"'
      ', "CD_MEDICO"'
      ', "CD_NOTIFICACAO"'
      ', "NR_IDADE_MESES"'
      ', "NR_LIVRO"'
      ', "DT_REGISTRO"'
      ', "NR_REGISTRO"'
      ', "NM_INVESTIGADOR"'
      ', "NM_DIGITADOR"'
      ', "DT_INGRESSO_DIG"'
      ', "DS_OBSERVACAO_DIG"'
      ', "DS_LOCAL"'
      ', "CD_PROVINCIA"'
      ', "CD_MUNICIPIO"'
      ', "DS_BAIRRO"'
      ', "DS_SALA"'
      ')'
      'values'
      '('
      '  :"CD_MAPA_TESTAGEM_PACIENTE"'
      ', :"CD_MAPA_TESTAGEM"'
      ', :"NM_PACIENTE"'
      ', :"FL_SEXO"'
      ', :"NR_IDADE"'
      ', :"FL_ACONSELHADO"'
      ', :"FL_GESTANTE_NOVO"'
      ', :"FL_GESTANTE_RETORNO"'
      ', :"FL_RESULTADO_NEGATIVO"'
      ', :"FL_RESULTADO_POSITIVO"'
      ', :"FL_RESULTADO_INDETERMINADO"'
      ', :"DS_OBSERVACAO"'
      ', :"CD_MEDICO"'
      ', :"CD_NOTIFICACAO"'
      ', :"NR_IDADE_MESES"'
      ', :"NR_LIVRO"'
      ', :"DT_REGISTRO"'
      ', :"NR_REGISTRO"'
      ', :"NM_INVESTIGADOR"'
      ', :"NM_DIGITADOR"'
      ', :"DT_INGRESSO_DIG"'
      ', :"DS_OBSERVACAO_DIG"'
      ', :"DS_LOCAL"'
      ', :"CD_PROVINCIA"'
      ', :"CD_MUNICIPIO"'
      ', :"DS_BAIRRO"'
      ', :"DS_SALA"'
      ')')
    SelectSQL.Strings = (
      'select '
      ' MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM, '
      ' MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM_PACIENTE, '
      ' MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_ACONSELHADO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_NOVO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_RETORNO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_INDETERMINADO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_NEGATIVO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_POSITIVO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_SEXO, '
      ' MAPA_TESTAGEM_PACIENTE.NM_PACIENTE, '
      ' MAPA_TESTAGEM_PACIENTE.NR_IDADE, '
      ' MAPA_TESTAGEM_PACIENTE.NR_IDADE_MESES,'
      ' MAPA_TESTAGEM_PACIENTE.CD_MEDICO,'
      ' MAPA_TESTAGEM_PACIENTE.CD_NOTIFICACAO,'
      ' MAPA_TESTAGEM_PACIENTE.NR_LIVRO,'
      ' MAPA_TESTAGEM_PACIENTE.DT_REGISTRO,'
      ' MAPA_TESTAGEM_PACIENTE.NR_REGISTRO,'
      ' MAPA_TESTAGEM_PACIENTE.NM_INVESTIGADOR,'
      ' MAPA_TESTAGEM_PACIENTE.NM_DIGITADOR,'
      ' MAPA_TESTAGEM_PACIENTE.DT_INGRESSO_DIG,'
      ' MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO_DIG,'
      ' MAPA_TESTAGEM_PACIENTE.DS_LOCAL,'
      ' MAPA_TESTAGEM_PACIENTE.CD_PROVINCIA,'
      ' MAPA_TESTAGEM_PACIENTE.CD_MUNICIPIO,'
      ' MAPA_TESTAGEM_PACIENTE.DS_BAIRRO,'
      ' MAPA_TESTAGEM_PACIENTE.DS_SALA,'
      ' PROVINCIA.DS_PROVINCIA,'
      ' MUNICIPIO.DS_MUNICIPIO,'
      '(select NR_NOTIFICACAO from NOTIFICACAO'
      
        'where NOTIFICACAO.CD_FICHA = MAPA_TESTAGEM_PACIENTE.CD_NOTIFICAC' +
        'AO) NR_NOTIFICACAO '
      'from MAPA_TESTAGEM_PACIENTE'
      '  left outer join PROVINCIA  '
      
        '  on (PROVINCIA.CD_PROVINCIA = MAPA_TESTAGEM_PACIENTE.CD_PROVINC' +
        'IA)'
      '  left outer join MUNICIPIO'
      
        '  on (MUNICIPIO.CD_MUNICIPIO = MAPA_TESTAGEM_PACIENTE.CD_MUNICIP' +
        'IO)'
      'where '
      'MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM = :CD_MAPA_TESTAGEM')
    ModifySQL.Strings = (
      'update "MAPA_TESTAGEM_PACIENTE"'
      'set'
      ' "CD_MAPA_TESTAGEM_PACIENTE" = :"CD_MAPA_TESTAGEM_PACIENTE"'
      ', "CD_MAPA_TESTAGEM" = :"CD_MAPA_TESTAGEM"'
      ', "NM_PACIENTE" = :"NM_PACIENTE"'
      ', "FL_SEXO" = :"FL_SEXO"'
      ', "NR_IDADE" = :"NR_IDADE"'
      ', "FL_ACONSELHADO" = :"FL_ACONSELHADO"'
      ', "FL_GESTANTE_NOVO" = :"FL_GESTANTE_NOVO"'
      ', "FL_GESTANTE_RETORNO" = :"FL_GESTANTE_RETORNO"'
      ', "FL_RESULTADO_NEGATIVO" = :"FL_RESULTADO_NEGATIVO"'
      ', "FL_RESULTADO_POSITIVO" = :"FL_RESULTADO_POSITIVO"'
      ', "FL_RESULTADO_INDETERMINADO" = :"FL_RESULTADO_INDETERMINADO"'
      ', "DS_OBSERVACAO" = :"DS_OBSERVACAO"'
      ', "CD_MEDICO" = :"CD_MEDICO"'
      ', "CD_NOTIFICACAO" = :"CD_NOTIFICACAO"'
      ', "NR_IDADE_MESES" = :"NR_IDADE_MESES"'
      ', "NR_LIVRO" = :"NR_LIVRO"'
      ', "DT_REGISTRO" = :"DT_REGISTRO"'
      ', "NR_REGISTRO" = :"NR_REGISTRO"'
      ', "NM_INVESTIGADOR" = :"NM_INVESTIGADOR"'
      ', "NM_DIGITADOR" = :"NM_DIGITADOR"'
      ', "DT_INGRESSO_DIG" = :"DT_INGRESSO_DIG"'
      ', "DS_OBSERVACAO_DIG" = :"DS_OBSERVACAO_DIG"'
      ', "DS_LOCAL" = :"DS_LOCAL"'
      ', "CD_PROVINCIA" = :"CD_PROVINCIA"'
      ', "CD_MUNICIPIO" = :"CD_MUNICIPIO"'
      ', "DS_BAIRRO" = :"DS_BAIRRO"'
      ', "DS_SALA" = :"DS_SALA"'
      'where'
      '  "CD_MAPA_TESTAGEM_PACIENTE" = :"OLD_CD_MAPA_TESTAGEM_PACIENTE"')
    GeneratorField.Field = 'CD_MAPA_TESTAGEM_PACIENTE'
    GeneratorField.Generator = 'GEN_MAPA_PACIENTE'
    DataSource = dsMapaTestagem
    Left = 1048
    Top = 267
    object TB_MAPA_TESTAGEM_PACIENTECODIGO: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'CODIGO'
      Calculated = True
    end
    object TB_MAPA_TESTAGEM_PACIENTECD_MAPA_TESTAGEM: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM'
      Required = True
    end
    object TB_MAPA_TESTAGEM_PACIENTECD_MAPA_TESTAGEM_PACIENTE: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM_PACIENTE'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM_PACIENTE'
      Required = True
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_OBSERVACAO: TIBStringField
      FieldName = 'DS_OBSERVACAO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO'
      Size = 200
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_ACONSELHADO: TIBStringField
      FieldName = 'FL_ACONSELHADO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_ACONSELHADO'
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_GESTANTE_NOVO: TIBStringField
      FieldName = 'FL_GESTANTE_NOVO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_NOVO'
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_RESULTADO_INDETERMINADO: TIBStringField
      FieldName = 'FL_RESULTADO_INDETERMINADO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_INDETERMINADO'
      OnValidate = TB_MAPA_TESTAGEM_PACIENTEFL_RESULTADO_INDETERMINADOValidate
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_RESULTADO_NEGATIVO: TIBStringField
      FieldName = 'FL_RESULTADO_NEGATIVO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_NEGATIVO'
      OnValidate = TB_MAPA_TESTAGEM_PACIENTEFL_RESULTADO_INDETERMINADOValidate
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_RESULTADO_POSITIVO: TIBStringField
      FieldName = 'FL_RESULTADO_POSITIVO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_POSITIVO'
      OnValidate = TB_MAPA_TESTAGEM_PACIENTEFL_RESULTADO_INDETERMINADOValidate
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_SEXO'
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTENM_PACIENTE: TIBStringField
      FieldName = 'NM_PACIENTE'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NM_PACIENTE'
      Required = True
      Size = 100
    end
    object TB_MAPA_TESTAGEM_PACIENTENR_IDADE: TIntegerField
      FieldName = 'NR_IDADE'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_IDADE'
    end
    object TB_MAPA_TESTAGEM_PACIENTECD_MEDICO: TIntegerField
      FieldName = 'CD_MEDICO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MEDICO'
    end
    object TB_MAPA_TESTAGEM_PACIENTECD_NOTIFICACAO: TIntegerField
      FieldName = 'CD_NOTIFICACAO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_NOTIFICACAO'
      OnValidate = TB_MAPA_TESTAGEM_PACIENTECD_NOTIFICACAOValidate
    end
    object TB_MAPA_TESTAGEM_PACIENTENR_NOTIFICACAO: TIBStringField
      FieldName = 'NR_NOTIFICACAO'
      OnValidate = TB_MAPA_TESTAGEM_PACIENTENR_NOTIFICACAOValidate
      Size = 10
    end
    object TB_MAPA_TESTAGEM_PACIENTENR_IDADE_MESES: TIntegerField
      FieldName = 'NR_IDADE_MESES'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_IDADE_MESES'
    end
    object TB_MAPA_TESTAGEM_PACIENTEFL_GESTANTE_RETORNO: TIBStringField
      FieldName = 'FL_GESTANTE_RETORNO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_RETORNO'
      FixedChar = True
      Size = 1
    end
    object TB_MAPA_TESTAGEM_PACIENTENR_LIVRO: TIBStringField
      FieldName = 'NR_LIVRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_LIVRO'
      Size = 15
    end
    object TB_MAPA_TESTAGEM_PACIENTEDT_REGISTRO: TDateField
      FieldName = 'DT_REGISTRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DT_REGISTRO'
    end
    object TB_MAPA_TESTAGEM_PACIENTENM_INVESTIGADOR: TIBStringField
      FieldName = 'NM_INVESTIGADOR'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NM_INVESTIGADOR'
      Size = 30
    end
    object TB_MAPA_TESTAGEM_PACIENTENM_DIGITADOR: TIBStringField
      FieldName = 'NM_DIGITADOR'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NM_DIGITADOR'
      Size = 30
    end
    object TB_MAPA_TESTAGEM_PACIENTEDT_INGRESSO_DIG: TDateField
      FieldName = 'DT_INGRESSO_DIG'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DT_INGRESSO_DIG'
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_OBSERVACAO_DIG: TIBStringField
      FieldName = 'DS_OBSERVACAO_DIG'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO_DIG'
      Size = 200
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_LOCAL: TIBStringField
      FieldName = 'DS_LOCAL'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_LOCAL'
      Size = 30
    end
    object TB_MAPA_TESTAGEM_PACIENTECD_PROVINCIA: TIntegerField
      FieldName = 'CD_PROVINCIA'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_PROVINCIA'
      ReadOnly = True
      OnValidate = TB_MAPA_TESTAGEM_PACIENTECD_PROVINCIAValidate
    end
    object TB_MAPA_TESTAGEM_PACIENTECD_MUNICIPIO: TIntegerField
      FieldName = 'CD_MUNICIPIO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MUNICIPIO'
      ReadOnly = True
      OnValidate = TB_MAPA_TESTAGEM_PACIENTECD_MUNICIPIOValidate
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_BAIRRO: TIBStringField
      FieldName = 'DS_BAIRRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_BAIRRO'
      Size = 100
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_SALA: TIBStringField
      FieldName = 'DS_SALA'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_SALA'
      Size = 100
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_PROVINCIA: TIBStringField
      FieldName = 'DS_PROVINCIA'
      Origin = 'PROVINCIA.DS_PROVINCIA'
      ReadOnly = True
      Size = 50
    end
    object TB_MAPA_TESTAGEM_PACIENTEDS_MUNICIPIO: TIBStringField
      FieldName = 'DS_MUNICIPIO'
      Origin = 'MUNICIPIO.DS_MUNICIPIO'
      ReadOnly = True
      Size = 50
    end
    object TB_MAPA_TESTAGEM_PACIENTENR_REGISTRO: TIntegerField
      FieldName = 'NR_REGISTRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_REGISTRO'
    end
  end
  object dsMapaTestagem: TDataSource
    AutoEdit = False
    DataSet = TB_MAPA_TESTAGEM
    Left = 1048
    Top = 300
  end
  object qyExport: TIBQuery
    Database = DM.DB
    Transaction = DM.TS
    AfterScroll = qyExportAfterScroll
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select '
      ' '#39'TODOS'#39' TODOS,'
      ' MAPA_TESTAGEM.CD_MAPA_TESTAGEM,  '
      ' MAPA_TESTAGEM.DT_MAPA,'
      ' MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM, '
      ' MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM_PACIENTE, '
      ' MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_ACONSELHADO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_NOVO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_RETORNO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_INDETERMINADO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_NEGATIVO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_POSITIVO, '
      ' MAPA_TESTAGEM_PACIENTE.FL_SEXO, '
      ' MAPA_TESTAGEM_PACIENTE.NM_PACIENTE, '
      ' MAPA_TESTAGEM_PACIENTE.NR_IDADE, '
      ' MAPA_TESTAGEM_PACIENTE.NR_IDADE_MESES,'
      ' MAPA_TESTAGEM_PACIENTE.CD_MEDICO,'
      ' MAPA_TESTAGEM_PACIENTE.CD_NOTIFICACAO,'
      ' MAPA_TESTAGEM_PACIENTE.NR_LIVRO,'
      ' MAPA_TESTAGEM_PACIENTE.DT_REGISTRO,'
      ' MAPA_TESTAGEM_PACIENTE.NR_REGISTRO,'
      ' MAPA_TESTAGEM_PACIENTE.NM_INVESTIGADOR,'
      ' MAPA_TESTAGEM_PACIENTE.NM_DIGITADOR,'
      ' MAPA_TESTAGEM_PACIENTE.DT_INGRESSO_DIG,'
      ' MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO_DIG,'
      ' MAPA_TESTAGEM_PACIENTE.DS_LOCAL,'
      ' MAPA_TESTAGEM_PACIENTE.CD_PROVINCIA,'
      ' MAPA_TESTAGEM_PACIENTE.CD_MUNICIPIO,'
      ' MAPA_TESTAGEM_PACIENTE.DS_BAIRRO,'
      ' MAPA_TESTAGEM_PACIENTE.DS_SALA,'
      ' PROVINCIA.DS_PROVINCIA,'
      ' MUNICIPIO.DS_MUNICIPIO,'
      '(select NR_NOTIFICACAO from NOTIFICACAO'
      
        'where NOTIFICACAO.CD_FICHA = MAPA_TESTAGEM_PACIENTE.CD_NOTIFICAC' +
        'AO) NR_NOTIFICACAO '
      'from MAPA_TESTAGEM_PACIENTE'
      '  inner join MAPA_TESTAGEM'
      
        '  on MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM = MAPA_TESTAGEM.CD_' +
        'MAPA_TESTAGEM'
      '  left outer join PROVINCIA  '
      
        '  on (PROVINCIA.CD_PROVINCIA = MAPA_TESTAGEM_PACIENTE.CD_PROVINC' +
        'IA)'
      '  left outer join MUNICIPIO'
      
        '  on (MUNICIPIO.CD_MUNICIPIO = MAPA_TESTAGEM_PACIENTE.CD_MUNICIP' +
        'IO)'
      ''
      '')
    Left = 354
    Top = 286
    object qyExportTODOS: TIBStringField
      FieldName = 'TODOS'
      Required = True
      FixedChar = True
      Size = 5
    end
    object qyExportCD_MAPA_TESTAGEM: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM'
      Origin = 'MAPA_TESTAGEM.CD_MAPA_TESTAGEM'
      Required = True
    end
    object qyExportDT_MAPA: TDateField
      FieldName = 'DT_MAPA'
      Origin = 'MAPA_TESTAGEM.DT_MAPA'
    end
    object qyExportCD_MAPA_TESTAGEM1: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM1'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM'
      Required = True
    end
    object qyExportCD_MAPA_TESTAGEM_PACIENTE: TIntegerField
      FieldName = 'CD_MAPA_TESTAGEM_PACIENTE'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MAPA_TESTAGEM_PACIENTE'
      Required = True
    end
    object qyExportDS_OBSERVACAO: TIBStringField
      FieldName = 'DS_OBSERVACAO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO'
      Size = 200
    end
    object qyExportFL_ACONSELHADO: TIBStringField
      FieldName = 'FL_ACONSELHADO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_ACONSELHADO'
      FixedChar = True
      Size = 1
    end
    object qyExportFL_GESTANTE_NOVO: TIBStringField
      FieldName = 'FL_GESTANTE_NOVO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_NOVO'
      FixedChar = True
      Size = 1
    end
    object qyExportFL_GESTANTE_RETORNO: TIBStringField
      FieldName = 'FL_GESTANTE_RETORNO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_GESTANTE_RETORNO'
      FixedChar = True
      Size = 1
    end
    object qyExportFL_RESULTADO_INDETERMINADO: TIBStringField
      FieldName = 'FL_RESULTADO_INDETERMINADO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_INDETERMINADO'
      FixedChar = True
      Size = 1
    end
    object qyExportFL_RESULTADO_NEGATIVO: TIBStringField
      FieldName = 'FL_RESULTADO_NEGATIVO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_NEGATIVO'
      FixedChar = True
      Size = 1
    end
    object qyExportFL_RESULTADO_POSITIVO: TIBStringField
      FieldName = 'FL_RESULTADO_POSITIVO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_RESULTADO_POSITIVO'
      FixedChar = True
      Size = 1
    end
    object qyExportFL_SEXO: TIBStringField
      FieldName = 'FL_SEXO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.FL_SEXO'
      FixedChar = True
      Size = 1
    end
    object qyExportNM_PACIENTE: TIBStringField
      FieldName = 'NM_PACIENTE'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NM_PACIENTE'
      Required = True
      Size = 100
    end
    object qyExportNR_IDADE: TIntegerField
      FieldName = 'NR_IDADE'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_IDADE'
    end
    object qyExportNR_IDADE_MESES: TIntegerField
      FieldName = 'NR_IDADE_MESES'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_IDADE_MESES'
    end
    object qyExportCD_MEDICO: TIntegerField
      FieldName = 'CD_MEDICO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MEDICO'
    end
    object qyExportCD_NOTIFICACAO: TIntegerField
      FieldName = 'CD_NOTIFICACAO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_NOTIFICACAO'
    end
    object qyExportNR_LIVRO: TIBStringField
      FieldName = 'NR_LIVRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_LIVRO'
      Size = 15
    end
    object qyExportDT_REGISTRO: TDateField
      FieldName = 'DT_REGISTRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DT_REGISTRO'
    end
    object qyExportNR_REGISTRO: TIntegerField
      FieldName = 'NR_REGISTRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NR_REGISTRO'
    end
    object qyExportNM_INVESTIGADOR: TIBStringField
      FieldName = 'NM_INVESTIGADOR'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NM_INVESTIGADOR'
      Size = 30
    end
    object qyExportNM_DIGITADOR: TIBStringField
      FieldName = 'NM_DIGITADOR'
      Origin = 'MAPA_TESTAGEM_PACIENTE.NM_DIGITADOR'
      Size = 30
    end
    object qyExportDT_INGRESSO_DIG: TDateField
      FieldName = 'DT_INGRESSO_DIG'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DT_INGRESSO_DIG'
    end
    object qyExportDS_OBSERVACAO_DIG: TIBStringField
      FieldName = 'DS_OBSERVACAO_DIG'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_OBSERVACAO_DIG'
      Size = 200
    end
    object qyExportDS_LOCAL: TIBStringField
      FieldName = 'DS_LOCAL'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_LOCAL'
      Size = 30
    end
    object qyExportCD_PROVINCIA: TIntegerField
      FieldName = 'CD_PROVINCIA'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_PROVINCIA'
    end
    object qyExportCD_MUNICIPIO: TIntegerField
      FieldName = 'CD_MUNICIPIO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.CD_MUNICIPIO'
    end
    object qyExportDS_BAIRRO: TIBStringField
      FieldName = 'DS_BAIRRO'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_BAIRRO'
      Size = 100
    end
    object qyExportDS_SALA: TIBStringField
      FieldName = 'DS_SALA'
      Origin = 'MAPA_TESTAGEM_PACIENTE.DS_SALA'
      Size = 100
    end
    object qyExportDS_PROVINCIA: TIBStringField
      FieldName = 'DS_PROVINCIA'
      Origin = 'PROVINCIA.DS_PROVINCIA'
      Size = 50
    end
    object qyExportDS_MUNICIPIO: TIBStringField
      FieldName = 'DS_MUNICIPIO'
      Origin = 'MUNICIPIO.DS_MUNICIPIO'
      Size = 50
    end
    object qyExportNR_NOTIFICACAO: TIBStringField
      FieldName = 'NR_NOTIFICACAO'
      Size = 10
    end
  end
  object dsExport: TDataSource
    AutoEdit = False
    DataSet = qyExport
    Left = 389
    Top = 285
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = '*.xls'
    Filter = 'Excel|*.xls'
    Left = 280
    Top = 192
  end
end
