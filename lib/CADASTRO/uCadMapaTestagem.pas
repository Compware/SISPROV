...
procedure TfrmCadMapaTestagem.gcDS_PROVINCIAButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{ VG - 18/08/2016 - Os dados da provincia vem automatico quando a linha é inserida e não pode ser editado
  inherited;
...
procedure TfrmCadMapaTestagem.gcDS_MUNICIPIOButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
{ VG - 18/08/2016 - Os dados do municipio vem automatico quando a linha é inserida e não pode ser editado
  inherited;
  if not btGravar.Enabled then Exit;
...
procedure TfrmCadMapaTestagem.TB_MAPA_TESTAGEM_PACIENTECD_PROVINCIAValidate(
  Sender: TField);
begin
{ VG - 18/08/2016 - Dado vem automatico quando a linha é inserida
  inherited;
  TB_MAPA_TESTAGEM_PACIENTEDS_PROVINCIA.AsString :=
...
begin
  ...
  if (AColumn.ClassType <> TdxDBGridCheckColumn)
    and ((AText = '') or (AText = '<NÃO INFORMADO>'))
...
begin
  ...
  if TB_MAPA_TESTAGEMDT_MAPA.IsNull then
  begin
    ShowMessage('O campo data não pode estar vazio');
    Abort;
  end;
end;
