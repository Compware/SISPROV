...
uses
  ...
  uFrameDashBoard, JPEG, IB_PARSE, RXCtrls, RpRenderRTF, RpRenderHTML,
  ...
  begin
    try
     try
      ...
      if id in [cObitoTarvProvincia] then
      begin
      ...
      if id in [cAcompTarvProvincia,cAcompPosiProvincia, cPosiTarvProvincia,
                cCriancaAcompTarvProvincia,cCriancasExpostasTrimestral,
                cGravidasPosiPTVTrimestral,cAbanTarvProvincia,
      ...
      else //if id in [cAdultoTestPosiProvincias] then
      begin
...
{     on E:Exception do
  ...
  if id=cAcompPosiProvincia then
    sAux := '<td class="ExpSubTit" align="center">Posi.</td>'+
            '<td class="ExpSubTit" align="center">Acomp.</td>'+
            '<td class="ExpSubTit" align="center">%</td>';
  ...
  if id=cCriancasExpostasTrimestral then
    sAux := '<td class="ExpSubTit" align="center">Receb.</td>'+
            '<td class="ExpSubTit" align="center">NReceb.</td>'+
            '<td class="ExpSubTit" align="center">%</td>';

  if id=cObitoTrimestral then
    sAux := '<td class="ExpSubTit" align="center">Crianca</td>'+
            '<td class="ExpSubTit" align="center">Adulto</td>'+
            '<td class="ExpSubTit" align="center">Total</td>';

  if id=cGravidasPosiPTVTrimestral then
    sAux := '<td class="ExpSubTit" align="center">PTV</td>'+
            '<td class="ExpSubTit" align="center">Posi.</td>'+
            '<td class="ExpSubTit" align="center">%</td>';

  if id=cAbanTarvProvincia then
    sAux := '<td class="ExpSubTit" align="center">TARV</td>'+
            '<td class="ExpSubTit" align="center">Aban.</td>'+
            '<td class="ExpSubTit" align="center">%</td>';

  if id=cTransTarvProvincia then
    sAux := '<td class="ExpSubTit" align="center">TARV</td>'+
            '<td class="ExpSubTit" align="center">Transf.</td>'+
            '<td class="ExpSubTit" align="center">%</td>';

  if id=cObitoTarvProvincia then
    sAux := '<td class="ExpSubTit" align="center">TARV</td>'+
            '<td class="ExpSubTit" align="center">Obito</td>'+
            '<td class="ExpSubTit" align="center">%</td>';
  ...
  try
    ...
    begin
      ...
      if id in [cAcompTarvProvincia,cAcompPosiProvincia, cPosiTarvProvincia,
                cCriancaAcompTarvProvincia,cCriancasExpostasTrimestral,cPessoasTestPosiProvincias,
                cGravidasPosiPTVTrimestral,cAbanTarvProvincia,cAdultoTestPosiProvincias,
                cTransTarvProvincia,cObitoTarvProvincia] then
      begin
        if vValor.Posi > 0 then
          vValor.Perc :=  (vValor.Test/vValor.Posi)*100;
      end
      else if id in [cObitoTrimestral,cAdultoAcompTarvProvincias] then
      begin
...
try
  ...
  begin
    case piRelatorio of
      ...
      cPessoasPosiProvincias : i:=6;
      cPessoasAconProvincias : i:=7;
      cPessoasAconTestProvincias : i:=8;
      cPessoasTestProvincias : i:=9;
      cAdultoSexoPosiProvincias : i:=10;
      cAdultoSexoTestPosiProvincias : i:=11;
      cAcompTarvProvincia : i:=12;
      cCriancaAcompTarvProvincia : i:=13;
      cPosiTarvProvincia : i:=14;
      cAcompPosiProvincia : i:=15;
      cAcompProvincia : i:=16;
      cTarvProvincia : i:=17;
      cCriancasExpostasTrimestral : i:=18;
      cObitoTrimestral : i:=19;
      cGravidasPosiPTVTrimestral : i:=20;
      cAbanTarvProvincia : i:=21;
      cTransTarvProvincia : i:=22;
      cObitoTarvProvincia : i:=23;
      cAdultoAcompTarvProvincias : i:=24;
    end;
...
{      on E:Exception do
    ...
    try
      ...
      begin
        sAux := Relatorio(sSQLProvinciaTrimestral, cFaixaCrianca, sTitulo, cCriancaTestPosiProvincias);
        sRel.Add(sAux);
      end;
    except
{      on E:Exception do
...
{      on E:Exception do
    ...
    sTitulo := 'Distribuição de gestantes, criancas e adultos aconselhadas e testadas por províncias';
    try
      if IncluirRelatorio(cPessoasAconTestProvincias) then
      begin
        sAux := Relatorio(sSQLAconTestProvincia, '',sTitulo, cPessoasAconTestProvincias);
        sRel.Add(sAux);
      end;
    except
{      on E:Exception do
...
{      on E:Exception do
    ...
    sTitulo := 'Distribuição de criancas adultos em obito por províncias';
    try
...
{      on E:Exception do
    ...
    sTitulo := 'Distribuição de grávidas positivas em PTV por províncias';
    try
...
{      on E:Exception do
    ...
    sTitulo := 'Distribuição de criancas e adultos Falecidos TARV por províncias';
    try
