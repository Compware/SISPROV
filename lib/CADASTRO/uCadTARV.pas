...
type
  TfrmCadTARV = class(TfrmCadPadFichas)
    Panel5: TPanel;
    sgTARV: TStringGrid;
    DBGrid1: TDBGrid;
  ...
  public
    ...
    procedure CarregaGrid(TB_TARV:TDataSet);
...
begin
   ...
   try
      ...
      sXML.Add('<?xml-stylesheet type="text/xsl" href="TARV.xslt"?>');
      sXML.Add('<Programa>');
      sXML.Add('  <Parametros>');
...
procedure TfrmCadTARV.CriaGrid;
var r,c:integer;
begin
  for r:=0 to 11 do
   for c:=0 to 8 do
...
procedure TfrmCadTARV.FormShow(Sender: TObject);
begin
  inherited;
  qyGrid.Open;
  CriaGrid;
...
procedure TfrmCadTARV.btgravarClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    if (dmGeral.TB_TARV.State = dsEdit) then sOP := 'A';
    if (dmGeral.TB_TARV.State = dsInsert) then sOP := 'I';
    ...
    try
      ...
      GeraXML;
      dmGeral.ProtocolaEnvio(dmGeral.TB_TARV, cvTARV, sOP, sArq);
    finally
...
procedure TfrmCadTARV.Varregrid;
...
begin
  ...
  begin
     ...
     begin
       if not (dmGeral.TB_TARV.State in [dsEdit,dsInsert]) then
         dmGeral.TB_TARV.Insert;

       dmGeral.TB_TARVCD_REGISTRO_GERAL.AsInteger := nRegistro;
       if cdUnidade <> null then
         dmGeral.TB_TARVCD_UNIDADE.AsInteger := cdUnidade;

       dmGeral.TB_TARVCD_PROVINCIA.AsInteger := cdProvincia;

       if cdMunicipio <> null then
         dmGeral.TB_TARVCD_MUNICIPIO.AsInteger := cdMunicipio;

       dmGeral.TB_TARVVL_MESANO.AsString := sMesAno;
       dmGeral.TB_TARVVL_Y.AsInteger := i;
       dmGeral.TB_TARVDT_REGISTRO.AsDateTime := dm.GetDateTime;

       if (i in [10]) then dmGeral.TB_TARVFL_GESTANTE.AsInteger := cvFlSim;
       if (i in [3,4]) then dmGeral.TB_TARVID_FAIXA_ETARIA.AsInteger := cvFaixaEtaria2;
       if (i in [7,8]) then dmGeral.TB_TARVID_FAIXA_ETARIA.AsInteger := cvFaixaEtaria6;
       if (i in [3,7]) then dmGeral.TB_TARVFL_SEXO.AsString := cvFeminino;
       if (i in [4,8]) then dmGeral.TB_TARVFL_SEXO.AsString := cvMasculino;
       if (i in [3,4,7,8,10]) then begin
         dmGeral.TB_TARVVL_ACUMULADOS.AsString := sgTARV.Cells[2,i];
         dmGeral.TB_TARVVL_NOVOS.AsString := sgTARV.Cells[3,i];
         dmGeral.TB_TARVVL_TRANSFERIDOS.AsString := sgTARV.Cells[4,i];
         dmGeral.TB_TARVVL_ABANDONOS.AsString := sgTARV.Cells[5,i];
         dmGeral.TB_TARVVL_TRANSFERIDOS_PARA.AsString := sgTARV.Cells[6,i];
         dmGeral.TB_TARVVL_OBITOS.AsString := sgTARV.Cells[7,i];
       end;

       dmGeral.TB_TARV.Post;
     end;
  end;
end;

procedure TfrmCadTARV.AbreConsultas;
begin
...
procedure TfrmCadTARV.FechaConsultas;
begin
  qyProvincia.Close;
  qyMunicipio.Close;
  qyUnidade.Close;
...
procedure TfrmCadTARV.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  FechaConsultas;
  //dmGeral.TB_TARV.SelectSQL.Text := sSQLTARV;
end;

procedure TfrmCadTARV.FormCreate(Sender: TObject);
begin
  nRelatorio := cvTARV;
  inherited;
  ...
  CarregaComboMesAno;
  bCarregaGrid := True;
  sSQLTARV := dmGeral.TB_TARV.SelectSQL.Text;
  sNomeTabela := 'REGISTRO_TARV';
end;

procedure TfrmCadTARV.CarregaDemo;
var i,j,x:integer;
begin
...
procedure TfrmCadTARV.CarregaComboMesAno;
...
procedure TfrmCadTARV.btConsultarClick(Sender: TObject);
var sSQLWhere, sSQL, sW:String; nB,nE:Integer;
begin
  inherited;
  if (dmGeral.TB_TARV.State in [dsEdit, dsInsert]) then
    Exit;

  dmGeral.TB_TARV.Close;
  dmGeral.TB_TARV.SelectSQL.Text := sSQLTARV;
  sSQL := sSQLTARV;
  ...
  begin
    ...
    dmGeral.TB_TARV.SelectSQL.Text := sSQL;
    if (trim(cbMesAno.Text) <> '/') and (trim(cbAte.Text) <> '/') then
    begin
      dmGeral.TB_TARV.ParamByName('de').AsString := FormatDateTime('mm/dd/yyyy',(StrToDate('01/'+cbMesAno.text)));
      dmGeral.TB_TARV.ParamByName('ate').AsString := FormatDateTime('mm/dd/yyyy',(StrToDate('01/'+cbAte.text)));
    end;
  ...
  begin
    sSQL := StringReplace(sSQL, 'from "REGISTRO_TARV"', 'from '+sNomeTabela+' REGISTRO_TARV',[]);
    dmGeral.TB_TARV.SelectSQL.Text := sSQL;
    bConsolidado := false;
  end;
  ...
  {if cbProvincia.KeyValue <> Null then
    dmGeral.TB_TARV.ParamByName('CD_PROVINCIA').AsInteger := cbProvincia.KeyValue
  ...
  if cbMesAno.Text <> '' then
    dmGeral.TB_TARV.ParamByName('VL_MESANO').AsString := cbMesAno.Text
  else
   begin
  ...
  if cbMunicipio.KeyValue <> Null then
    dmGeral.TB_TARV.ParamByName('CD_MUNICIPIO').AsInteger := cbMunicipio.KeyValue
      else if not ckConsolidar.Checked then
   begin
  ...
  if cbUnidade.KeyValue <> Null then
    dmGeral.TB_TARV.ParamByName('CD_UNIDADE').AsInteger := cbUnidade.KeyValue
      else if not ckConsolidar.Checked then
   begin
   ...
   //Felipe 08/05/2013
  if (leProvincia.LookupValue <> '') then
    dmGeral.TB_TARV.ParamByName('CD_PROVINCIA').AsInteger := strtoint(leProvincia.LookupValue);
  if (leMunicipio.LookupValue <> '') then
    dmGeral.TB_TARV.ParamByName('CD_MUNICIPIO').AsInteger := StrToInt(leMunicipio.LookupValue);
  if (leUnidade.LookupValue <> '') then
    dmGeral.TB_TARV.ParamByName('CD_UNIDADE').AsInteger := StrToInt(leUnidade.LookupValue);

  if not ckConsolidar.Checked then  
    if (trim(cbMesAno.Text) <> '') then
      dmGeral.TB_TARV.ParamByName('VL_MESANO').AsString := cbMesAno.Text;

  dmGeral.TB_TARV.Open;
  CarregaGrid(dmGeral.TB_TARV);

end;

procedure TfrmCadTARV.CarregaGrid(TB_TARV:TDataSet);
var rAux:real;
begin
    CriaGrid;
    dmGeral.TB_TARV.First;
    while Not dmGeral.TB_TARV.Eof do
    begin
      raux := valor(sgTARV.Cells[2,dmGeral.TB_TARVVL_Y.AsInteger]) +
              valor(dmGeral.TB_TARVVL_ACUMULADOS.AsString);
      sgTARV.Cells[2,dmGeral.TB_TARVVL_Y.AsInteger] := floattostr(raux);

      raux := valor(sgTARV.Cells[3,dmGeral.TB_TARVVL_Y.AsInteger]) +
              valor(dmGeral.TB_TARVVL_NOVOS.AsString);
      sgTARV.Cells[3,dmGeral.TB_TARVVL_Y.AsInteger] := floattostr(raux);

      raux := valor(sgTARV.Cells[4,dmGeral.TB_TARVVL_Y.AsInteger]) +
              valor(dmGeral.TB_TARVVL_TRANSFERIDOS.AsString);
      sgTARV.Cells[4,dmGeral.TB_TARVVL_Y.AsInteger] := floattostr(raux);

      raux := valor(sgTARV.Cells[5,dmGeral.TB_TARVVL_Y.AsInteger]) +
              valor(dmGeral.TB_TARVVL_ABANDONOS.AsString);
      sgTARV.Cells[5,dmGeral.TB_TARVVL_Y.AsInteger] := floattostr(raux);

      raux := valor(sgTARV.Cells[6,dmGeral.TB_TARVVL_Y.AsInteger]) +
              valor(dmGeral.TB_TARVVL_TRANSFERIDOS_PARA.AsString);
      sgTARV.Cells[6,dmGeral.TB_TARVVL_Y.AsInteger] := floattostr(raux);

      raux := valor(sgTARV.Cells[7,dmGeral.TB_TARVVL_Y.AsInteger]) +
              valor(dmGeral.TB_TARVVL_OBITOS.AsString);
      sgTARV.Cells[7,dmGeral.TB_TARVVL_Y.AsInteger] := floattostr(raux);
      
      dmGeral.TB_TARV.Next;
    end;
    CalculaTotal;
end;

procedure TfrmCadTARV.btexcluirClick(Sender: TObject);
begin
  pgGeral.ActivePageIndex := 1;
    ...
    //Felipe 08/05/2013
{
    dmGeral.TB_TARV.First;
    while not dmGeral.TB_TARV.Eof do
      dmGeral.TB_TARV.Delete;

    dmGeral.TB_TARV.Close; dmGeral.TB_TARV.Open;
    if bCarregaGrid then
      ...
      try
        sArq := sPathConsulta+'RelatorioTARV'+FormatDateTime('ddmmyyhhnnss',now)+'.xml';
        GeraXML;
        dmGeral.ProtocolaEnvio(dmGeral.TB_TARV, cvTARV, 'E', sArq);
      finally
        if FileExists(sArq) then
          DeleteFile(sArq);
      end;
...
procedure TfrmCadTARV.sgTARVKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmCadTARV.sgTARVDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
...
procedure TfrmCadTARV.sgTARVSelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin
  inherited;
...
procedure TfrmCadTARV.sgTARVSetEditText(Sender: TObject; ACol, ARow: Integer;
  const Value: String);
begin
  inherited;
  CalculaToTal;
...
procedure TfrmCadTARV.CalculaTotal;
...
procedure TfrmCadTARV.btIncluirClick(Sender: TObject);
begin
  pgGeral.ActivePageIndex := 1;
  dmGeral.TB_TARV.Close;
  dmGeral.TB_TARV.Open;
...
procedure TfrmCadTARV.btimprimirClick(Sender: TObject);
begin
...
procedure TfrmCadTARV.CriaDataSet;
var r,c:integer; nome,valor:string;
begin
...
procedure TfrmCadTARV.RvProject1Create(Sender: TObject);
begin
  inherited;
  RvProject1.ProjectFile := sPathLayout +'\RelATV.rav';
end;

function TfrmCadTARV.ValidaExclusao: boolean;
begin
  try
    if dmGeral.TB_TARV.State in [dsInsert] then
    begin
      //Primeiro deve Excluir, para depois entao incluir todos
      if dmGeral.TB_TARV.State in dsEditModes then
        dmGeral.TB_TARV.Cancel;
    ...
    if dmGeral.TB_TARV.State in [dsEdit] then
    begin
      //Primeiro deve Excluir, para depois entao incluir todos
      if dmGeral.TB_TARV.State in dsEditModes then
        dmGeral.TB_TARV.Cancel;
    ...
    dmGeral.TB_TARV.Close;
    if leUnidade.LookupValue <> '' then
      dmGeral.TB_TARV.ParamByName('CD_UNIDADE').AsInteger := Unidade;
    if leProvincia.LookupValue <> '' then
      dmGeral.TB_TARV.ParamByName('CD_PROVINCIA').AsInteger := Provincia;
    if leMunicipio.LookupValue <> '' then
      dmGeral.TB_TARV.ParamByName('CD_MUNICIPIO').AsInteger := Municipio;
    dmGeral.TB_TARV.ParamByName('VL_MESANO').AsString := MesAno;
    dmGeral.TB_TARV.Open;
...
procedure TfrmCadTARV.grDadosDblClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmCadTARV.btCancelarClick(Sender: TObject);
begin
  if bConsolidado then
  begin
    MessageDlg('Dados em modo de consolidação, não podem ser modificados!',mtInformation,[mbOK],0);
...
procedure TfrmCadTARV.bteditarClick(Sender: TObject);
begin
  pgGeral.ActivePageIndex := 1;
  pgGeralChange(sender);
  ...
  begin
    Unidade := dmGeral.TB_TARVCD_UNIDADE.AsInteger;
    Provincia := dmGeral.TB_TARVCD_PROVINCIA.AsInteger;
    Municipio := dmGeral.TB_TARVCD_MUNICIPIO.AsInteger;
    MesAno := dmGeral.TB_TARVVL_MESANO.AsString;
    inherited;
  end;

end;

procedure TfrmCadTARV.btLimparClick(Sender: TObject);
begin
  inherited;
  Limpar;
...
procedure TfrmCadTARV.ckConsolidarClick(Sender: TObject);
begin
  inherited;
  if ckConsolidar.Checked then
  begin
...
procedure TfrmCadTARV.leProvinciaChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmCadTARV.leMunicipioChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmCadTARV.leUnidadeChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmCadTARV.qyGridCalcFields(DataSet: TDataSet);
var sAux:String;
begin
  inherited;
...
procedure TfrmCadTARV.FormDestroy(Sender: TObject);
begin
  inherited;
  dmGeral.TB_TARV.SelectSQL.Text := sSQLTARV;
end;

procedure TfrmCadTARV.Validar;
begin
  if (leProvincia.LookupValue = '') then
  begin
    MessageDlg('É necessário informar Provincia!',mtInformation,[mbOK],0);
...
procedure TfrmCadTARV.cbMesAnoChange(Sender: TObject);
var texto: string;
begin
  inherited;
...
procedure TfrmCadTARV.cbMesAnoKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TfrmCadTARV.cbAteChange(Sender: TObject);
var texto: string;
begin
  inherited;
...
procedure TfrmCadTARV.cbAteKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
    if not (Key in ['0'..'9', #8]) then
    Key := #0
...
procedure TfrmCadTARV.sgTARVKeyPress(Sender: TObject; var Key: Char);
begin
    if not (Key in ['0'..'9', #8, #13, #0, #17, #18, #19, #20]) then
      Key := #0
end;

procedure TfrmCadTARV.cbMesAnoExit(Sender: TObject);
begin
  if ValidarMesAno(cbMesAno.Text) then
    cbMesAno.SetFocus;
end;

procedure TfrmCadTARV.cbAteExit(Sender: TObject);
begin
  if ValidarMesAno(cbAte.Text) then
    cbAte.SetFocus;
end;

procedure TfrmCadTARV.leUnidadeButtonClick(Sender: TObject);
begin
  inherited;
  qyUnidade.Close;
  qyUnidade.SQL.Text := USQL;
...
procedure TfrmCadTARV.BitBtn2Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      Database := DM.DB;
      SQL.Clear;
      SQL.Add('update "REGISTRO_TARV" set DT_EXP = NULL, DT_ALTERACAO = ''NOW'', DT_REGISTRO = ''NOW'' '+
