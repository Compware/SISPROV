...
begin
  ...
  if DMGERAL.TB_NOT_GESTANTENM_GEST.AsString = '' then
  begin
    MessageDlg('É necessário incluir o Nome da Utente!',mtInformation,
     [mbOK],0);
  ...
  begin
    MessageDlg('É necessário incluir a Idade da Utente!',mtInformation,
     [mbOK],0);
  ...
  begin
    ...
    begin
      MessageDlg('É necessário incluir o Nome da Criança!',mtInformation,
       [mbOK],0);
    ...
    begin
      MessageDlg('É necessário incluir a Idade da Criança!',mtInformation,
       [mbOK],0);
...
var
  ...
  iFlusaant         ,
  iFlnaousa         ,
  //Data alterei para string por conta da data padrão do componente quando forem null
  dNrnotificacao   ,
  dDtnascgest      ,
  dDtaproxgest     ,
  dDtdiagvih1      ,
...
begin
  ...
  //Numero Notificação GESTANTE 0000(UN)00000(GEN)
  ...
  inherited;
  if bGemelar then
  if MessageDlg('Parto Gemelar, deseja incluir uma nova a Notificação '+ #10#13 +
                'com os mesmos dados da Gestante atual?',mtConfirmation,[mbYes,mbNo], 0) = mrYes then
    IncluirNotGemelar; //Proxima notificacao automática
...
begin
  ...
  begin
    MessageDlg('É necessário incluir uma província!',mtInformation,[mbOK],0);
    abort;
  end;
...
begin
  ...
  begin
    MessageDlg('É necessário incluir um município!',mtInformation,[mbOK],0);
    abort;
  end;
...
begin
  ...
  begin
    ...
    if RxDBComboBox9.Text = 'Não' then
    begin
...
begin
  ...
  begin
    ...
    if RxDBComboBox14.Text = 'Não usou' then
    begin
...
begin
  ...
  begin
    ...
    if RxDBComboBox23.Text = 'Não' then
    begin
...
begin
  ...
  begin
    ...
    if (RxDBComboBox27.Text = 'Detectável') or (RxDBComboBox27.Text = 'Indetectável') then
    begin
...
begin
  ...
  begin
    ...
    if (RxDBComboBox28.Text = 'Detectável') or (RxDBComboBox28.Text = 'Indetectável') then
    begin
...
begin
  ...
  begin
    ...
    if (RxDBComboBox29.Text = 'Detectável') or (RxDBComboBox29.Text = 'Indetectável') then
    begin
...
begin
  ...
  if (dmGeral.TB_NOT_GESTANTE.State in [dsInsert, dsEdit]) and (pgGeral.TabIndex = 1) then
  begin
    if RxDBComboBox34.Text = 'Criança INFECTADA' then
    begin
    ...
    if RxDBComboBox34.Text = 'Criança NÃO INFECTADA' then
    begin
    ...
    if RxDBComboBox34.Text = 'Transferência' then
    begin
...
begin
  ...
  begin
    ...
    if RxDBComboBox22.Text = 'Não' then
    begin
...
begin
  ...
  begin
  if MessageDlg('Data de Parto é maior que a data de Nascimento da Criança.' +#10#13+
  ...
  if DBDateEdit9.Date < DBDateEdit11.Date then
  begin
  if MessageDlg('Data de Parto é menor que a data de Nascimento da Criança.' +#10#13+
               'Deseja continuar?',mtConfirmation,[mbYes,mbNo], 0) = mrNo then
    begin
      DBDateEdit11.Clear;
      abort;
