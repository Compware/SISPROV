object frmLocNotificacao: TfrmLocNotificacao
  Left = 261
  Top = 117
  Width = 562
  Height = 493
  BorderIcons = [biSystemMenu]
  Caption = 'Localizar Notifica'#231#227'o'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 554
    Height = 418
    Align = alClient
    TabOrder = 0
    object GroupBox1: TGroupBox
      Left = 9
      Top = 2
      Width = 528
      Height = 180
      Caption = 'Notifica'#231#227'o'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      object Label8: TLabel
        Left = 12
        Top = 16
        Width = 97
        Height = 13
        Caption = 'Data Notifica'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label9: TLabel
        Left = 11
        Top = 55
        Width = 141
        Height = 13
        Caption = 'Provincia da Notifica'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label10: TLabel
        Left = 11
        Top = 91
        Width = 144
        Height = 13
        Caption = 'Munic'#237'pio da Notifica'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label11: TLabel
        Left = 11
        Top = 128
        Width = 135
        Height = 13
        Caption = 'Unidade da Notifica'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label12: TLabel
        Left = 140
        Top = 16
        Width = 142
        Height = 13
        Caption = 'Data do Diagn'#243'stico VIH'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label13: TLabel
        Left = 347
        Top = 92
        Width = 100
        Height = 13
        Caption = 'Teste de Triagem'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label14: TLabel
        Left = 347
        Top = 128
        Width = 129
        Height = 13
        Caption = 'Teste de Confirmat'#243'rio'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edDataNotificacao: TDateEdit
        Left = 12
        Top = 31
        Width = 121
        Height = 21
        NumGlyphs = 2
        TabOrder = 0
      end
      object btProvinciaNot: TdxButtonEdit
        Left = 12
        Top = 69
        Width = 62
        TabOrder = 1
        OnExit = btProvinciaNotExit
        OnChange = btProvinciaNotChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              0505050507070714046A6A000101010101010101010101010F6A}
            Kind = bkGlyph
          end>
        OnButtonClick = btProvinciaNotButtonClick
        ExistButtons = True
      end
      object edProvinciaNot: TdxEdit
        Left = 76
        Top = 69
        Width = 265
        Color = cl3DLight
        TabOrder = 2
        TabStop = False
        ReadOnly = True
        StoredValues = 64
      end
      object btMunicipioNot: TdxButtonEdit
        Left = 12
        Top = 106
        Width = 62
        TabOrder = 3
        OnExit = btMunicipioNotExit
        OnChange = btMunicipioNotChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              0505050507070714046A6A000101010101010101010101010F6A}
            Kind = bkGlyph
          end>
        OnButtonClick = btMunicipioNotButtonClick
        ExistButtons = True
      end
      object edMunicipioNot: TdxEdit
        Left = 76
        Top = 106
        Width = 265
        Color = cl3DLight
        TabOrder = 4
        TabStop = False
        ReadOnly = True
        StoredValues = 64
      end
      object btUnidadeNot: TdxButtonEdit
        Left = 13
        Top = 143
        Width = 61
        TabOrder = 5
        OnExit = btUnidadeNotExit
        OnChange = btUnidadeNotChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              0505050507070714046A6A000101010101010101010101010F6A}
            Kind = bkGlyph
          end>
        OnButtonClick = btUnidadeNotButtonClick
        ExistButtons = True
      end
      object edUnidadeNot: TdxEdit
        Left = 76
        Top = 143
        Width = 265
        Color = cl3DLight
        TabOrder = 6
        TabStop = False
        ReadOnly = True
        StoredValues = 64
      end
      object edDataDiagnostico: TDateEdit
        Left = 140
        Top = 31
        Width = 121
        Height = 21
        NumGlyphs = 2
        TabOrder = 7
      end
      object edTesteTriagem: TComboBox
        Left = 348
        Top = 106
        Width = 173
        Height = 21
        ItemHeight = 13
        TabOrder = 8
        Items.Strings = (
          'Positivo'
          'N'#227'o Realizado')
      end
      object edTesteConfirmatorio: TComboBox
        Left = 347
        Top = 143
        Width = 173
        Height = 21
        ItemHeight = 13
        TabOrder = 9
        Items.Strings = (
          'Positivo'
          'N'#227'o Realizado')
      end
      object GroupBox3: TGroupBox
        Left = 348
        Top = 8
        Width = 173
        Height = 81
        Caption = 'Per'#237'odo'
        TabOrder = 10
        object Label16: TLabel
          Left = 20
          Top = 24
          Width = 21
          Height = 13
          Caption = 'De:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label17: TLabel
          Left = 18
          Top = 55
          Width = 24
          Height = 13
          Caption = 'At'#233':'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object edDe: TDateEdit
          Left = 44
          Top = 21
          Width = 121
          Height = 21
          NumGlyphs = 2
          TabOrder = 0
        end
        object edAte: TDateEdit
          Left = 44
          Top = 50
          Width = 121
          Height = 21
          NumGlyphs = 2
          TabOrder = 1
        end
      end
    end
    object GroupBox2: TGroupBox
      Left = 8
      Top = 187
      Width = 529
      Height = 216
      Caption = 'Dados do Utente'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      object Label1: TLabel
        Left = 11
        Top = 20
        Width = 93
        Height = 13
        Caption = 'Nome do Utente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 11
        Top = 57
        Width = 29
        Height = 13
        Caption = 'Sexo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 260
        Top = 56
        Width = 98
        Height = 13
        Caption = 'Data Nascimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 391
        Top = 55
        Width = 33
        Height = 13
        Caption = 'Idade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label7: TLabel
        Left = 11
        Top = 94
        Width = 79
        Height = 13
        Caption = 'Nome da M'#227'e'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 11
        Top = 132
        Width = 123
        Height = 13
        Caption = 'Prov'#237'ncia de Moradia'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 11
        Top = 172
        Width = 124
        Height = 13
        Caption = 'Munic'#237'pio de Moradia'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label15: TLabel
        Left = 349
        Top = 19
        Width = 70
        Height = 13
        Caption = 'Nr Processo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edNome: TEdit
        Left = 12
        Top = 34
        Width = 333
        Height = 21
        TabOrder = 0
      end
      object edSexo: TComboBox
        Left = 12
        Top = 71
        Width = 117
        Height = 21
        ItemHeight = 13
        TabOrder = 1
        Items.Strings = (
          'Feminino'
          'Masculino')
      end
      object ckGestante: TCheckBox
        Left = 136
        Top = 72
        Width = 113
        Height = 17
        Alignment = taLeftJustify
        BiDiMode = bdLeftToRight
        Caption = 'Se for Gestante'
        ParentBiDiMode = False
        TabOrder = 2
      end
      object edDataNasc: TDateEdit
        Left = 260
        Top = 71
        Width = 121
        Height = 21
        NumGlyphs = 2
        TabOrder = 3
      end
      object edIdade: TEdit
        Left = 391
        Top = 70
        Width = 62
        Height = 21
        TabOrder = 4
      end
      object edNomeMae: TEdit
        Left = 12
        Top = 108
        Width = 333
        Height = 21
        TabOrder = 5
      end
      object btProvinciaUtente: TdxButtonEdit
        Left = 12
        Top = 148
        Width = 62
        TabOrder = 6
        OnExit = btProvinciaUtenteExit
        OnChange = btProvinciaUtenteChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              0505050507070714046A6A000101010101010101010101010F6A}
            Kind = bkGlyph
          end>
        OnButtonClick = btProvinciaUtenteButtonClick
        ExistButtons = True
      end
      object edProvinciaUtente: TdxEdit
        Left = 76
        Top = 148
        Width = 269
        Color = cl3DLight
        TabOrder = 7
        TabStop = False
        ReadOnly = True
        StoredValues = 64
      end
      object btMunicipioUtente: TdxButtonEdit
        Left = 12
        Top = 187
        Width = 62
        TabOrder = 8
        OnExit = btMunicipioUtenteExit
        OnChange = btMunicipioUtenteChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              0505050507070714046A6A000101010101010101010101010F6A}
            Kind = bkGlyph
          end>
        OnButtonClick = btMunicipioUtenteButtonClick
        ExistButtons = True
      end
      object edMunicipioUtente: TdxEdit
        Left = 76
        Top = 187
        Width = 269
        Color = cl3DLight
        TabOrder = 9
        TabStop = False
        ReadOnly = True
        StoredValues = 64
      end
      object edprocesso: TdxEdit
        Left = 350
        Top = 34
        Width = 125
        TabOrder = 10
        ReadOnly = False
        StoredValues = 64
      end
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 418
    Width = 554
    Height = 41
    Align = alBottom
    TabOrder = 1
    object BitBtn1: TBitBtn
      Left = 303
      Top = 8
      Width = 105
      Height = 25
      Caption = 'Confirmar'
      ModalResult = 1
      TabOrder = 0
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object BitBtn2: TBitBtn
      Left = 416
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Cancelar'
      TabOrder = 1
      Kind = bkCancel
    end
  end
end
